...
/// <summary>
/// End-to-end tests for Aspire CLI with Go polyglot AppHost.
/// Tests creating a Go apphost and adding integrations.
/// Note: Does not run the apphost since Go runtime may not be available on CI.
/// </summary>
public sealed class PolyglotGoTests(ITestOutputHelper output)
{
    [Fact]
    public async Task CreateGoAppHostWithRedis()
    {
        ...
        var recordingPath = CliE2ETestHelpers.GetTestResultsRecordingPath(nameof(CreateGoAppHostWithRedis));
        ...
        // Step 1: Create Go apphost
        sequenceBuilder
            .Type("aspire init -l go")
            .Enter()
        ...
        var apphostFile = Path.Combine(workspace.WorkspaceRoot.FullName, "apphost.go");
        Assert.True(File.Exists(apphostFile), "apphost.go should exist");
