...
public class NodeFunctionalTests
{
    ...
    public async Task VerifyNodeAppWorks()
    {
        ...
        var response2 = await testProgram.PnpmAppBuilder!.HttpGetStringWithRetryAsync(client, "http", "/", cts.Token);
        var response3 = await testProgram.YarnAppBuilder!.HttpGetStringWithRetryAsync(client, "http", "/", cts.Token);
