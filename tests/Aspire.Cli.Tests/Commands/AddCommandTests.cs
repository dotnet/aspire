...
public class AddCommandTests(ITestOutputHelper outputHelper)
{
    ...
    [Fact]
    public async Task PromptForIntegrationVersionAsync_WithExplicitChannels_ShowsChannelsEntry()
    {
        // Arrange
        var interactionService = new TestConsoleInteractionService();
        var prompter = new AddCommandPrompter(interactionService);

        var implicitChannel = PackageChannel.CreateImplicitChannel(null!);
        var explicitChannel1 = PackageChannel.CreateExplicitChannel("Daily", Aspire.Cli.Packaging.PackageChannelQuality.Prerelease, [], null!);
        var explicitChannel2 = PackageChannel.CreateExplicitChannel("Stable", Aspire.Cli.Packaging.PackageChannelQuality.Stable, [], null!);

        var packages = new[]
        {
            ("redis", new NuGetPackage { Id = "Aspire.Hosting.Redis", Version = "1.0.0", Source = "nuget" }, implicitChannel),
            ("redis", new NuGetPackage { Id = "Aspire.Hosting.Redis", Version = "2.0.0-dev", Source = "daily" }, explicitChannel1),
            ("redis", new NuGetPackage { Id = "Aspire.Hosting.Redis", Version = "1.5.0", Source = "stable" }, explicitChannel2)
        };

        // Since TestConsoleInteractionService just returns the first choice,
        // and my implementation puts implicit versions first, followed by "Channels",
        // this test will verify the implicit version is returned when it's selected first.
        
        // Act
        var result = await prompter.PromptForIntegrationVersionAsync(packages, CancellationToken.None);

        // Assert - Should return the implicit version (highest semver) since it comes first in root menu
        Assert.Equal("redis", result.FriendlyName);
        Assert.Equal("Aspire.Hosting.Redis", result.Package.Id);
        Assert.Equal("1.0.0", result.Package.Version);
        Assert.Equal("nuget", result.Package.Source);
        Assert.Equal(implicitChannel, result.Channel);
    }

    [Fact]
    public async Task PromptForIntegrationVersionAsync_OnlyImplicitChannel_NoChannelsEntry()
    {
        // Arrange
        var interactionService = new TestConsoleInteractionService();
        var prompter = new AddCommandPrompter(interactionService);
        ...
        {
            ("redis", new NuGetPackage { Id = "Aspire.Hosting.Redis", Version = "1.0.0", Source = "nuget" }, implicitChannel),
            ("redis", new NuGetPackage { Id = "Aspire.Hosting.Redis", Version = "0.9.0", Source = "nuget" }, implicitChannel)
        };

        // Act
        var result = await prompter.PromptForIntegrationVersionAsync(packages, CancellationToken.None);

        // Assert - Should return the highest version (1.0.0) since implicit versions are ordered descending
        Assert.Equal("redis", result.FriendlyName);
        Assert.Equal("Aspire.Hosting.Redis", result.Package.Id);
        Assert.Equal("1.0.0", result.Package.Version); // Should be ordered descending by semver
        Assert.Equal(implicitChannel, result.Channel);
    }

    [Fact]
    public async Task PromptForIntegrationVersionAsync_OnlyExplicitChannels_ThrowsEmptyChoicesException()
    {
        // Arrange
        var interactionService = new TestConsoleInteractionService();
        var prompter = new AddCommandPrompter(interactionService);

        var explicitChannel = PackageChannel.CreateExplicitChannel("Daily", Aspire.Cli.Packaging.PackageChannelQuality.Prerelease, [], null!);

        var packages = new[]
        {
            ("redis", new NuGetPackage { Id = "Aspire.Hosting.Redis", Version = "2.0.0-dev", Source = "daily" }, explicitChannel)
        };

        // This test verifies that with only explicit channels, the root menu would only have "Channels"
        // When TestConsoleInteractionService selects the first choice ("Channels"), it drills down
        // Since TestConsoleInteractionService always selects first, it should go through the flow
        
        // Act & Assert
        var result = await prompter.PromptForIntegrationVersionAsync(packages, CancellationToken.None);
        
        // Should successfully return the explicit channel version
        Assert.Equal("redis", result.FriendlyName);
        Assert.Equal("Aspire.Hosting.Redis", result.Package.Id);
        Assert.Equal("2.0.0-dev", result.Package.Version);
        Assert.Equal(explicitChannel, result.Channel);
    }
