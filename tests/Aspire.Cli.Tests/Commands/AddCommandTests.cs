...
public class AddCommandTests(ITestOutputHelper outputHelper)
{
    ...
    [Fact]
    public async Task AddCommand_WithSingleFileAppHost_CallsAddPackageWithFileSwitch()
    {
        // Arrange
        string? usedProjectPath = null;
        string? usedPackageName = null;
        string? usedPackageVersion = null;
        ...
        var services = CliTestHelper.CreateServiceCollection(workspace, outputHelper, options =>
        {
            ...
            // Configure project locator to return a single-file AppHost
            options.ProjectLocatorFactory = _ =>
            {
                var locator = new TestProjectLocator();
                locator.UseOrFindAppHostProjectFileAsyncCallback = (projectFile, cancellationToken) =>
                {
                    // Return an apphost.cs file to simulate single file AppHost
            ...
            options.DotNetCliRunnerFactory = (sp) =>
            {
                ...
                runner.AddPackageAsyncCallback = (projectFilePath, packageName, packageVersion, nugetSource, options, cancellationToken) =>
                {
                    usedProjectPath = projectFilePath.FullName;
                    usedPackageName = packageName;
                    usedPackageVersion = packageVersion;
                    return 0; // Success.
                };
        ...
        // Assert
        Assert.Equal(0, exitCode);
        Assert.Equal("/tmp/apphost.cs", usedProjectPath);
        Assert.Equal("Aspire.Hosting.Redis", usedPackageName);
        Assert.Equal("9.2.0", usedPackageVersion);
    }
