    ...
    public async Task NewCommandDerivesOutputPathFromProjectNameForStarterTemplate()
    {
        ...
        var services = CliTestHelper.CreateServiceCollection(workspace, outputHelper, options => {
            ...
            options.NewCommandPrompterFactory = (sp) =>
            {
                ...
                {
                    // CLI mode: the default path is derived from the project name
                    Assert.Equal("./CustomName", path);
                    return path;
                };

                // In CLI mode, the subfolder prompt is not shown
                // (only shown in VS Code extension mode)
    ...
    public async Task NewCommandEscapesMarkupInProjectNameAndOutputPath()
    {
        ...
        var services = CliTestHelper.CreateServiceCollection(workspace, outputHelper, options =>
        {
            options.NewCommandPrompterFactory = (sp) =>
            {
                ...
                // In CLI mode, the derived path includes the project name with markup characters
                prompter.PromptForOutputPathCallback = (path) =>
                {
        ...
        // Verify the output path default is derived from the project name with markup characters
