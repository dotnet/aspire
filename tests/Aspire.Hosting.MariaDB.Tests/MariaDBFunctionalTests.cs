...
public class MariaDBFunctionalTests(ITestOutputHelper testOutputHelper)
{
    private static readonly Predicate<string> s_mariaDBReadyText = log => log.Contains("ready for connections") && log.Contains("port: 3306");
    ...
    public async Task VerifyWaitForOnMySqlBlocksDependentResources()
    {
        ...
        var resource = builder.AddMariaDB("resource")
                              .WithHealthCheck("blocking_check");

        var dependentResource = builder.AddMariaDB("dependentresource")
                                       .WaitFor(resource);
    ...
    public async Task VerifyMySqlResource()
    {
        ...
        var mariadbDbName = "db1";

        var mariadb = builder.AddMariaDB("mysql").WithEnvironment("MYSQL_DATABASE", mariadbDbName);
        var db = mariadb.AddDatabase(mariadbDbName);

        using var app = builder.Build();

        await app.StartAsync();

        await app.WaitForTextAsync(s_mariaDBReadyText).WaitAsync(TimeSpan.FromMinutes(2));
    ...
    public async Task WithDataShouldPersistStateBetweenUsages(bool useVolume)
    {
        ...
        try
        {
            ...
            var mysql1 = builder1.AddMariaDB("mariadb").WithEnvironment("MARIADB_DATABASE", mariaDBDbName);
            var password = mysql1.Resource.PasswordParameter.Value;

            var db1 = mysql1.AddDatabase(mariaDBDbName);
            ...
            var mariadDB2 = builder2.AddMariaDB("mariadb", passwordParameter2);
            var db2 = mariadDB2.AddDatabase(mariaDBDbName);

            if (useVolume)
            {
                mariadDB2.WithDataVolume(volumeName);
            }
            else
            {
                mariadDB2.WithDataBindMount(bindMountPath!);
            }
    ...
    public async Task VerifyWithInitBindMount()
    {
        ...
        try
        {
            ...
            var mariadb = builder.AddMariaDB("mariadb").WithEnvironment("MARIADB_DATABASE", mariaDBDbName);
            var db = mariadb.AddDatabase(mariaDBDbName);

            mariadb.WithInitBindMount(bindMountPath);
    ...
    [Fact]
    [RequiresDocker]
    public async Task VerifyEfMariaDB()
    {
        var cts = new CancellationTokenSource(TimeSpan.FromMinutes(5));
        var pipeline = new ResiliencePipelineBuilder()
