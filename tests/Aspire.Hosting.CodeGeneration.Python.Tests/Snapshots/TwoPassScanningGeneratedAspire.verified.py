    ...
    def __init__(self, handle: Handle, client: AspireClient, **kwargs: Unpack[ContainerResourceOptions]) -> None:
        if _bind_mount := kwargs.pop("bind_mount", None):
            if _validate_tuple_types(_bind_mount, (str, str)):
                rpc_args: dict[str, Any] = {"builder": handle}
                rpc_args["source"] = cast(tuple[str, str], _bind_mount)[0]
                rpc_args["target"] = cast(tuple[str, str], _bind_mount)[1]
                ...
                rpc_args: dict[str, Any] = {"builder": handle}
                rpc_args["source"] = cast(BindMountParameters, _bind_mount)["source"]
                rpc_args["target"] = cast(BindMountParameters, _bind_mount)["target"]
        ...
        if _image := kwargs.pop("image", None):
            if _validate_type(_image, str):
                ...
                rpc_args: dict[str, Any] = {"builder": handle}
                rpc_args["image"] = cast(tuple[str, str], _image)[0]
        ...
        if _volume := kwargs.pop("volume", None):
            if _validate_type(_volume, str):
                ...
                rpc_args: dict[str, Any] = {"resource": handle}
                rpc_args["target"] = cast(VolumeParameters, _volume)["target"]
                rpc_args["name"] = cast(VolumeParameters, _volume).get("name")
...
class ProjectResource(_BaseResource, ResourceWithEnvironment, ResourceWithArgs, ResourceWithServiceDiscovery, ResourceWithWaitSupport, ResourceWithProbes, ComputeResource, ContainerFilesDestinationResource):
    ...
    def __init__(self, handle: Handle, client: AspireClient, **kwargs: Unpack[ProjectResourceOptions]) -> None:
...
class TestDatabaseResource(ContainerResource):
    ...
    def __init__(self, handle: Handle, client: AspireClient, **kwargs: Unpack[TestDatabaseResourceOptions]) -> None:
        super().__init__(handle, client, **kwargs)
...
class TestRedisResource(ContainerResource, ResourceWithConnectionString):
    ...
    def add_test_child_database(self, name: str, *, database_name: str | None = None, **kwargs: Unpack[TestDatabaseResourceOptions]) -> TestDatabaseResource:
        ...
        return cast(TestDatabaseResource, result)
