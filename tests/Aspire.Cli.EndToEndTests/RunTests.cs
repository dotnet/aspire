...
public sealed class RunTests : IAsyncDisposable
{
    ...
    // Debug diagnostic tests - these use --debug flag to get verbose output
    // to help diagnose why .NET starter templates hang in CI

    [Fact]
    public async Task CreateAndRunAspireStarterProject_Debug()
    {
        var (prNumber, commitSha) = GetTestContext();
        _output.WriteLine($"[DEBUG] Testing aspire-starter from PR #{prNumber} (commit: {commitSha[..8]})");

        await using var builder = await AspireCliAutomationBuilder.CreateAsync(
            workingDirectory: _workDirectory,
            recordingName: "run-aspire-starter-debug",
            output: _output,
            prNumber: prNumber);
        ...
        await builder
            .CreateAspireStarterProject("StarterAppDebug", debug: true)
            .RunAspireProject("StarterAppDebug")
            .StopAspireProject()
            .ExitTerminal()
            .ExecuteAsync();
    }
    ...
    [Fact]
    public async Task CreateAndRunAspireTypeScriptCSharpStarterProject_Debug()
    {
        var (prNumber, commitSha) = GetTestContext();
        _output.WriteLine($"[DEBUG] Testing aspire-ts-cs-starter from PR #{prNumber} (commit: {commitSha[..8]})");

        await using var builder = await AspireCliAutomationBuilder.CreateAsync(
            workingDirectory: _workDirectory,
            recordingName: "run-aspire-ts-cs-starter-debug",
            output: _output,
            prNumber: prNumber);
        ...
        await builder
            .CreateAspireTypeScriptCSharpStarterProject("TsCsAppDebug", debug: true)
            .RunAspireProject("TsCsAppDebug")
            .StopAspireProject()
            .ExitTerminal()
            .ExecuteAsync();
    }

    private static (int prNumber, string commitSha) GetTestContext()
    {
        return (CliE2ETestHelpers.GetRequiredPrNumber(), CliE2ETestHelpers.GetRequiredCommitSha());
    }
