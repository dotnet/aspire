...
public abstract class StarterTemplateRunTestsBase<T> : WorkloadTestsBase, IClassFixture<T> where T : TemplateAppFixture
{
    ...
    public async Task WebFrontendWorks(string urlPrefix)
    {
        ...
        await CheckWebFrontendWorksAsync(context, url, _testOutput, hasRedisCache: HasRedisCache);
    }

    public static async Task CheckWebFrontendWorksAsync(IBrowserContext context, string url, ITestOutputHelper testOutput, bool hasRedisCache = false)
    {
