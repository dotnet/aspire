...
public class MarkdownProcessorTests
{
    ...
    [InlineData("http://localhost:8080", "http://localhost:8080")]
    [InlineData("https://localhost", "https://localhost")]
    public void ToHtml_UrlInsideCode_LinkAdded(string url, string text)
    {
        // Arrange
        var processor = CreateMarkdownProcessor();
    ...
    public void ToHtml_ResourceCode_ConvertedToLink(string code)
    {
        ...
        var processor = CreateMarkdownProcessor(dashboardClient: dashboardClient);
    ...
    internal static MarkdownProcessor CreateMarkdownProcessor(TelemetryRepository? telemetryRepository = null, IDashboardClient? dashboardClient = null)
    {
        var context = CreateAssistantChatDataContext(telemetryRepository, dashboardClient);

        return new MarkdownProcessor(context, new TestStringLocalizer<ControlsStrings>());
    }
}
