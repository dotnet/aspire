...
impl IDistributedApplicationBuilder {
    ...
    pub fn add_test_parent(&self, name: &str) -> Result<TestParentResource, Box<dyn std::error::Error>> {
        let mut args: HashMap<String, Value> = HashMap::new();
        args.insert("builder".to_string(), self.handle.to_json());
        args.insert("name".to_string(), serde_json::to_value(&name).unwrap_or(Value::Null));
        let result = self.client.invoke_capability("Aspire.Hosting.CodeGeneration.Rust.Tests/addTestParent", args)?;
        let handle: Handle = serde_json::from_value(result)?;
        Ok(TestParentResource::new(handle, self.client.clone()))
    }
}
...
/// Wrapper for Aspire.Hosting.CodeGeneration.Rust.Tests/Aspire.Hosting.CodeGeneration.TypeScript.Tests.TestTypes.TestChildResource
pub struct TestChildResource {
    handle: Handle,
    client: Arc<AspireClient>,
}

impl HasHandle for TestChildResource {
    fn handle(&self) -> &Handle {
        &self.handle
    }
}

impl TestChildResource {
    pub fn new(handle: Handle, client: Arc<AspireClient>) -> Self {
        Self { handle, client }
    }
    ...
    pub fn with_child_option(&self, option: &str) -> Result<TestChildResource, Box<dyn std::error::Error>> {
        let mut args: HashMap<String, Value> = HashMap::new();
        args.insert("builder".to_string(), self.handle.to_json());
        args.insert("option".to_string(), serde_json::to_value(&option).unwrap_or(Value::Null));
        let result = self.client.invoke_capability("Aspire.Hosting.CodeGeneration.Rust.Tests/withChildOption", args)?;
        let handle: Handle = serde_json::from_value(result)?;
        Ok(TestChildResource::new(handle, self.client.clone()))
    }
...
/// Wrapper for Aspire.Hosting.CodeGeneration.Rust.Tests/Aspire.Hosting.CodeGeneration.TypeScript.Tests.TestTypes.TestParentResource
pub struct TestParentResource {
    handle: Handle,
    client: Arc<AspireClient>,
}

impl HasHandle for TestParentResource {
    fn handle(&self) -> &Handle {
        &self.handle
    }
}

impl TestParentResource {
    pub fn new(handle: Handle, client: Arc<AspireClient>) -> Self {
        Self { handle, client }
    }
    ...
    pub fn add_child(&self, name: &str) -> Result<TestChildResource, Box<dyn std::error::Error>> {
        let mut args: HashMap<String, Value> = HashMap::new();
        args.insert("builder".to_string(), self.handle.to_json());
        args.insert("name".to_string(), serde_json::to_value(&name).unwrap_or(Value::Null));
        let result = self.client.invoke_capability("Aspire.Hosting.CodeGeneration.Rust.Tests/addChild", args)?;
        let handle: Handle = serde_json::from_value(result)?;
        Ok(TestChildResource::new(handle, self.client.clone()))
    }
