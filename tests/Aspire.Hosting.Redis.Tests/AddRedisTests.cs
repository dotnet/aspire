...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    [Fact]
    public void WithRedisMcpAddsHttpEndpoint()
    {
        using var builder = TestDistributedApplicationBuilder.Create();
        var redis = builder.AddRedis("redis").WithRedisMcp();
    ...
    [Fact]
    public void WithRedisMcpAddsParentRelationship()
    {
        using var builder = TestDistributedApplicationBuilder.Create();
        var redis = builder.AddRedis("redis").WithRedisMcp();
    ...
    [Fact]
    public async Task WithRedisMcpPublishesMcpEndpointAnnotationWhenEndpointAllocated()
    {
        using var builder = TestDistributedApplicationBuilder.Create();
        var redis = builder.AddRedis("redis")
            .WithEndpoint("tcp", e => e.AllocatedEndpoint = new AllocatedEndpoint(e, "localhost", 6379))
