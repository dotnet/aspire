...
public sealed class VnetStorageBlobConnectivityDeploymentTests(ITestOutputHelper output)
{
    ...
    private async Task DeployStarterTemplateWithStorageBlobPrivateEndpointCore(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (DeploymentE2ETestHelpers.IsRunningInCI)
            {
                await auto.WaitUntilAsync(s => waitingForAddVersionSelectionPrompt.Search(s).Count > 0, timeout: TimeSpan.FromSeconds(60), description: "version selection prompt for AppContainers");
                await auto.EnterAsync();
            }
            ...
            if (DeploymentE2ETestHelpers.IsRunningInCI)
            {
                await auto.WaitUntilAsync(s => waitingForAddVersionSelectionPrompt.Search(s).Count > 0, timeout: TimeSpan.FromSeconds(60), description: "version selection prompt for Network");
                await auto.EnterAsync();
            }
            ...
            if (DeploymentE2ETestHelpers.IsRunningInCI)
            {
                await auto.WaitUntilAsync(s => waitingForAddVersionSelectionPrompt.Search(s).Count > 0, timeout: TimeSpan.FromSeconds(60), description: "version selection prompt for Storage");
                await auto.EnterAsync();
            }
