...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    [Fact]
    public void AddAsExistingResource_RespectsExistingAzureResourceAnnotation_ForAzureServiceBusResource()
    {
        // Arrange
        using var builder = TestDistributedApplicationBuilder.Create();
        ...
        var serviceBusResource = new AzureServiceBusResource("test-servicebus", _ => { });
        serviceBusResource.Annotations.Add(new ExistingAzureResourceAnnotation(existingName.Resource, existingResourceGroup.Resource));
        
        var infrastructure = new AzureResourceInfrastructure(serviceBusResource, "test-servicebus");

        // Act
        var result = serviceBusResource.AddAsExistingResource(infrastructure);
    ...
    [Fact]
    public void AddAsExistingResource_FallsBackToDefault_WhenNoAnnotation_ForAzureServiceBusResource()
    {
        // Arrange
        var serviceBusResource = new AzureServiceBusResource("test-servicebus", _ => { });
        var infrastructure = new AzureResourceInfrastructure(serviceBusResource, "test-servicebus");
