...
resource env_playwright 'Microsoft.LoadTestService/playwrightworkspaces@2025-07-01-preview' = {
  location: location
  name: take('pw-${uniqueString(resourceGroup().id)}', 24)
  properties: {
...
output AZURE_PLAYWRIGHT_WORKSPACE_NAME string = env_playwright.name

output AZURE_PLAYWRIGHT_WORKSPACE_ID string = env_playwright.id
