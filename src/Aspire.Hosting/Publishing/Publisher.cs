    ...
    IServiceProvider serviceProvider) : IDistributedApplicationPublisher
{
    public async Task PublishAsync(DistributedApplicationModel model, CancellationToken cancellationToken)
    {
        ...
        await progressReporter.CompleteStepAsync(
                    ...
                    taskInfo.State == CompletionState.CompletedWithError ? CompletionState.CompletedWithError : CompletionState.Completed,
                    cancellationToken)
                    .ConfigureAwait(false);
        ...
        if (taskInfo.State == CompletionState.CompletedWithError)
        {
            ...
            var aggregatedState = progressReporter is PublishingActivityProgressReporter reporter 
                ? reporter.GetAggregatedCompletionState() 
                : CompletionState.CompletedWithError;
            
            await progressReporter.CompletePublishAsync(aggregatedState, cancellationToken)
                .ConfigureAwait(false);
