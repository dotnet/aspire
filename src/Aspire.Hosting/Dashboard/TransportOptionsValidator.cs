...
internal class TransportOptionsValidator(IOptions<DashboardOptions> dashboardOptions, DistributedApplicationExecutionContext executionContext, DistributedApplicationOptions distributedApplicationOptions) : IValidateOptions<TransportOptions>
{
    public ValidateOptionsResult Validate(string? name, TransportOptions transportOptions)
    {
        ...
        var dashboardUrls = dashboardOptions.Value.DashboardUrl;
        if (string.IsNullOrEmpty(dashboardUrls))
        {
            return ValidateOptionsResult.Fail($"Dashboard URL could not be determined. AppHost does not have applicationUrl in launch profile and the {KnownConfigNames.AspNetCoreUrls} environment variable is not set.");
        }
        else
        {
            var firstApplicationUrl = dashboardUrls.Split(";").First();

            if (!Uri.TryCreate(firstApplicationUrl, UriKind.Absolute, out var parsedFirstApplicationUrl))
            {
                return ValidateOptionsResult.Fail($"Dashboard URL is invalid. The 'applicationUrl' setting of the launch profile has value '{firstApplicationUrl}' which could not be parsed as a URI.");
            }

            if (parsedFirstApplicationUrl.Scheme == "http")
            {
                return ValidateOptionsResult.Fail($"Dashboard URL is invalid. The 'applicationUrl' setting must be an https address unless the '{KnownConfigNames.AllowUnsecuredTransport}' environment variable is set to true. This configuration is commonly set in the launch profile. See https://aka.ms/dotnet/aspire/allowunsecuredtransport for more details.");
            }
        }

        // Validate ASPIRE_DASHBOARD_OTLP_ENDPOINT_URL
        var dashboardOtlpGrpcEndpointUrl = dashboardOptions.Value.OtlpGrpcEndpointUrl;
        var dashboardOtlpHttpEndpointUrl = dashboardOptions.Value.OtlpHttpEndpointUrl;
        ...
        var resourceServiceEndpointUrl = dashboardOptions.Value.ResourceServiceUrl;
        if (string.IsNullOrEmpty(resourceServiceEndpointUrl))
        {
            return ValidateOptionsResult.Fail($"AppHost does not have the {KnownConfigNames.ResourceServiceEndpointUrl} setting defined or default value was cleared.");
        }
        else
        {
