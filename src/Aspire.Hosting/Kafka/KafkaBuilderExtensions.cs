...
public static class KafkaBuilderExtensions
{
    ...
    public static IResourceBuilder<KafkaContainerResource> AddKafkaContainer(this IDistributedApplicationBuilder builder, string name, int? port = null)
    {
        ...
        return builder.AddResource(kafka)
             .WithAnnotation(new EndpointAnnotation(ProtocolType.Tcp, port: port, containerPort: KafkaBrokerPort))
             ...
             {
                 int hostPort = context.PublisherName == "manifest"
                     ? KafkaBrokerPort
                     :  kafka.GetPort();
                 context.EnvironmentVariables.Add("KAFKA_ADVERTISED_LISTENERS", $"PLAINTEXT://localhost:29092,PLAINTEXT_HOST://localhost:{hostPort}");
             });
    ...
    public static IResourceBuilder<KafkaServerResource> AddKafka(this IDistributedApplicationBuilder builder, string name)
    {
        ...
        return builder.AddResource(kafka)
            .WithAnnotation(new EndpointAnnotation(ProtocolType.Tcp, containerPort: 9092))
