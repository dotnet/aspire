...
```csharp
builder.AddAzureWebPubSub("WebPubSubConnectionName", "your_hub_name");
```
...
```csharp
    builder.AddAzureWebPubSub("wps", "your_hub_name", settings => settings.HealthChecks = false);
```
...
```csharp
    builder.AddAzureWebPubSub("wps", "your_hub_name", configureClientBuilder: clientBuilder => clientBuilder.ConfigureOptions(options => options.Identifier = "CLIENT_ID"));
```
