...
internal sealed class AzureEventHubsEmulatorConfigWriterHook : IDistributedApplicationLifecycleHook
{
    ...
    {
        if (appModel.Resources.OfType<AzureEventHubsResource>().SingleOrDefault() is not { } emulatorResource || !emulatorResource.IsEmulator)
        {
        ...
        if (!emulatorResource.Hubs.Any())
        {
            // No-op if there are no Azure Event Hubs resources present.
            return Task.CompletedTask;
        }
