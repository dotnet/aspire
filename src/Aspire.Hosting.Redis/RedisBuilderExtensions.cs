...
public static class RedisBuilderExtensions
{
    ...
    /// <summary>
    /// Configures an MCP (Model Context Protocol) server for the <see cref="RedisResource"/> that enables AI agents to interact with Redis data.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The MCP server runs as a stdio-based process using the <c>redis-mcp-server</c> package via <c>uvx</c>.
    /// It is exposed via an HTTP endpoint that the Aspire Dashboard can connect to.
    /// </para>
    /// <para>
    /// The Redis MCP server provides tools for interacting with Redis data structures including strings, hashes, lists,
    /// sets, sorted sets, streams, JSON documents, and more.
    /// </para>
    /// </remarks>
    /// <param name="builder">The <see cref="IResourceBuilder{T}"/> for the <see cref="RedisResource"/>.</param>
    /// <param name="configureResource">Configuration callback for the MCP bridge resource.</param>
    /// <param name="resourceName">Override the name used for the MCP bridge resource.</param>
    /// <returns>The <see cref="IResourceBuilder{T}"/> for the <see cref="RedisResource"/>.</returns>
    public static IResourceBuilder<RedisResource> WithRedisMcp(
        this IResourceBuilder<RedisResource> builder,
        Action<IResourceBuilder<McpBridgeResource>>? configureResource = null,
        string? resourceName = null)
    {
        ArgumentNullException.ThrowIfNull(builder);

        resourceName ??= $"{builder.Resource.Name}-mcp";

        var resourceBuilder = builder.ApplicationBuilder.AddMcpBridge(
            resourceName,
            "uvx",
            ["--from", "redis-mcp-server@latest", "redis-mcp-server"])
            .WithMcpNamespace("redis")
            .WithServerArgument("--url", () => builder.Resource.UriExpression)
            .WaitFor(builder)
