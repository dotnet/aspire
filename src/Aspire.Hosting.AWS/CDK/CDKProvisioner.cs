    ...
    ResourceLoggerService loggerService) : CloudFormationProvisioner(appModel, notificationService, loggerService)
{
    ...
    private async Task ProcessAppResourceAsync(AppResource appResource, CancellationToken cancellationToken = default)
    {
        ...
        var templates = cloudAssembly.Stacks.Where(stack => stack.Assets.Length == 0).Select(stack => new CDKStackTemplate(stack, stackResources[stack.StackName])).ToArray();
    ...
    /*public async Task BeforeStartAsync(DistributedApplicationModel appModel, CancellationToken cancellationToken = default)
    {
        ...
        // Skip provisioning when there is not
        var appResource = appModel.Resources.OfType<AppResource>().FirstOrDefault();
        if (appResource == null)
        {
            return;
        }
        await context.PublishUpdateStateAsync(appResource, Constants.ResourceStateStarting).ConfigureAwait(false);
        appResource.ProvisioningTaskCompletionSource = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                await ProvisionStackResources(appModel, cancellationToken).ConfigureAwait(false);
                await context.PublishUpdateStateAsync(appResource, Constants.ResourceStateRunning).ConfigureAwait(false);
                appResource.ProvisioningTaskCompletionSource?.TrySetResult();
            }
            catch (Exception ex)
            {
                await context.PublishUpdateStateAsync(appResource, Constants.ResourceStateFailedToStart).ConfigureAwait(false);
                appResource.ProvisioningTaskCompletionSource?.TrySetException(ex);
            }
        }, cancellationToken);
    }
