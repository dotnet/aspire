...
public static class AzureAIFoundryExtensions
{
    ...
    /// <param name="builder">The resource to which the specified roles will be assigned.</param>
    /// <param name="target">The target Azure AI Foundry resource.</param>
    /// <param name="roles">The Azure AI Foundry roles to be assigned (for example, <see cref="AzureAIFoundryRole.CognitiveServicesOpenAIUser"/>).</param>
    /// <returns>The updated <see cref="IResourceBuilder{T}"/> with the applied role assignments.</returns>
    /// <exception cref="ArgumentException">Thrown when a role value is not a valid <see cref="AzureAIFoundryRole"/> value.</exception>
    [AspireExport("withAIFoundryRoleAssignments", Description = "Assigns Azure AI Foundry roles to a resource")]
