    ...
    public async Task<string> GetModelsAsync(string? continuationToken = null)
    {
        ...
        var basePayload = """
        {
            ...
            "indexEntitiesRequest": {
                ...
                "pageSize": 250,
                "facets": [
                    {"name": "annotations/systemCatalogData/inferenceTasks", "count": 100},
                    {"name": "annotations/systemCatalogData/fineTuningTasks", "count": 100},
                    {"name": "annotations/systemCatalogData/license", "count": 100},
                    {"name": "annotations/systemCatalogData/deploymentTypes", "count": 100},
                    {"name": "annotations/systemCatalogData/publisher", "count": 100},
                    {"name": "annotations/systemCatalogData/azureOffers", "count": 100},
                    {"name": "annotations/systemCatalogData/modelCapabilities", "count": 100}
                ],
        ...
        {
            // Parse the base payload and add continuation token
            var payloadDoc = JsonDocument.Parse(basePayload);
            var root = payloadDoc.RootElement.Clone();

            // Create a new payload with the continuation token added
            var modifiedPayload = new
            {
                resourceIds = root.GetProperty("resourceIds"),
                indexEntitiesRequest = root.GetProperty("indexEntitiesRequest"),
                continuationToken = continuationToken
            };

            jsonContent = JsonSerializer.Serialize(modifiedPayload);
        }
        else
        {
