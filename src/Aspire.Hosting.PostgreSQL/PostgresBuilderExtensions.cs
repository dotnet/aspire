...
public static class PostgresBuilderExtensions
{
    ...
    private static IResourceBuilder<PostgresServerResource> WithAgentContent(this IResourceBuilder<PostgresServerResource> builder)
    {
#pragma warning disable ASPIREAGENTCONTENT001 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
        return builder.WithAgentContent((context =>
        {
            if (context.Resource is not PostgresServerResource resource)
            {
            ...
            context.AppendText(
                ...
                The '{{resource.Name}}' is a Postgres database server. It is managed by Aspire and is running
                as a container application. The following information relates to this specific instance of the
                resource. From time to time the model should refresh this information especially if previous
                information does not seem accurate anymore. This information can only be fetched
                if the apphost is running.
                """
            );

            context.AppendLine("\n## Databases");
            if (resource.Databases.Any())
            {
                foreach (var db in resource.Databases)
                {
                    context.AppendText($"- `{db.Key} (resource name in app model: {db.Value})`");
                }
            }
            else
            {
                context.AppendText("No databases have been defined on this resource yet.");
            }

            context.AppendLine("\n## Bind Mounts");
            var bindMounts = resource.Annotations.OfType<ContainerMountAnnotation>()
                .Where(m => m.Type == ContainerMountType.BindMount);
            if (bindMounts.Any())
            {
                foreach (var mount in bindMounts)
                {
                    context.AppendText($"- Source: `{mount.Source}` -> Target: `{mount.Target}` (ReadOnly: {mount.IsReadOnly})");
                }
            }
            else
            {
                context.AppendText("No bind mounts have been defined on this resource.");
            }

            context.AppendLine("\n## Data Volumes");
            var volumes = resource.Annotations.OfType<ContainerMountAnnotation>()
                .Where(m => m.Type == ContainerMountType.Volume);
            if (volumes.Any())
            {
                foreach (var volume in volumes)
                {
                    context.AppendText($"- Name: `{volume.Source}` -> Target: `{volume.Target}` (ReadOnly: {volume.IsReadOnly})");
                }
            }
            else
            {
                context.AppendText("No data volumes have been defined on this resource.");
            }
            ...
            context.AppendText(
                $$"""
                The following guidance provides useful information for connecting to the {{resource.Name}} Postgres server
                from application code running within Aspire.

                ### Scripting databases

                ### .NET - client connectivity
                To connect to the Postgres server from a .NET application add a reference to the .NET application
                in the app model using the `WithReference(...)` extension method. This will automatically inject
                connection strings into the app at runtime.


                Aspire includes specific client libraries which simplify the configuration of database connections
                to Postgres. To use these libraries install the `Aspire.Npgsql` package an use the following code
                in the application entrypoint (assuming the use of the host builder).

                ```csharp
                builder.AddNpgsqlDatasource("{{resource.Name}}");
                ```

                ### Python - client connectivity
                The most popular PostgreSQL client driver for Python is `psycopg2` (or `psycopg` for the newer version 3).
                ...
                conn = psycopg2.connect(
                    host=os.environ["{{resource.Name}}_HOST"],
                    port=os.environ["{{resource.Name}}_PORT"],
                    user=os.environ["{{resource.Name}}_USERNAME"],
                    password=os.environ["{{resource.Name}}_PASSWORD"],
