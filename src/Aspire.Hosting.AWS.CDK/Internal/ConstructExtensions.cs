...
internal static class ConstructExtensions
{
    ...
    public static string StackUniqueId(this Construct construct, string id)
    {
        var placeholder = new Construct(construct, id);
        var uniqueId = placeholder.StackUniqueId();
        construct.Node.TryRemoveChild(placeholder.Node.Id);
        return uniqueId;
    }
}
