...
internal static class CommonAgentApplicators
{
    ...
    /// <summary>The relative skill base directory for the industry-standard location.</summary>
    private static readonly string s_standardSkillBaseDirectory = Path.Combine(".agents", "skills");

    /// <summary>Relative path to the Aspire skill file under the standard location.</summary>
    private static readonly string s_standardAspireSkillRelativePath = Path.Combine(".agents", "skills", AspireSkillName, "SKILL.md");

    /// <summary>Relative path to the dotnet-inspect skill file under the standard location.</summary>
    private static readonly string s_standardDotnetInspectSkillRelativePath = Path.Combine(".agents", "skills", DotnetInspectSkillName, "SKILL.md");

    /// <summary>
    /// Adds skill file applicators for the industry-standard <c>.agents/skills/</c> locations.
    /// This includes workspace-level (<c>&lt;workspace&gt;/.agents/skills/</c>) and
    /// user-level (<c>~/.agents/skills/</c>) skill files.
    /// </summary>
    /// <param name="context">The scan context.</param>
    /// <param name="workspaceRoot">The workspace root directory.</param>
    /// <param name="homeDirectory">The user's home directory for user-level skill files.</param>
    public static void TryAddStandardLocationSkillFileApplicators(
        AgentEnvironmentScanContext context,
        DirectoryInfo workspaceRoot,
        DirectoryInfo homeDirectory)
    {
        // Workspace-level .agents/skills/
        TryAddSkillFileApplicator(
            context,
            workspaceRoot,
            s_standardAspireSkillRelativePath,
            "Create Aspire skill file (.agents/skills/aspire/SKILL.md)");

        TryAddSkillFileApplicator(
            context,
            workspaceRoot,
            s_standardDotnetInspectSkillRelativePath,
            "Create dotnet-inspect skill file (.agents/skills/dotnet-inspect/SKILL.md)",
            DotnetInspectSkillFileContent);

        context.AddSkillBaseDirectory(s_standardSkillBaseDirectory);

        // User-level ~/.agents/skills/
        // Use absolute paths as deduplication keys to differentiate from workspace-level
        TryAddSkillFileApplicator(
            context,
            homeDirectory,
            s_standardAspireSkillRelativePath,
            "Create Aspire skill file (~/.agents/skills/aspire/SKILL.md)",
            deduplicationKey: Path.Combine(homeDirectory.FullName, s_standardAspireSkillRelativePath));
        ...
        TryAddSkillFileApplicator(
            ...
            "Create dotnet-inspect skill file (~/.agents/skills/dotnet-inspect/SKILL.md)",
            DotnetInspectSkillFileContent,
            deduplicationKey: Path.Combine(homeDirectory.FullName, s_standardDotnetInspectSkillRelativePath));
    }
