...
internal sealed class ExtensionInternalCommand : BaseCommand
{
    ...
    private sealed class GetAppHostCandidatesCommand : BaseCommand
    {
        ...
        public GetAppHostCandidatesCommand(IFeatures features, ICliUpdateNotifier updateNotifier, IProjectLocator projectLocator, CliExecutionContext executionContext, IInteractionService interactionService) : base("get-apphosts", "Get AppHosts in the specified directory", features, updateNotifier, executionContext, interactionService)
        {
            ...
            var directoryOption = new Option<string?>("--project");
            directoryOption.Description = "The project file or directory to search for AppHost projects.";
        ...
        {
            var projectArgument = parseResult.GetValue<string?>("--project");
            var projectFile = projectArgument is not null ? new FileInfo(projectArgument) : new FileInfo(Environment.CurrentDirectory);
            try
            {
                var result = await _projectLocator.UseOrFindAppHostProjectFileAsync(projectFile, MultipleAppHostProjectsFoundBehavior.First, cancellationToken);
