...
internal sealed class DeployCommand : PipelineCommandBase
{
    ...
    public DeployCommand(IDotNetCliRunner runner, IInteractionService interactionService, IProjectLocator projectLocator, AspireCliTelemetry telemetry, IDotNetSdkInstaller sdkInstaller, IFeatures features, ICliUpdateNotifier updateNotifier, CliExecutionContext executionContext, ICliHostEnvironment hostEnvironment, Microsoft.Extensions.Logging.ILoggerFactory? loggerFactory, Diagnostics.FileLoggerProvider? fileLoggerProvider)
        : base("deploy", DeployCommandStrings.Description, runner, interactionService, projectLocator, telemetry, sdkInstaller, features, updateNotifier, executionContext, hostEnvironment, loggerFactory, fileLoggerProvider)
    {
