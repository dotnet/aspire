    ...
    public async Task<int> RunAsync(AppHostProjectContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (!watch)
            {
                ...
                var extensionHasBuildCapability = extensionBackchannel is not null && await extensionBackchannel.HasCapabilityAsync(KnownExtensionCapabilities.BuildDotnetUsingCli, cancellationToken);
