...
internal sealed class RunningInstanceManager
{
    ...
    public async Task<bool> StopRunningInstanceAsync(string socketPath, CancellationToken cancellationToken)
    {
        try
        {
            ...
            _interactionService.DisplayMessage(KnownEmojis.StopSign, string.Format(CultureInfo.CurrentCulture, RunCommandStrings.StoppingPreviousInstance, appHostInfo.ProcessId.ToString(CultureInfo.InvariantCulture), cliPidText));
