...
public class AzureBicepResource :
    ...
    IAzureResource
{
    private AzureBicepAnnotation Annotation => Annotations.OfType<AzureBicepAnnotation>().Last();

    internal string? TemplateFile => Annotation.TemplateFile;

    internal string? TemplateString => Annotation.TemplateString;

    internal string? TemplateResourceName => Annotation.TemplateResourceName;

    /// <summary>
    /// Parameters that will be passed into the bicep template.
    /// </summary>
    public Dictionary<string, object?> Parameters => Annotation.Parameters;

    /// <summary>
    /// Outputs that will be generated by the bicep template.
    /// </summary>
    public Dictionary<string, object?> Outputs => Annotation.Outputs;

    /// <summary>
    /// Secret outputs that will be generated by the bicep template.
    /// </summary>
    public Dictionary<string, string?> SecretOutputs => Annotation.SecretOutputs;

    /// <summary>
    /// The task completion source for the provisioning operation.
    /// </summary>
    public TaskCompletionSource? ProvisioningTaskCompletionSource
    {
        get => Annotation.ProvisioningTaskCompletionSource;
        set
        {
            if (value is not null)
            {
                Annotation.ProvisioningTaskCompletionSource = value;
            }
        }
    }
    ...
    {
        Annotations.Add(new AzureBicepAnnotation(templateFile, templateString, templateResourceName));
        Annotations.Add(new ManifestPublishingCallbackAnnotation(WriteToManifest));
    }

    /// <summary>
    /// Initializes a new instance of <see cref="AzureBicepResource"/>.
    /// </summary>
    /// <param name="name"></param>
    /// <param name="resourceAnnotations"></param>
    public AzureBicepResource(string name, ResourceAnnotationCollection resourceAnnotations) : base(name, resourceAnnotations)
    {
        Annotations.Add(new ManifestPublishingCallbackAnnotation(WriteToManifest));
    }
