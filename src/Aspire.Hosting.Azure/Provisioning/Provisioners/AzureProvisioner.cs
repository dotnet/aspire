...
internal sealed class AzureProvisioner(
    ...
    IBicepCliInvoker bicepCliInvoker,
    ...
    ) : IDistributedApplicationLifecycleHook
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var provisioningContextLazy = new Lazy<Task<ProvisioningContext>>(() => provisioningContextProvider.GetProvisioningContextAsync(cancellationToken));

        var tasks = new List<Task>();

        foreach (var resource in azureResources)
        {
            tasks.Add(ProcessResourceAsync(provisioningContextLazy, resource, cancellationToken));
        }
        ...
        {
            logger.LogError(ex, "Failed to save user secrets.");
        }
    ...
    private async Task ProcessResourceAsync(Lazy<Task<ProvisioningContext>> provisioningContextLazy, (IResource Resource, IAzureResource AzureResource) resource, CancellationToken cancellationToken)
    {
        ...
        // Only process AzureBicepResource instances
        if (resource.AzureResource is not AzureBicepResource bicepResource)
        {
            resource.AzureResource.ProvisioningTaskCompletionSource?.TrySetResult();
            resourceLogger.LogWarning("Skipping {resourceType} because it is not a bicep resource.", resource.GetType().Name);
            return;
        }
        ...
        else if (await ConfigureResourceAsync(bicepResource, cancellationToken).ConfigureAwait(false))
        {
            resource.AzureResource.ProvisioningTaskCompletionSource?.TrySetResult();
    ...
    // Methods copied from BicepProvisioner

    private static bool ShouldProvision(AzureBicepResource resource)
        => !resource.IsContainer();

    private async Task<bool> ConfigureResourceAsync(AzureBicepResource resource, CancellationToken cancellationToken)
    {
    ...
    private async Task GetOrCreateResourceAsync(AzureBicepResource resource, ProvisioningContext context, CancellationToken cancellationToken)
    {
        ...
        var armTemplateContents = await bicepCliInvoker.CompileTemplateAsync(path, cancellationToken).ConfigureAwait(false);
    ...
    // Static utility methods from BicepProvisioner
...
internal sealed class MissingConfigurationException(string message) : Exception(message)
{
}