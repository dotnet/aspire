...
export function spawnCliProcess(aspireTerminalProvider: AspireTerminalProvider, dcpId: string | null, command: string, args?: string[], options?: SpawnProcessOptions): ChildProcessWithoutNullStreams {
    const envVars = mergeEnvs(process.env, options?.env);
    const additionalEnv = options?.excludeExtensionEnvironment ? { } : aspireTerminalProvider.createEnvironment(dcpId);
