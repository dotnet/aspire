...
export class AspireEditorCommandProvider implements vscode.Disposable {
    ...
    private _vscodeExtensionContext: vscode.ExtensionContext;

    constructor(vscodeExtensionContext: vscode.ExtensionContext) {
        this._vscodeExtensionContext = vscodeExtensionContext;
        this._disposable = vscode.workspace.onDidOpenTextDocument(async (document) => {
            await this.processDocument(vscodeExtensionContext, document);
        });
    ...
    async tryExecuteRunAppHost(noDebug: boolean, uri?: vscode.Uri): Promise<void> {
        ...
        try {
            ...
            let appHostProject = this._vscodeExtensionContext.workspaceState.get<string | null>('aspire.appHostProjectPath');

            if (appHostProject === undefined) {
                appHostProject = await this.getFileAppHostProject(uri);
                if (appHostProject) {
                    await this._vscodeExtensionContext.workspaceState.update('aspire.appHostProjectPath', appHostProject);
                    await vscode.commands.executeCommand('setContext', 'aspire.appHostProjectPath', appHostProject);
                }
            }
