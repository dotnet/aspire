...
export async function activate(context: vscode.ExtensionContext) {
    ...
    const rpcServer = await AspireRpcServer.create(
        (rpcServerConnectionInfo: RpcServerConnectionInfo, connection: MessageConnection, token: string, debugSessionId: string | null) => {
            ...
            return new RpcClient(terminalProvider, connection, debugSessionId, interactionService);
        }
    );
    ...
    context.subscriptions.push(cliAddCommandRegistration, cliNewCommandRegistration, cliConfigCommandRegistration, cliDeployCommandRegistration, cliPublishCommandRegistration, configureLaunchJsonCommandRegistration, runAppHostCommandRegistration, debugAppHostCommandRegistration);
