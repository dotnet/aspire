...
- For the 2nd and 3rd attempts (`run_attempt > 1`), a stricter cap applies: rerun is suppressed unless the matched job count is strictly less than the configured cap (i.e., fewer than the default 5 jobs). Aggregator jobs such as `Final Results` and `Tests / Final Test Results` are excluded from this count.
