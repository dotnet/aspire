...
To get the process of adding/modifying API going, file a new issue using [the API Proposal issue template](https://github.com/dotnet/aspire/issues/new?assignees=&labels=api-suggestion&template=api-proposal.md&title=). Below is more information about the process.
...
The API Review meeting is open to all members of the .NET Aspire team. The meeting invite as well as communication about API reviews is shared on the internal `.NET Aspire API Reviews` distribution list. Every API review meeting should include the area owners of the API change proposals as mandatory attendees. To list of all pending API review proposals can be found at: <https://aka.ms/dotnet/aspire/apireviews>