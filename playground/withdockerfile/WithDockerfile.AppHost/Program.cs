...
var goVersion = builder.AddParameter("goversion", "1.23");
...
builder.AddContainer("dynamic-sync", "dynamic-sync-image")
    .WithDockerfileFactory("qots", context =>
    {
        ...
        return $"""
            FROM mcr.microsoft.com/oss/go/microsoft/golang:1.23-alpine AS builder
            WORKDIR /app
            COPY . .
...
builder.AddContainer("dynamic-async", "dynamic-async-image")
    .WithDockerfileFactory("qots", async context =>
    {
        ...
        var baseImage = Environment.GetEnvironmentVariable("BASE_IMAGE") ?? "mcr.microsoft.com/oss/go/microsoft/golang:1.23-alpine";
        return $"""
