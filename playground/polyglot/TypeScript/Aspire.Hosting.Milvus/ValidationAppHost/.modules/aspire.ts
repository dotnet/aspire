...
/** Handle to MilvusDatabaseResource */
type MilvusDatabaseResourceHandle = Handle<'Aspire.Hosting.Milvus/Aspire.Hosting.ApplicationModel.MilvusDatabaseResource'>;

/** Handle to AttuResource */
type AttuResourceHandle = Handle<'Aspire.Hosting.Milvus/Aspire.Hosting.Milvus.AttuResource'>;

/** Handle to MilvusServerResource */
type MilvusServerResourceHandle = Handle<'Aspire.Hosting.Milvus/Aspire.Hosting.Milvus.MilvusServerResource'>;
...
export interface AddMilvusOptions {
...
export interface WithAttuOptions {
...
export class DistributedApplicationBuilder {
    ...
    /** @internal */
    async _addMilvusInternal(name: string, apiKey?: ParameterResource, grpcPort?: number): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name };
        if (apiKey !== undefined) rpcArgs.apiKey = apiKey;
        if (grpcPort !== undefined) rpcArgs.grpcPort = grpcPort;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting.Milvus/addMilvus',
            rpcArgs
        );
        return new MilvusServerResource(result, this._client);
    }

    addMilvus(name: string, options?: AddMilvusOptions): MilvusServerResourcePromise {
        const apiKey = options?.apiKey;
        const grpcPort = options?.grpcPort;
        return new MilvusServerResourcePromise(this._addMilvusInternal(name, apiKey, grpcPort));
    }
...
export class DistributedApplicationBuilderPromise implements PromiseLike<DistributedApplicationBuilder> {
    ...
    /** Adds a Milvus server resource to the distributed application model. */
    addMilvus(name: string, options?: AddMilvusOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.addMilvus(name, options)));
    }
...
export class AttuResource extends ResourceBuilderBase<AttuResourceHandle> {
    constructor(handle: AttuResourceHandle, client: AspireClientRpc) {
        super(handle, client);
    }

    /** @internal */
    private async _withContainerRegistryInternal(registry: ResourceBuilderBase): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
            'Aspire.Hosting/withContainerRegistry',
            rpcArgs
        );
        return new AttuResource(result, this._client);
    }

    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._withContainerRegistryInternal(registry));
    }

    /** @internal */
    private async _withBindMountInternal(source: string, target: string, isReadOnly?: boolean): Promise<AttuResource> {
    ...
    /** Adds a bind mount */
    withBindMount(source: string, target: string, options?: WithBindMountOptions): AttuResourcePromise {
        const isReadOnly = options?.isReadOnly;
        return new AttuResourcePromise(this._withBindMountInternal(source, target, isReadOnly));
    }

    /** @internal */
    private async _withEntrypointInternal(entrypoint: string): Promise<AttuResource> {
    ...
    withEntrypoint(entrypoint: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withEntrypointInternal(entrypoint));
    }

    /** @internal */
    private async _withImageTagInternal(tag: string): Promise<AttuResource> {
    ...
    withImageTag(tag: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withImageTagInternal(tag));
    }

    /** @internal */
    private async _withImageRegistryInternal(registry: string): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withImageRegistry(registry: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withImageRegistryInternal(registry));
    }

    /** @internal */
    private async _withImageInternal(image: string, tag?: string): Promise<AttuResource> {
    ...
    withImage(image: string, options?: WithImageOptions): AttuResourcePromise {
        const tag = options?.tag;
        return new AttuResourcePromise(this._withImageInternal(image, tag));
    }

    /** @internal */
    private async _withImageSHA256Internal(sha256: string): Promise<AttuResource> {
    ...
    withImageSHA256(sha256: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withImageSHA256Internal(sha256));
    }

    /** @internal */
    private async _withContainerRuntimeArgsInternal(args: string[]): Promise<AttuResource> {
    ...
    withContainerRuntimeArgs(args: string[]): AttuResourcePromise {
        return new AttuResourcePromise(this._withContainerRuntimeArgsInternal(args));
    }

    /** @internal */
    private async _withLifetimeInternal(lifetime: ContainerLifetime): Promise<AttuResource> {
    ...
    withLifetime(lifetime: ContainerLifetime): AttuResourcePromise {
        return new AttuResourcePromise(this._withLifetimeInternal(lifetime));
    }

    /** @internal */
    private async _withImagePullPolicyInternal(pullPolicy: ImagePullPolicy): Promise<AttuResource> {
    ...
    withImagePullPolicy(pullPolicy: ImagePullPolicy): AttuResourcePromise {
        return new AttuResourcePromise(this._withImagePullPolicyInternal(pullPolicy));
    }

    /** @internal */
    private async _publishAsContainerInternal(): Promise<AttuResource> {
    ...
    publishAsContainer(): AttuResourcePromise {
        return new AttuResourcePromise(this._publishAsContainerInternal());
    }

    /** @internal */
    private async _withDockerfileInternal(contextPath: string, dockerfilePath?: string, stage?: string): Promise<AttuResource> {
    ...
    withDockerfile(contextPath: string, options?: WithDockerfileOptions): AttuResourcePromise {
        const dockerfilePath = options?.dockerfilePath;
        const stage = options?.stage;
        return new AttuResourcePromise(this._withDockerfileInternal(contextPath, dockerfilePath, stage));
    }

    /** @internal */
    private async _withContainerNameInternal(name: string): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withContainerName(name: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withContainerNameInternal(name));
    }

    /** @internal */
    private async _withBuildArgInternal(name: string, value: ParameterResource): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withBuildArg(name: string, value: ParameterResource): AttuResourcePromise {
        return new AttuResourcePromise(this._withBuildArgInternal(name, value));
    }

    /** @internal */
    private async _withBuildSecretInternal(name: string, value: ParameterResource): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withBuildSecret(name: string, value: ParameterResource): AttuResourcePromise {
        return new AttuResourcePromise(this._withBuildSecretInternal(name, value));
    }

    /** @internal */
    private async _withEndpointProxySupportInternal(proxyEnabled: boolean): Promise<AttuResource> {
    ...
    withEndpointProxySupport(proxyEnabled: boolean): AttuResourcePromise {
        return new AttuResourcePromise(this._withEndpointProxySupportInternal(proxyEnabled));
    }

    /** @internal */
    private async _withDockerfileBaseImageInternal(buildImage?: string, runtimeImage?: string): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
    ...
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): AttuResourcePromise {
        const buildImage = options?.buildImage;
        const runtimeImage = options?.runtimeImage;
        return new AttuResourcePromise(this._withDockerfileBaseImageInternal(buildImage, runtimeImage));
    }

    /** @internal */
    private async _withContainerNetworkAliasInternal(alias: string): Promise<AttuResource> {
    ...
    withContainerNetworkAlias(alias: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withContainerNetworkAliasInternal(alias));
    }

    /** @internal */
    private async _withMcpServerInternal(path?: string, endpointName?: string): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
    ...
    withMcpServer(options?: WithMcpServerOptions): AttuResourcePromise {
        const path = options?.path;
        const endpointName = options?.endpointName;
        return new AttuResourcePromise(this._withMcpServerInternal(path, endpointName));
    }

    /** @internal */
    private async _withOtlpExporterInternal(): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withOtlpExporter(): AttuResourcePromise {
        return new AttuResourcePromise(this._withOtlpExporterInternal());
    }

    /** @internal */
    private async _withOtlpExporterProtocolInternal(protocol: OtlpProtocol): Promise<AttuResource> {
    ...
    withOtlpExporterProtocol(protocol: OtlpProtocol): AttuResourcePromise {
        return new AttuResourcePromise(this._withOtlpExporterProtocolInternal(protocol));
    }

    /** @internal */
    private async _publishAsConnectionStringInternal(): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    publishAsConnectionString(): AttuResourcePromise {
        return new AttuResourcePromise(this._publishAsConnectionStringInternal());
    }

    /** @internal */
    private async _withRequiredCommandInternal(command: string, helpLink?: string): Promise<AttuResource> {
    ...
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): AttuResourcePromise {
        const helpLink = options?.helpLink;
        return new AttuResourcePromise(this._withRequiredCommandInternal(command, helpLink));
    }

    /** @internal */
    private async _withEnvironmentInternal(name: string, value: string): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withEnvironment(name: string, value: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withEnvironmentInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentExpressionInternal(name: string, value: ReferenceExpression): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withEnvironmentExpression(name: string, value: ReferenceExpression): AttuResourcePromise {
        return new AttuResourcePromise(this._withEnvironmentExpressionInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentCallbackInternal(callback: (obj: EnvironmentCallbackContext) => Promise<void>): Promise<AttuResource> {
    ...
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._withEnvironmentCallbackInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentCallbackAsyncInternal(callback: (arg: EnvironmentCallbackContext) => Promise<void>): Promise<AttuResource> {
    ...
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._withEnvironmentCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentEndpointInternal(name: string, endpointReference: EndpointReference): Promise<AttuResource> {
    ...
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): AttuResourcePromise {
        return new AttuResourcePromise(this._withEnvironmentEndpointInternal(name, endpointReference));
    }

    /** @internal */
    private async _withEnvironmentParameterInternal(name: string, parameter: ParameterResource): Promise<AttuResource> {
    ...
    withEnvironmentParameter(name: string, parameter: ParameterResource): AttuResourcePromise {
        return new AttuResourcePromise(this._withEnvironmentParameterInternal(name, parameter));
    }

    /** @internal */
    private async _withEnvironmentConnectionStringInternal(envVarName: string, resource: ResourceBuilderBase): Promise<AttuResource> {
    ...
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._withEnvironmentConnectionStringInternal(envVarName, resource));
    }

    /** @internal */
    private async _withArgsInternal(args: string[]): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, args };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    /** Adds arguments */
    withArgs(args: string[]): AttuResourcePromise {
        return new AttuResourcePromise(this._withArgsInternal(args));
    }

    /** @internal */
    private async _withArgsCallbackInternal(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): Promise<AttuResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
    ...
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._withArgsCallbackInternal(callback));
    }

    /** @internal */
    private async _withArgsCallbackAsyncInternal(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): Promise<AttuResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
    ...
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._withArgsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withReferenceInternal(source: ResourceBuilderBase, connectionName?: string, optional?: boolean): Promise<AttuResource> {
    ...
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): AttuResourcePromise {
        const connectionName = options?.connectionName;
        const optional = options?.optional;
        return new AttuResourcePromise(this._withReferenceInternal(source, connectionName, optional));
    }

    /** @internal */
    private async _withServiceReferenceInternal(source: ResourceBuilderBase): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withServiceReference(source: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._withServiceReferenceInternal(source));
    }

    /** @internal */
    private async _withServiceReferenceNamedInternal(source: ResourceBuilderBase, name: string): Promise<AttuResource> {
    ...
    withServiceReferenceNamed(source: ResourceBuilderBase, name: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withServiceReferenceNamedInternal(source, name));
    }

    /** @internal */
    private async _withReferenceUriInternal(name: string, uri: string): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, uri };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withReferenceUri(name: string, uri: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withReferenceUriInternal(name, uri));
    }

    /** @internal */
    private async _withReferenceExternalServiceInternal(externalService: ExternalServiceResource): Promise<AttuResource> {
    ...
    withReferenceExternalService(externalService: ExternalServiceResource): AttuResourcePromise {
        return new AttuResourcePromise(this._withReferenceExternalServiceInternal(externalService));
    }

    /** @internal */
    private async _withReferenceEndpointInternal(endpointReference: EndpointReference): Promise<AttuResource> {
    ...
    withReferenceEndpoint(endpointReference: EndpointReference): AttuResourcePromise {
        return new AttuResourcePromise(this._withReferenceEndpointInternal(endpointReference));
    }

    /** @internal */
    private async _withEndpointInternal(port?: number, targetPort?: number, scheme?: string, name?: string, env?: string, isProxied?: boolean, isExternal?: boolean, protocol?: ProtocolType): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
    ...
    withEndpoint(options?: WithEndpointOptions): AttuResourcePromise {
        ...
        return new AttuResourcePromise(this._withEndpointInternal(port, targetPort, scheme, name, env, isProxied, isExternal, protocol));
    }

    /** @internal */
    private async _withHttpEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    withHttpEndpoint(options?: WithHttpEndpointOptions): AttuResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new AttuResourcePromise(this._withHttpEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withHttpsEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): AttuResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new AttuResourcePromise(this._withHttpsEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withExternalHttpEndpointsInternal(): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withExternalHttpEndpoints(): AttuResourcePromise {
        return new AttuResourcePromise(this._withExternalHttpEndpointsInternal());
    }
    ...
    /** @internal */
    private async _asHttp2ServiceInternal(): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    asHttp2Service(): AttuResourcePromise {
        return new AttuResourcePromise(this._asHttp2ServiceInternal());
    }

    /** @internal */
    private async _withUrlsCallbackInternal(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): Promise<AttuResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
    ...
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._withUrlsCallbackInternal(callback));
    }

    /** @internal */
    private async _withUrlsCallbackAsyncInternal(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): Promise<AttuResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
    ...
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._withUrlsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withUrlInternal(url: string, displayText?: string): Promise<AttuResource> {
    ...
    withUrl(url: string, options?: WithUrlOptions): AttuResourcePromise {
        const displayText = options?.displayText;
        return new AttuResourcePromise(this._withUrlInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlExpressionInternal(url: ReferenceExpression, displayText?: string): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): AttuResourcePromise {
        const displayText = options?.displayText;
        return new AttuResourcePromise(this._withUrlExpressionInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlForEndpointInternal(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): Promise<AttuResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
    ...
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._withUrlForEndpointInternal(endpointName, callback));
    }

    /** @internal */
    private async _withUrlForEndpointFactoryInternal(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): Promise<AttuResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
    ...
    withUrlForEndpointFactory(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): AttuResourcePromise {
        return new AttuResourcePromise(this._withUrlForEndpointFactoryInternal(endpointName, callback));
    }

    /** @internal */
    private async _excludeFromManifestInternal(): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    excludeFromManifest(): AttuResourcePromise {
        return new AttuResourcePromise(this._excludeFromManifestInternal());
    }

    /** @internal */
    private async _waitForInternal(dependency: ResourceBuilderBase): Promise<AttuResource> {
    ...
    waitFor(dependency: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._waitForInternal(dependency));
    }

    /** @internal */
    private async _waitForWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<AttuResource> {
    ...
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): AttuResourcePromise {
        return new AttuResourcePromise(this._waitForWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _waitForStartInternal(dependency: ResourceBuilderBase): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    waitForStart(dependency: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._waitForStartInternal(dependency));
    }

    /** @internal */
    private async _waitForStartWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency, waitBehavior };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): AttuResourcePromise {
        return new AttuResourcePromise(this._waitForStartWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _withExplicitStartInternal(): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withExplicitStart(): AttuResourcePromise {
        return new AttuResourcePromise(this._withExplicitStartInternal());
    }

    /** @internal */
    private async _waitForCompletionInternal(dependency: ResourceBuilderBase, exitCode?: number): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
    ...
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): AttuResourcePromise {
        const exitCode = options?.exitCode;
        return new AttuResourcePromise(this._waitForCompletionInternal(dependency, exitCode));
    }

    /** @internal */
    private async _withHealthCheckInternal(key: string): Promise<AttuResource> {
    ...
    withHealthCheck(key: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withHealthCheckInternal(key));
    }

    /** @internal */
    private async _withHttpHealthCheckInternal(path?: string, statusCode?: number, endpointName?: string): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (path !== undefined) rpcArgs.path = path;
    ...
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): AttuResourcePromise {
        const path = options?.path;
        const statusCode = options?.statusCode;
        const endpointName = options?.endpointName;
        return new AttuResourcePromise(this._withHttpHealthCheckInternal(path, statusCode, endpointName));
    }

    /** @internal */
    private async _withCommandInternal(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, commandOptions?: CommandOptions): Promise<AttuResource> {
    ...
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): AttuResourcePromise {
        const commandOptions = options?.commandOptions;
        return new AttuResourcePromise(this._withCommandInternal(name, displayName, executeCommand, commandOptions));
    }

    /** @internal */
    private async _withDeveloperCertificateTrustInternal(trust: boolean): Promise<AttuResource> {
    ...
    withDeveloperCertificateTrust(trust: boolean): AttuResourcePromise {
        return new AttuResourcePromise(this._withDeveloperCertificateTrustInternal(trust));
    }

    /** @internal */
    private async _withCertificateTrustScopeInternal(scope: CertificateTrustScope): Promise<AttuResource> {
    ...
    withCertificateTrustScope(scope: CertificateTrustScope): AttuResourcePromise {
        return new AttuResourcePromise(this._withCertificateTrustScopeInternal(scope));
    }

    /** @internal */
    private async _withHttpsDeveloperCertificateInternal(password?: ParameterResource): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
    ...
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): AttuResourcePromise {
        const password = options?.password;
        return new AttuResourcePromise(this._withHttpsDeveloperCertificateInternal(password));
    }

    /** @internal */
    private async _withoutHttpsCertificateInternal(): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    withoutHttpsCertificate(): AttuResourcePromise {
        return new AttuResourcePromise(this._withoutHttpsCertificateInternal());
    }

    /** @internal */
    private async _withParentRelationshipInternal(parent: ResourceBuilderBase): Promise<AttuResource> {
    ...
    withParentRelationship(parent: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._withParentRelationshipInternal(parent));
    }

    /** @internal */
    private async _withChildRelationshipInternal(child: ResourceBuilderBase): Promise<AttuResource> {
    ...
    withChildRelationship(child: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._withChildRelationshipInternal(child));
    }

    /** @internal */
    private async _withIconNameInternal(iconName: string, iconVariant?: IconVariant): Promise<AttuResource> {
    ...
    withIconName(iconName: string, options?: WithIconNameOptions): AttuResourcePromise {
        const iconVariant = options?.iconVariant;
        return new AttuResourcePromise(this._withIconNameInternal(iconName, iconVariant));
    }

    /** @internal */
    private async _withHttpProbeInternal(probeType: ProbeType, path?: string, initialDelaySeconds?: number, periodSeconds?: number, timeoutSeconds?: number, failureThreshold?: number, successThreshold?: number, endpointName?: string): Promise<AttuResource> {
    ...
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): AttuResourcePromise {
        const path = options?.path;
        ...
        const endpointName = options?.endpointName;
        return new AttuResourcePromise(this._withHttpProbeInternal(probeType, path, initialDelaySeconds, periodSeconds, timeoutSeconds, failureThreshold, successThreshold, endpointName));
    }

    /** @internal */
    private async _excludeFromMcpInternal(): Promise<AttuResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AttuResourceHandle>(
    ...
    excludeFromMcp(): AttuResourcePromise {
        return new AttuResourcePromise(this._excludeFromMcpInternal());
    }

    /** @internal */
    private async _withRemoteImageNameInternal(remoteImageName: string): Promise<AttuResource> {
    ...
    withRemoteImageName(remoteImageName: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withRemoteImageNameInternal(remoteImageName));
    }

    /** @internal */
    private async _withRemoteImageTagInternal(remoteImageTag: string): Promise<AttuResource> {
    ...
    withRemoteImageTag(remoteImageTag: string): AttuResourcePromise {
        return new AttuResourcePromise(this._withRemoteImageTagInternal(remoteImageTag));
    }

    /** @internal */
    private async _withPipelineStepFactoryInternal(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, dependsOn?: string[], requiredBy?: string[], tags?: string[], description?: string): Promise<AttuResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
    ...
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): AttuResourcePromise {
        const dependsOn = options?.dependsOn;
        const requiredBy = options?.requiredBy;
        const tags = options?.tags;
        const description = options?.description;
        return new AttuResourcePromise(this._withPipelineStepFactoryInternal(stepName, callback, dependsOn, requiredBy, tags, description));
    }

    /** @internal */
    private async _withPipelineConfigurationAsyncInternal(callback: (arg: PipelineConfigurationContext) => Promise<void>): Promise<AttuResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
    ...
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._withPipelineConfigurationAsyncInternal(callback));
    }

    /** @internal */
    private async _withPipelineConfigurationInternal(callback: (obj: PipelineConfigurationContext) => Promise<void>): Promise<AttuResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
    ...
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._withPipelineConfigurationInternal(callback));
    }

    /** @internal */
    private async _withVolumeInternal(target: string, name?: string, isReadOnly?: boolean): Promise<AttuResource> {
    ...
    /** Adds a volume */
    withVolume(target: string, options?: WithVolumeOptions): AttuResourcePromise {
        const name = options?.name;
        const isReadOnly = options?.isReadOnly;
        return new AttuResourcePromise(this._withVolumeInternal(target, name, isReadOnly));
    }
...
/**
 * Thenable wrapper for AttuResource that enables fluent chaining.
 * @example
 * await builder.addSomething().withX().withY();
 */
export class AttuResourcePromise implements PromiseLike<AttuResource> {
    constructor(private _promise: Promise<AttuResource>) {}

    then<TResult1 = AttuResource, TResult2 = never>(
        onfulfilled?: ((value: AttuResource) => TResult1 | PromiseLike<TResult1>) | null,
        onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null
    ): PromiseLike<TResult1 | TResult2> {
        return this._promise.then(onfulfilled, onrejected);
    }
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withContainerRegistry(registry)));
    }

    /** Adds a bind mount */
    withBindMount(source: string, target: string, options?: WithBindMountOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withBindMount(source, target, options)));
    }

    /** Sets the container entrypoint */
    withEntrypoint(entrypoint: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withEntrypoint(entrypoint)));
    }

    /** Sets the container image tag */
    withImageTag(tag: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withImageTag(tag)));
    }

    /** Sets the container image registry */
    withImageRegistry(registry: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withImageRegistry(registry)));
    }

    /** Sets the container image */
    withImage(image: string, options?: WithImageOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withImage(image, options)));
    }

    /** Sets the image SHA256 digest */
    withImageSHA256(sha256: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withImageSHA256(sha256)));
    }

    /** Adds runtime arguments for the container */
    withContainerRuntimeArgs(args: string[]): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withContainerRuntimeArgs(args)));
    }

    /** Sets the lifetime behavior of the container resource */
    withLifetime(lifetime: ContainerLifetime): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withLifetime(lifetime)));
    }

    /** Sets the container image pull policy */
    withImagePullPolicy(pullPolicy: ImagePullPolicy): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withImagePullPolicy(pullPolicy)));
    }

    /** Configures the resource to be published as a container */
    publishAsContainer(): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.publishAsContainer()));
    }

    /** Configures the resource to use a Dockerfile */
    withDockerfile(contextPath: string, options?: WithDockerfileOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withDockerfile(contextPath, options)));
    }

    /** Sets the container name */
    withContainerName(name: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withContainerName(name)));
    }

    /** Adds a build argument from a parameter resource */
    withBuildArg(name: string, value: ParameterResource): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withBuildArg(name, value)));
    }

    /** Adds a build secret from a parameter resource */
    withBuildSecret(name: string, value: ParameterResource): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withBuildSecret(name, value)));
    }

    /** Configures endpoint proxy support */
    withEndpointProxySupport(proxyEnabled: boolean): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withEndpointProxySupport(proxyEnabled)));
    }

    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withDockerfileBaseImage(options)));
    }

    /** Adds a network alias for the container */
    withContainerNetworkAlias(alias: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withContainerNetworkAlias(alias)));
    }

    /** Configures an MCP server endpoint on the resource */
    withMcpServer(options?: WithMcpServerOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withMcpServer(options)));
    }

    /** Configures OTLP telemetry export */
    withOtlpExporter(): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withOtlpExporter()));
    }

    /** Configures OTLP telemetry export with specific protocol */
    withOtlpExporterProtocol(protocol: OtlpProtocol): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withOtlpExporterProtocol(protocol)));
    }

    /** Publishes the resource as a connection string */
    publishAsConnectionString(): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.publishAsConnectionString()));
    }

    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withRequiredCommand(command, options)));
    }

    /** Sets an environment variable */
    withEnvironment(name: string, value: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withEnvironment(name, value)));
    }

    /** Adds an environment variable with a reference expression */
    withEnvironmentExpression(name: string, value: ReferenceExpression): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withEnvironmentExpression(name, value)));
    }

    /** Sets environment variables via callback */
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withEnvironmentCallback(callback)));
    }

    /** Sets environment variables via async callback */
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withEnvironmentCallbackAsync(callback)));
    }

    /** Sets an environment variable from an endpoint reference */
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withEnvironmentEndpoint(name, endpointReference)));
    }

    /** Sets an environment variable from a parameter resource */
    withEnvironmentParameter(name: string, parameter: ParameterResource): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withEnvironmentParameter(name, parameter)));
    }

    /** Sets an environment variable from a connection string resource */
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withEnvironmentConnectionString(envVarName, resource)));
    }

    /** Adds arguments */
    withArgs(args: string[]): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withArgs(args)));
    }

    /** Sets command-line arguments via callback */
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withArgsCallback(callback)));
    }

    /** Sets command-line arguments via async callback */
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withArgsCallbackAsync(callback)));
    }

    /** Adds a reference to another resource */
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withReference(source, options)));
    }

    /** Adds a service discovery reference to another resource */
    withServiceReference(source: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withServiceReference(source)));
    }

    /** Adds a named service discovery reference */
    withServiceReferenceNamed(source: ResourceBuilderBase, name: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withServiceReferenceNamed(source, name)));
    }

    /** Adds a reference to a URI */
    withReferenceUri(name: string, uri: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withReferenceUri(name, uri)));
    }

    /** Adds a reference to an external service */
    withReferenceExternalService(externalService: ExternalServiceResource): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withReferenceExternalService(externalService)));
    }

    /** Adds a reference to an endpoint */
    withReferenceEndpoint(endpointReference: EndpointReference): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withReferenceEndpoint(endpointReference)));
    }

    /** Adds a network endpoint */
    withEndpoint(options?: WithEndpointOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withEndpoint(options)));
    }

    /** Adds an HTTP endpoint */
    withHttpEndpoint(options?: WithHttpEndpointOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withHttpEndpoint(options)));
    }

    /** Adds an HTTPS endpoint */
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withHttpsEndpoint(options)));
    }

    /** Makes HTTP endpoints externally accessible */
    withExternalHttpEndpoints(): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withExternalHttpEndpoints()));
    }
    ...
    /** Configures resource for HTTP/2 */
    asHttp2Service(): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.asHttp2Service()));
    }

    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withUrlsCallback(callback)));
    }

    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withUrlsCallbackAsync(callback)));
    }

    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withUrl(url, options)));
    }

    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withUrlExpression(url, options)));
    }

    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withUrlForEndpoint(endpointName, callback)));
    }

    /** Adds a URL for a specific endpoint via factory callback */
    withUrlForEndpointFactory(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withUrlForEndpointFactory(endpointName, callback)));
    }

    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.excludeFromManifest()));
    }

    /** Waits for another resource to be ready */
    waitFor(dependency: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.waitFor(dependency)));
    }

    /** Waits for another resource with specific behavior */
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.waitForWithBehavior(dependency, waitBehavior)));
    }

    /** Waits for another resource to start */
    waitForStart(dependency: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.waitForStart(dependency)));
    }

    /** Waits for another resource to start with specific behavior */
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.waitForStartWithBehavior(dependency, waitBehavior)));
    }

    /** Prevents resource from starting automatically */
    withExplicitStart(): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withExplicitStart()));
    }

    /** Waits for resource completion */
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.waitForCompletion(dependency, options)));
    }

    /** Adds a health check by key */
    withHealthCheck(key: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withHealthCheck(key)));
    }

    /** Adds an HTTP health check */
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withHttpHealthCheck(options)));
    }

    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withCommand(name, displayName, executeCommand, options)));
    }

    /** Configures developer certificate trust */
    withDeveloperCertificateTrust(trust: boolean): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withDeveloperCertificateTrust(trust)));
    }

    /** Sets the certificate trust scope */
    withCertificateTrustScope(scope: CertificateTrustScope): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withCertificateTrustScope(scope)));
    }

    /** Configures HTTPS with a developer certificate */
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withHttpsDeveloperCertificate(options)));
    }

    /** Removes HTTPS certificate configuration */
    withoutHttpsCertificate(): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withoutHttpsCertificate()));
    }

    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withParentRelationship(parent)));
    }

    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withChildRelationship(child)));
    }

    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withIconName(iconName, options)));
    }

    /** Adds an HTTP health probe to the resource */
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withHttpProbe(probeType, options)));
    }

    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.excludeFromMcp()));
    }

    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withRemoteImageName(remoteImageName)));
    }

    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withRemoteImageTag(remoteImageTag)));
    }

    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withPipelineStepFactory(stepName, callback, options)));
    }

    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withPipelineConfigurationAsync(callback)));
    }

    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withPipelineConfiguration(callback)));
    }

    /** Adds a volume */
    withVolume(target: string, options?: WithVolumeOptions): AttuResourcePromise {
        return new AttuResourcePromise(this._promise.then(obj => obj.withVolume(target, options)));
    }
...
export class MilvusDatabaseResource extends ResourceBuilderBase<MilvusDatabaseResourceHandle> {
    constructor(handle: MilvusDatabaseResourceHandle, client: AspireClientRpc) {
        super(handle, client);
    }
    ...
    /** @internal */
    private async _withContainerRegistryInternal(registry: ResourceBuilderBase): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withContainerRegistry',
            rpcArgs
        );
        return new MilvusDatabaseResource(result, this._client);
    }

    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withContainerRegistryInternal(registry));
    }

    /** @internal */
    private async _withDockerfileBaseImageInternal(buildImage?: string, runtimeImage?: string): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (buildImage !== undefined) rpcArgs.buildImage = buildImage;
        if (runtimeImage !== undefined) rpcArgs.runtimeImage = runtimeImage;
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
    ...
    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): MilvusDatabaseResourcePromise {
        const buildImage = options?.buildImage;
        const runtimeImage = options?.runtimeImage;
        return new MilvusDatabaseResourcePromise(this._withDockerfileBaseImageInternal(buildImage, runtimeImage));
    }

    /** @internal */
    private async _withRequiredCommandInternal(command: string, helpLink?: string): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, command };
        if (helpLink !== undefined) rpcArgs.helpLink = helpLink;
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withRequiredCommand',
    ...
    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): MilvusDatabaseResourcePromise {
        const helpLink = options?.helpLink;
        return new MilvusDatabaseResourcePromise(this._withRequiredCommandInternal(command, helpLink));
    }

    /** @internal */
    private async _withConnectionPropertyInternal(name: string, value: ReferenceExpression): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withConnectionProperty',
            rpcArgs
    ...
    /** Adds a connection property with a reference expression */
    withConnectionProperty(name: string, value: ReferenceExpression): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withConnectionPropertyInternal(name, value));
    }

    /** @internal */
    private async _withConnectionPropertyValueInternal(name: string, value: string): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withConnectionPropertyValue',
            rpcArgs
    ...
    /** Adds a connection property with a string value */
    withConnectionPropertyValue(name: string, value: string): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withConnectionPropertyValueInternal(name, value));
    }
    ...
    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withUrlsCallbackInternal(callback));
    }

    /** @internal */
    private async _withUrlsCallbackAsyncInternal(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): Promise<MilvusDatabaseResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ResourceUrlsCallbackContextHandle;
            const arg = new ResourceUrlsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withUrlsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withUrlInternal(url: string, displayText?: string): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withUrl',
    ...
    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): MilvusDatabaseResourcePromise {
        const displayText = options?.displayText;
        return new MilvusDatabaseResourcePromise(this._withUrlInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlExpressionInternal(url: ReferenceExpression, displayText?: string): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withUrlExpression',
    ...
    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): MilvusDatabaseResourcePromise {
        const displayText = options?.displayText;
        return new MilvusDatabaseResourcePromise(this._withUrlExpressionInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlForEndpointInternal(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): Promise<MilvusDatabaseResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const obj = wrapIfHandle(objData) as ResourceUrlAnnotation;
            await callback(obj);
        });
    ...
    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withUrlForEndpointInternal(endpointName, callback));
    }

    /** @internal */
    private async _excludeFromManifestInternal(): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/excludeFromManifest',
            rpcArgs
    ...
    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._excludeFromManifestInternal());
    }

    /** @internal */
    private async _withExplicitStartInternal(): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withExplicitStart',
            rpcArgs
    ...
    /** Prevents resource from starting automatically */
    withExplicitStart(): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withExplicitStartInternal());
    }

    /** @internal */
    private async _withHealthCheckInternal(key: string): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, key };
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withHealthCheck',
            rpcArgs
    ...
    /** Adds a health check by key */
    withHealthCheck(key: string): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withHealthCheckInternal(key));
    }

    /** @internal */
    private async _withCommandInternal(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, commandOptions?: CommandOptions): Promise<MilvusDatabaseResource> {
        const executeCommandId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ExecuteCommandContextHandle;
            const arg = new ExecuteCommandContext(argHandle, this._client);
            return await executeCommand(arg);
    ...
    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): MilvusDatabaseResourcePromise {
        const commandOptions = options?.commandOptions;
        return new MilvusDatabaseResourcePromise(this._withCommandInternal(name, displayName, executeCommand, commandOptions));
    }

    /** @internal */
    private async _withParentRelationshipInternal(parent: ResourceBuilderBase): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, parent };
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withParentRelationship',
            rpcArgs
    ...
    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withParentRelationshipInternal(parent));
    }

    /** @internal */
    private async _withChildRelationshipInternal(child: ResourceBuilderBase): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, child };
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withChildRelationship',
            rpcArgs
    ...
    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withChildRelationshipInternal(child));
    }

    /** @internal */
    private async _withIconNameInternal(iconName: string, iconVariant?: IconVariant): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, iconName };
        if (iconVariant !== undefined) rpcArgs.iconVariant = iconVariant;
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withIconName',
    ...
    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): MilvusDatabaseResourcePromise {
        const iconVariant = options?.iconVariant;
        return new MilvusDatabaseResourcePromise(this._withIconNameInternal(iconName, iconVariant));
    }

    /** @internal */
    private async _excludeFromMcpInternal(): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/excludeFromMcp',
            rpcArgs
    ...
    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._excludeFromMcpInternal());
    }

    /** @internal */
    private async _withRemoteImageNameInternal(remoteImageName: string): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageName };
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withRemoteImageName',
            rpcArgs
    ...
    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withRemoteImageNameInternal(remoteImageName));
    }

    /** @internal */
    private async _withRemoteImageTagInternal(remoteImageTag: string): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageTag };
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting/withRemoteImageTag',
            rpcArgs
    ...
    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withRemoteImageTagInternal(remoteImageTag));
    }

    /** @internal */
    private async _withPipelineStepFactoryInternal(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, dependsOn?: string[], requiredBy?: string[], tags?: string[], description?: string): Promise<MilvusDatabaseResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineStepContextHandle;
            const arg = new PipelineStepContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): MilvusDatabaseResourcePromise {
        const dependsOn = options?.dependsOn;
        const requiredBy = options?.requiredBy;
        const tags = options?.tags;
        const description = options?.description;
        return new MilvusDatabaseResourcePromise(this._withPipelineStepFactoryInternal(stepName, callback, dependsOn, requiredBy, tags, description));
    }

    /** @internal */
    private async _withPipelineConfigurationAsyncInternal(callback: (arg: PipelineConfigurationContext) => Promise<void>): Promise<MilvusDatabaseResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineConfigurationContextHandle;
            const arg = new PipelineConfigurationContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withPipelineConfigurationAsyncInternal(callback));
    }

    /** @internal */
    private async _withPipelineConfigurationInternal(callback: (obj: PipelineConfigurationContext) => Promise<void>): Promise<MilvusDatabaseResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as PipelineConfigurationContextHandle;
            const obj = new PipelineConfigurationContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._withPipelineConfigurationInternal(callback));
    }
...
/**
 * Thenable wrapper for MilvusDatabaseResource that enables fluent chaining.
 * @example
 * await builder.addSomething().withX().withY();
 */
export class MilvusDatabaseResourcePromise implements PromiseLike<MilvusDatabaseResource> {
    constructor(private _promise: Promise<MilvusDatabaseResource>) {}

    then<TResult1 = MilvusDatabaseResource, TResult2 = never>(
        onfulfilled?: ((value: MilvusDatabaseResource) => TResult1 | PromiseLike<TResult1>) | null,
        onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null
    ): PromiseLike<TResult1 | TResult2> {
        return this._promise.then(onfulfilled, onrejected);
    }
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withContainerRegistry(registry)));
    }

    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withDockerfileBaseImage(options)));
    }

    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withRequiredCommand(command, options)));
    }

    /** Adds a connection property with a reference expression */
    withConnectionProperty(name: string, value: ReferenceExpression): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withConnectionProperty(name, value)));
    }

    /** Adds a connection property with a string value */
    withConnectionPropertyValue(name: string, value: string): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withConnectionPropertyValue(name, value)));
    }

    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withUrlsCallback(callback)));
    }

    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withUrlsCallbackAsync(callback)));
    }

    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withUrl(url, options)));
    }

    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withUrlExpression(url, options)));
    }

    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withUrlForEndpoint(endpointName, callback)));
    }

    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.excludeFromManifest()));
    }

    /** Prevents resource from starting automatically */
    withExplicitStart(): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withExplicitStart()));
    }

    /** Adds a health check by key */
    withHealthCheck(key: string): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withHealthCheck(key)));
    }

    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withCommand(name, displayName, executeCommand, options)));
    }

    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withParentRelationship(parent)));
    }

    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withChildRelationship(child)));
    }

    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withIconName(iconName, options)));
    }

    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.excludeFromMcp()));
    }

    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withRemoteImageName(remoteImageName)));
    }

    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withRemoteImageTag(remoteImageTag)));
    }

    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withPipelineStepFactory(stepName, callback, options)));
    }

    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withPipelineConfigurationAsync(callback)));
    }

    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.withPipelineConfiguration(callback)));
    }
...
export class MilvusServerResource extends ResourceBuilderBase<MilvusServerResourceHandle> {
    ...
    /** @internal */
    private async _withContainerRegistryInternal(registry: ResourceBuilderBase): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withContainerRegistry',
            rpcArgs
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withContainerRegistryInternal(registry));
    }
    ...
    /** Adds a bind mount */
    withBindMount(source: string, target: string, options?: WithBindMountOptions): MilvusServerResourcePromise {
        const isReadOnly = options?.isReadOnly;
        return new MilvusServerResourcePromise(this._withBindMountInternal(source, target, isReadOnly));
    }

    /** @internal */
    private async _withEntrypointInternal(entrypoint: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, entrypoint };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withEntrypoint',
            rpcArgs
    ...
    /** Sets the container entrypoint */
    withEntrypoint(entrypoint: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withEntrypointInternal(entrypoint));
    }

    /** @internal */
    private async _withImageTagInternal(tag: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, tag };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withImageTag',
            rpcArgs
    ...
    /** Sets the container image tag */
    withImageTag(tag: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withImageTagInternal(tag));
    }

    /** @internal */
    private async _withImageRegistryInternal(registry: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withImageRegistry',
            rpcArgs
    ...
    /** Sets the container image registry */
    withImageRegistry(registry: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withImageRegistryInternal(registry));
    }

    /** @internal */
    private async _withImageInternal(image: string, tag?: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, image };
        if (tag !== undefined) rpcArgs.tag = tag;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withImage',
    ...
    /** Sets the container image */
    withImage(image: string, options?: WithImageOptions): MilvusServerResourcePromise {
        const tag = options?.tag;
        return new MilvusServerResourcePromise(this._withImageInternal(image, tag));
    }

    /** @internal */
    private async _withImageSHA256Internal(sha256: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, sha256 };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withImageSHA256',
            rpcArgs
    ...
    /** Sets the image SHA256 digest */
    withImageSHA256(sha256: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withImageSHA256Internal(sha256));
    }

    /** @internal */
    private async _withContainerRuntimeArgsInternal(args: string[]): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, args };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withContainerRuntimeArgs',
            rpcArgs
    ...
    /** Adds runtime arguments for the container */
    withContainerRuntimeArgs(args: string[]): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withContainerRuntimeArgsInternal(args));
    }

    /** @internal */
    private async _withLifetimeInternal(lifetime: ContainerLifetime): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, lifetime };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withLifetime',
            rpcArgs
    ...
    /** Sets the lifetime behavior of the container resource */
    withLifetime(lifetime: ContainerLifetime): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withLifetimeInternal(lifetime));
    }

    /** @internal */
    private async _withImagePullPolicyInternal(pullPolicy: ImagePullPolicy): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, pullPolicy };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withImagePullPolicy',
            rpcArgs
    ...
    /** Sets the container image pull policy */
    withImagePullPolicy(pullPolicy: ImagePullPolicy): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withImagePullPolicyInternal(pullPolicy));
    }

    /** @internal */
    private async _publishAsContainerInternal(): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/publishAsContainer',
            rpcArgs
    ...
    /** Configures the resource to be published as a container */
    publishAsContainer(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._publishAsContainerInternal());
    }

    /** @internal */
    private async _withDockerfileInternal(contextPath: string, dockerfilePath?: string, stage?: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, contextPath };
        if (dockerfilePath !== undefined) rpcArgs.dockerfilePath = dockerfilePath;
        if (stage !== undefined) rpcArgs.stage = stage;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
    ...
    /** Configures the resource to use a Dockerfile */
    withDockerfile(contextPath: string, options?: WithDockerfileOptions): MilvusServerResourcePromise {
        const dockerfilePath = options?.dockerfilePath;
        const stage = options?.stage;
        return new MilvusServerResourcePromise(this._withDockerfileInternal(contextPath, dockerfilePath, stage));
    }

    /** @internal */
    private async _withContainerNameInternal(name: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withContainerName',
            rpcArgs
    ...
    /** Sets the container name */
    withContainerName(name: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withContainerNameInternal(name));
    }

    /** @internal */
    private async _withBuildArgInternal(name: string, value: ParameterResource): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withBuildArg',
            rpcArgs
    ...
    /** Adds a build argument from a parameter resource */
    withBuildArg(name: string, value: ParameterResource): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withBuildArgInternal(name, value));
    }

    /** @internal */
    private async _withBuildSecretInternal(name: string, value: ParameterResource): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withBuildSecret',
            rpcArgs
    ...
    /** Adds a build secret from a parameter resource */
    withBuildSecret(name: string, value: ParameterResource): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withBuildSecretInternal(name, value));
    }

    /** @internal */
    private async _withEndpointProxySupportInternal(proxyEnabled: boolean): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, proxyEnabled };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withEndpointProxySupport',
            rpcArgs
    ...
    /** Configures endpoint proxy support */
    withEndpointProxySupport(proxyEnabled: boolean): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withEndpointProxySupportInternal(proxyEnabled));
    }

    /** @internal */
    private async _withDockerfileBaseImageInternal(buildImage?: string, runtimeImage?: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (buildImage !== undefined) rpcArgs.buildImage = buildImage;
        if (runtimeImage !== undefined) rpcArgs.runtimeImage = runtimeImage;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
    ...
    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): MilvusServerResourcePromise {
        const buildImage = options?.buildImage;
        const runtimeImage = options?.runtimeImage;
        return new MilvusServerResourcePromise(this._withDockerfileBaseImageInternal(buildImage, runtimeImage));
    }

    /** @internal */
    private async _withContainerNetworkAliasInternal(alias: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, alias };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withContainerNetworkAlias',
            rpcArgs
    ...
    /** Adds a network alias for the container */
    withContainerNetworkAlias(alias: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withContainerNetworkAliasInternal(alias));
    }

    /** @internal */
    private async _withMcpServerInternal(path?: string, endpointName?: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (path !== undefined) rpcArgs.path = path;
        if (endpointName !== undefined) rpcArgs.endpointName = endpointName;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
    ...
    /** Configures an MCP server endpoint on the resource */
    withMcpServer(options?: WithMcpServerOptions): MilvusServerResourcePromise {
        const path = options?.path;
        const endpointName = options?.endpointName;
        return new MilvusServerResourcePromise(this._withMcpServerInternal(path, endpointName));
    }

    /** @internal */
    private async _withOtlpExporterInternal(): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withOtlpExporter',
            rpcArgs
    ...
    /** Configures OTLP telemetry export */
    withOtlpExporter(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withOtlpExporterInternal());
    }

    /** @internal */
    private async _withOtlpExporterProtocolInternal(protocol: OtlpProtocol): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, protocol };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withOtlpExporterProtocol',
            rpcArgs
    ...
    /** Configures OTLP telemetry export with specific protocol */
    withOtlpExporterProtocol(protocol: OtlpProtocol): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withOtlpExporterProtocolInternal(protocol));
    }

    /** @internal */
    private async _publishAsConnectionStringInternal(): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/publishAsConnectionString',
            rpcArgs
    ...
    /** Publishes the resource as a connection string */
    publishAsConnectionString(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._publishAsConnectionStringInternal());
    }

    /** @internal */
    private async _withRequiredCommandInternal(command: string, helpLink?: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, command };
        if (helpLink !== undefined) rpcArgs.helpLink = helpLink;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withRequiredCommand',
    ...
    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): MilvusServerResourcePromise {
        const helpLink = options?.helpLink;
        return new MilvusServerResourcePromise(this._withRequiredCommandInternal(command, helpLink));
    }

    /** @internal */
    private async _withEnvironmentInternal(name: string, value: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withEnvironment',
            rpcArgs
    ...
    /** Sets an environment variable */
    withEnvironment(name: string, value: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withEnvironmentInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentExpressionInternal(name: string, value: ReferenceExpression): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withEnvironmentExpression',
            rpcArgs
    ...
    /** Adds an environment variable with a reference expression */
    withEnvironmentExpression(name: string, value: ReferenceExpression): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withEnvironmentExpressionInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentCallbackInternal(callback: (obj: EnvironmentCallbackContext) => Promise<void>): Promise<MilvusServerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as EnvironmentCallbackContextHandle;
            const obj = new EnvironmentCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Sets environment variables via callback */
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withEnvironmentCallbackInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentCallbackAsyncInternal(callback: (arg: EnvironmentCallbackContext) => Promise<void>): Promise<MilvusServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as EnvironmentCallbackContextHandle;
            const arg = new EnvironmentCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Sets environment variables via async callback */
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withEnvironmentCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentEndpointInternal(name: string, endpointReference: EndpointReference): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, endpointReference };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withEnvironmentEndpoint',
            rpcArgs
    ...
    /** Sets an environment variable from an endpoint reference */
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withEnvironmentEndpointInternal(name, endpointReference));
    }

    /** @internal */
    private async _withEnvironmentParameterInternal(name: string, parameter: ParameterResource): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, parameter };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withEnvironmentParameter',
            rpcArgs
    ...
    /** Sets an environment variable from a parameter resource */
    withEnvironmentParameter(name: string, parameter: ParameterResource): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withEnvironmentParameterInternal(name, parameter));
    }

    /** @internal */
    private async _withEnvironmentConnectionStringInternal(envVarName: string, resource: ResourceBuilderBase): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, envVarName, resource };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withEnvironmentConnectionString',
            rpcArgs
    ...
    /** Sets an environment variable from a connection string resource */
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withEnvironmentConnectionStringInternal(envVarName, resource));
    }

    /** @internal */
    private async _withConnectionPropertyInternal(name: string, value: ReferenceExpression): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withConnectionProperty',
            rpcArgs
    ...
    /** Adds a connection property with a reference expression */
    withConnectionProperty(name: string, value: ReferenceExpression): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withConnectionPropertyInternal(name, value));
    }

    /** @internal */
    private async _withConnectionPropertyValueInternal(name: string, value: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withConnectionPropertyValue',
            rpcArgs
    ...
    /** Adds a connection property with a string value */
    withConnectionPropertyValue(name: string, value: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withConnectionPropertyValueInternal(name, value));
    }

    /** @internal */
    private async _withArgsInternal(args: string[]): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, args };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withArgs',
            rpcArgs
    ...
    /** Adds arguments */
    withArgs(args: string[]): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withArgsInternal(args));
    }

    /** @internal */
    private async _withArgsCallbackInternal(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): Promise<MilvusServerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as CommandLineArgsCallbackContextHandle;
            const obj = new CommandLineArgsCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Sets command-line arguments via callback */
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withArgsCallbackInternal(callback));
    }

    /** @internal */
    private async _withArgsCallbackAsyncInternal(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): Promise<MilvusServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as CommandLineArgsCallbackContextHandle;
            const arg = new CommandLineArgsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Sets command-line arguments via async callback */
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withArgsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withReferenceInternal(source: ResourceBuilderBase, connectionName?: string, optional?: boolean): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        if (connectionName !== undefined) rpcArgs.connectionName = connectionName;
        if (optional !== undefined) rpcArgs.optional = optional;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
    ...
    /** Adds a reference to another resource */
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): MilvusServerResourcePromise {
        const connectionName = options?.connectionName;
        const optional = options?.optional;
        return new MilvusServerResourcePromise(this._withReferenceInternal(source, connectionName, optional));
    }

    /** @internal */
    private async _withServiceReferenceInternal(source: ResourceBuilderBase): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withServiceReference',
            rpcArgs
    ...
    /** Adds a service discovery reference to another resource */
    withServiceReference(source: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withServiceReferenceInternal(source));
    }

    /** @internal */
    private async _withServiceReferenceNamedInternal(source: ResourceBuilderBase, name: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source, name };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withServiceReferenceNamed',
            rpcArgs
    ...
    /** Adds a named service discovery reference */
    withServiceReferenceNamed(source: ResourceBuilderBase, name: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withServiceReferenceNamedInternal(source, name));
    }

    /** @internal */
    private async _withReferenceUriInternal(name: string, uri: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, uri };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withReferenceUri',
            rpcArgs
    ...
    /** Adds a reference to a URI */
    withReferenceUri(name: string, uri: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withReferenceUriInternal(name, uri));
    }

    /** @internal */
    private async _withReferenceExternalServiceInternal(externalService: ExternalServiceResource): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, externalService };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withReferenceExternalService',
            rpcArgs
    ...
    /** Adds a reference to an external service */
    withReferenceExternalService(externalService: ExternalServiceResource): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withReferenceExternalServiceInternal(externalService));
    }

    /** @internal */
    private async _withReferenceEndpointInternal(endpointReference: EndpointReference): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, endpointReference };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withReferenceEndpoint',
            rpcArgs
    ...
    /** Adds a reference to an endpoint */
    withReferenceEndpoint(endpointReference: EndpointReference): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withReferenceEndpointInternal(endpointReference));
    }

    /** @internal */
    private async _withEndpointInternal(port?: number, targetPort?: number, scheme?: string, name?: string, env?: string, isProxied?: boolean, isExternal?: boolean, protocol?: ProtocolType): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (scheme !== undefined) rpcArgs.scheme = scheme;
    ...
    /** Adds a network endpoint */
    withEndpoint(options?: WithEndpointOptions): MilvusServerResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const scheme = options?.scheme;
        const name = options?.name;
        ...
        const isExternal = options?.isExternal;
        const protocol = options?.protocol;
        return new MilvusServerResourcePromise(this._withEndpointInternal(port, targetPort, scheme, name, env, isProxied, isExternal, protocol));
    }

    /** @internal */
    private async _withHttpEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    /** Adds an HTTP endpoint */
    withHttpEndpoint(options?: WithHttpEndpointOptions): MilvusServerResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new MilvusServerResourcePromise(this._withHttpEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withHttpsEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    /** Adds an HTTPS endpoint */
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): MilvusServerResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new MilvusServerResourcePromise(this._withHttpsEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withExternalHttpEndpointsInternal(): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withExternalHttpEndpoints',
            rpcArgs
    ...
    /** Makes HTTP endpoints externally accessible */
    withExternalHttpEndpoints(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withExternalHttpEndpointsInternal());
    }
    ...
    /** @internal */
    private async _asHttp2ServiceInternal(): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/asHttp2Service',
            rpcArgs
    ...
    /** Configures resource for HTTP/2 */
    asHttp2Service(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._asHttp2ServiceInternal());
    }

    /** @internal */
    private async _withUrlsCallbackInternal(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): Promise<MilvusServerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as ResourceUrlsCallbackContextHandle;
            const obj = new ResourceUrlsCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withUrlsCallbackInternal(callback));
    }

    /** @internal */
    private async _withUrlsCallbackAsyncInternal(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): Promise<MilvusServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ResourceUrlsCallbackContextHandle;
            const arg = new ResourceUrlsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withUrlsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withUrlInternal(url: string, displayText?: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withUrl',
    ...
    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): MilvusServerResourcePromise {
        const displayText = options?.displayText;
        return new MilvusServerResourcePromise(this._withUrlInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlExpressionInternal(url: ReferenceExpression, displayText?: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withUrlExpression',
    ...
    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): MilvusServerResourcePromise {
        const displayText = options?.displayText;
        return new MilvusServerResourcePromise(this._withUrlExpressionInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlForEndpointInternal(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): Promise<MilvusServerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const obj = wrapIfHandle(objData) as ResourceUrlAnnotation;
            await callback(obj);
        });
    ...
    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withUrlForEndpointInternal(endpointName, callback));
    }

    /** @internal */
    private async _withUrlForEndpointFactoryInternal(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): Promise<MilvusServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as EndpointReferenceHandle;
            const arg = new EndpointReference(argHandle, this._client);
            return await callback(arg);
    ...
    /** Adds a URL for a specific endpoint via factory callback */
    withUrlForEndpointFactory(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withUrlForEndpointFactoryInternal(endpointName, callback));
    }

    /** @internal */
    private async _excludeFromManifestInternal(): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/excludeFromManifest',
            rpcArgs
    ...
    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._excludeFromManifestInternal());
    }

    /** @internal */
    private async _waitForInternal(dependency: ResourceBuilderBase): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/waitFor',
            rpcArgs
    ...
    /** Waits for another resource to be ready */
    waitFor(dependency: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._waitForInternal(dependency));
    }

    /** @internal */
    private async _waitForWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency, waitBehavior };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/waitForWithBehavior',
            rpcArgs
    ...
    /** Waits for another resource with specific behavior */
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._waitForWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _waitForStartInternal(dependency: ResourceBuilderBase): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/waitForStart',
            rpcArgs
    ...
    /** Waits for another resource to start */
    waitForStart(dependency: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._waitForStartInternal(dependency));
    }

    /** @internal */
    private async _waitForStartWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency, waitBehavior };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/waitForStartWithBehavior',
            rpcArgs
    ...
    /** Waits for another resource to start with specific behavior */
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._waitForStartWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _withExplicitStartInternal(): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withExplicitStart',
            rpcArgs
    ...
    /** Prevents resource from starting automatically */
    withExplicitStart(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withExplicitStartInternal());
    }

    /** @internal */
    private async _waitForCompletionInternal(dependency: ResourceBuilderBase, exitCode?: number): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        if (exitCode !== undefined) rpcArgs.exitCode = exitCode;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/waitForCompletion',
    ...
    /** Waits for resource completion */
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): MilvusServerResourcePromise {
        const exitCode = options?.exitCode;
        return new MilvusServerResourcePromise(this._waitForCompletionInternal(dependency, exitCode));
    }

    /** @internal */
    private async _withHealthCheckInternal(key: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, key };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withHealthCheck',
            rpcArgs
    ...
    /** Adds a health check by key */
    withHealthCheck(key: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withHealthCheckInternal(key));
    }

    /** @internal */
    private async _withHttpHealthCheckInternal(path?: string, statusCode?: number, endpointName?: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (path !== undefined) rpcArgs.path = path;
        if (statusCode !== undefined) rpcArgs.statusCode = statusCode;
        if (endpointName !== undefined) rpcArgs.endpointName = endpointName;
    ...
    /** Adds an HTTP health check */
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): MilvusServerResourcePromise {
        const path = options?.path;
        const statusCode = options?.statusCode;
        const endpointName = options?.endpointName;
        return new MilvusServerResourcePromise(this._withHttpHealthCheckInternal(path, statusCode, endpointName));
    }

    /** @internal */
    private async _withCommandInternal(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, commandOptions?: CommandOptions): Promise<MilvusServerResource> {
        const executeCommandId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ExecuteCommandContextHandle;
            const arg = new ExecuteCommandContext(argHandle, this._client);
            return await executeCommand(arg);
    ...
    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): MilvusServerResourcePromise {
        const commandOptions = options?.commandOptions;
        return new MilvusServerResourcePromise(this._withCommandInternal(name, displayName, executeCommand, commandOptions));
    }

    /** @internal */
    private async _withDeveloperCertificateTrustInternal(trust: boolean): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, trust };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withDeveloperCertificateTrust',
            rpcArgs
    ...
    /** Configures developer certificate trust */
    withDeveloperCertificateTrust(trust: boolean): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withDeveloperCertificateTrustInternal(trust));
    }

    /** @internal */
    private async _withCertificateTrustScopeInternal(scope: CertificateTrustScope): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, scope };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withCertificateTrustScope',
            rpcArgs
    ...
    /** Sets the certificate trust scope */
    withCertificateTrustScope(scope: CertificateTrustScope): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withCertificateTrustScopeInternal(scope));
    }

    /** @internal */
    private async _withHttpsDeveloperCertificateInternal(password?: ParameterResource): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (password !== undefined) rpcArgs.password = password;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withHttpsDeveloperCertificate',
    ...
    /** Configures HTTPS with a developer certificate */
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): MilvusServerResourcePromise {
        const password = options?.password;
        return new MilvusServerResourcePromise(this._withHttpsDeveloperCertificateInternal(password));
    }

    /** @internal */
    private async _withoutHttpsCertificateInternal(): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withoutHttpsCertificate',
            rpcArgs
    ...
    /** Removes HTTPS certificate configuration */
    withoutHttpsCertificate(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withoutHttpsCertificateInternal());
    }

    /** @internal */
    private async _withParentRelationshipInternal(parent: ResourceBuilderBase): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, parent };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withParentRelationship',
            rpcArgs
    ...
    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withParentRelationshipInternal(parent));
    }

    /** @internal */
    private async _withChildRelationshipInternal(child: ResourceBuilderBase): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, child };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withChildRelationship',
            rpcArgs
    ...
    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withChildRelationshipInternal(child));
    }

    /** @internal */
    private async _withIconNameInternal(iconName: string, iconVariant?: IconVariant): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, iconName };
        if (iconVariant !== undefined) rpcArgs.iconVariant = iconVariant;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withIconName',
    ...
    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): MilvusServerResourcePromise {
        const iconVariant = options?.iconVariant;
        return new MilvusServerResourcePromise(this._withIconNameInternal(iconName, iconVariant));
    }

    /** @internal */
    private async _withHttpProbeInternal(probeType: ProbeType, path?: string, initialDelaySeconds?: number, periodSeconds?: number, timeoutSeconds?: number, failureThreshold?: number, successThreshold?: number, endpointName?: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, probeType };
        if (path !== undefined) rpcArgs.path = path;
        if (initialDelaySeconds !== undefined) rpcArgs.initialDelaySeconds = initialDelaySeconds;
        if (periodSeconds !== undefined) rpcArgs.periodSeconds = periodSeconds;
    ...
    /** Adds an HTTP health probe to the resource */
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): MilvusServerResourcePromise {
        const path = options?.path;
        const initialDelaySeconds = options?.initialDelaySeconds;
        const periodSeconds = options?.periodSeconds;
        const timeoutSeconds = options?.timeoutSeconds;
        ...
        const successThreshold = options?.successThreshold;
        const endpointName = options?.endpointName;
        return new MilvusServerResourcePromise(this._withHttpProbeInternal(probeType, path, initialDelaySeconds, periodSeconds, timeoutSeconds, failureThreshold, successThreshold, endpointName));
    }

    /** @internal */
    private async _excludeFromMcpInternal(): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/excludeFromMcp',
            rpcArgs
    ...
    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._excludeFromMcpInternal());
    }

    /** @internal */
    private async _withRemoteImageNameInternal(remoteImageName: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageName };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withRemoteImageName',
            rpcArgs
    ...
    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withRemoteImageNameInternal(remoteImageName));
    }

    /** @internal */
    private async _withRemoteImageTagInternal(remoteImageTag: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageTag };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting/withRemoteImageTag',
            rpcArgs
    ...
    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withRemoteImageTagInternal(remoteImageTag));
    }

    /** @internal */
    private async _withPipelineStepFactoryInternal(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, dependsOn?: string[], requiredBy?: string[], tags?: string[], description?: string): Promise<MilvusServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineStepContextHandle;
            const arg = new PipelineStepContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): MilvusServerResourcePromise {
        const dependsOn = options?.dependsOn;
        const requiredBy = options?.requiredBy;
        const tags = options?.tags;
        const description = options?.description;
        return new MilvusServerResourcePromise(this._withPipelineStepFactoryInternal(stepName, callback, dependsOn, requiredBy, tags, description));
    }

    /** @internal */
    private async _withPipelineConfigurationAsyncInternal(callback: (arg: PipelineConfigurationContext) => Promise<void>): Promise<MilvusServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineConfigurationContextHandle;
            const arg = new PipelineConfigurationContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withPipelineConfigurationAsyncInternal(callback));
    }

    /** @internal */
    private async _withPipelineConfigurationInternal(callback: (obj: PipelineConfigurationContext) => Promise<void>): Promise<MilvusServerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as PipelineConfigurationContextHandle;
            const obj = new PipelineConfigurationContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withPipelineConfigurationInternal(callback));
    }

    /** @internal */
    private async _withVolumeInternal(target: string, name?: string, isReadOnly?: boolean): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { resource: this._handle, target };
        if (name !== undefined) rpcArgs.name = name;
        if (isReadOnly !== undefined) rpcArgs.isReadOnly = isReadOnly;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
    ...
    /** Adds a volume */
    withVolume(target: string, options?: WithVolumeOptions): MilvusServerResourcePromise {
        const name = options?.name;
        const isReadOnly = options?.isReadOnly;
        return new MilvusServerResourcePromise(this._withVolumeInternal(target, name, isReadOnly));
    }
    ...
    /** @internal */
    private async _addDatabaseInternal(name: string, databaseName?: string): Promise<MilvusDatabaseResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name };
        if (databaseName !== undefined) rpcArgs.databaseName = databaseName;
        const result = await this._client.invokeCapability<MilvusDatabaseResourceHandle>(
            'Aspire.Hosting.Milvus/addDatabase',
            rpcArgs
        );
        return new MilvusDatabaseResource(result, this._client);
    }
    ...
    addDatabase(name: string, options?: AddDatabaseOptions): MilvusDatabaseResourcePromise {
        const databaseName = options?.databaseName;
        return new MilvusDatabaseResourcePromise(this._addDatabaseInternal(name, databaseName));
    }

    /** @internal */
    private async _withAttuInternal(configureContainer?: (obj: AttuResource) => Promise<void>, containerName?: string): Promise<MilvusServerResource> {
        const configureContainerId = configureContainer ? registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as AttuResourceHandle;
            const obj = new AttuResource(objHandle, this._client);
        ...
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting.Milvus/withAttu',
            rpcArgs
        );
        return new MilvusServerResource(result, this._client);
    }
    ...
    withAttu(options?: WithAttuOptions): MilvusServerResourcePromise {
        const configureContainer = options?.configureContainer;
        const containerName = options?.containerName;
        return new MilvusServerResourcePromise(this._withAttuInternal(configureContainer, containerName));
    }

    /** @internal */
    private async _withDataVolumeInternal(name?: string, isReadOnly?: boolean): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (name !== undefined) rpcArgs.name = name;
        if (isReadOnly !== undefined) rpcArgs.isReadOnly = isReadOnly;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting.Milvus/withDataVolume',
            rpcArgs
        );
        return new MilvusServerResource(result, this._client);
    }
    ...
    withDataVolume(options?: WithDataVolumeOptions): MilvusServerResourcePromise {
        const name = options?.name;
        const isReadOnly = options?.isReadOnly;
        return new MilvusServerResourcePromise(this._withDataVolumeInternal(name, isReadOnly));
    }

    /** @internal */
    private async _withDataBindMountInternal(source: string, isReadOnly?: boolean): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        if (isReadOnly !== undefined) rpcArgs.isReadOnly = isReadOnly;
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting.Milvus/withDataBindMount',
            rpcArgs
        );
        return new MilvusServerResource(result, this._client);
    }
    ...
    withDataBindMount(source: string, options?: WithDataBindMountOptions): MilvusServerResourcePromise {
        const isReadOnly = options?.isReadOnly;
        return new MilvusServerResourcePromise(this._withDataBindMountInternal(source, isReadOnly));
    }

    /** @internal */
    private async _withConfigurationFileInternal(configurationFilePath: string): Promise<MilvusServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, configurationFilePath };
        const result = await this._client.invokeCapability<MilvusServerResourceHandle>(
            'Aspire.Hosting.Milvus/withConfigurationFile',
            rpcArgs
        );
        return new MilvusServerResource(result, this._client);
    }
    ...
    withConfigurationFile(configurationFilePath: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._withConfigurationFileInternal(configurationFilePath));
    }

}

/**
 * Thenable wrapper for MilvusServerResource that enables fluent chaining.
 * @example
 * await builder.addSomething().withX().withY();
 */
export class MilvusServerResourcePromise implements PromiseLike<MilvusServerResource> {
    constructor(private _promise: Promise<MilvusServerResource>) {}

    then<TResult1 = MilvusServerResource, TResult2 = never>(
        onfulfilled?: ((value: MilvusServerResource) => TResult1 | PromiseLike<TResult1>) | null,
        onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null
    ): PromiseLike<TResult1 | TResult2> {
        return this._promise.then(onfulfilled, onrejected);
    }
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withContainerRegistry(registry)));
    }

    /** Adds a bind mount */
    withBindMount(source: string, target: string, options?: WithBindMountOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withBindMount(source, target, options)));
    }

    /** Sets the container entrypoint */
    withEntrypoint(entrypoint: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withEntrypoint(entrypoint)));
    }

    /** Sets the container image tag */
    withImageTag(tag: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withImageTag(tag)));
    }

    /** Sets the container image registry */
    withImageRegistry(registry: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withImageRegistry(registry)));
    }

    /** Sets the container image */
    withImage(image: string, options?: WithImageOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withImage(image, options)));
    }

    /** Sets the image SHA256 digest */
    withImageSHA256(sha256: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withImageSHA256(sha256)));
    }

    /** Adds runtime arguments for the container */
    withContainerRuntimeArgs(args: string[]): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withContainerRuntimeArgs(args)));
    }

    /** Sets the lifetime behavior of the container resource */
    withLifetime(lifetime: ContainerLifetime): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withLifetime(lifetime)));
    }

    /** Sets the container image pull policy */
    withImagePullPolicy(pullPolicy: ImagePullPolicy): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withImagePullPolicy(pullPolicy)));
    }

    /** Configures the resource to be published as a container */
    publishAsContainer(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.publishAsContainer()));
    }

    /** Configures the resource to use a Dockerfile */
    withDockerfile(contextPath: string, options?: WithDockerfileOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withDockerfile(contextPath, options)));
    }

    /** Sets the container name */
    withContainerName(name: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withContainerName(name)));
    }

    /** Adds a build argument from a parameter resource */
    withBuildArg(name: string, value: ParameterResource): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withBuildArg(name, value)));
    }

    /** Adds a build secret from a parameter resource */
    withBuildSecret(name: string, value: ParameterResource): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withBuildSecret(name, value)));
    }

    /** Configures endpoint proxy support */
    withEndpointProxySupport(proxyEnabled: boolean): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withEndpointProxySupport(proxyEnabled)));
    }

    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withDockerfileBaseImage(options)));
    }

    /** Adds a network alias for the container */
    withContainerNetworkAlias(alias: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withContainerNetworkAlias(alias)));
    }

    /** Configures an MCP server endpoint on the resource */
    withMcpServer(options?: WithMcpServerOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withMcpServer(options)));
    }

    /** Configures OTLP telemetry export */
    withOtlpExporter(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withOtlpExporter()));
    }

    /** Configures OTLP telemetry export with specific protocol */
    withOtlpExporterProtocol(protocol: OtlpProtocol): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withOtlpExporterProtocol(protocol)));
    }

    /** Publishes the resource as a connection string */
    publishAsConnectionString(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.publishAsConnectionString()));
    }

    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withRequiredCommand(command, options)));
    }

    /** Sets an environment variable */
    withEnvironment(name: string, value: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withEnvironment(name, value)));
    }

    /** Adds an environment variable with a reference expression */
    withEnvironmentExpression(name: string, value: ReferenceExpression): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withEnvironmentExpression(name, value)));
    }

    /** Sets environment variables via callback */
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withEnvironmentCallback(callback)));
    }

    /** Sets environment variables via async callback */
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withEnvironmentCallbackAsync(callback)));
    }

    /** Sets an environment variable from an endpoint reference */
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withEnvironmentEndpoint(name, endpointReference)));
    }

    /** Sets an environment variable from a parameter resource */
    withEnvironmentParameter(name: string, parameter: ParameterResource): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withEnvironmentParameter(name, parameter)));
    }

    /** Sets an environment variable from a connection string resource */
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withEnvironmentConnectionString(envVarName, resource)));
    }

    /** Adds a connection property with a reference expression */
    withConnectionProperty(name: string, value: ReferenceExpression): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withConnectionProperty(name, value)));
    }

    /** Adds a connection property with a string value */
    withConnectionPropertyValue(name: string, value: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withConnectionPropertyValue(name, value)));
    }

    /** Adds arguments */
    withArgs(args: string[]): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withArgs(args)));
    }

    /** Sets command-line arguments via callback */
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withArgsCallback(callback)));
    }

    /** Sets command-line arguments via async callback */
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withArgsCallbackAsync(callback)));
    }

    /** Adds a reference to another resource */
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withReference(source, options)));
    }

    /** Adds a service discovery reference to another resource */
    withServiceReference(source: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withServiceReference(source)));
    }

    /** Adds a named service discovery reference */
    withServiceReferenceNamed(source: ResourceBuilderBase, name: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withServiceReferenceNamed(source, name)));
    }

    /** Adds a reference to a URI */
    withReferenceUri(name: string, uri: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withReferenceUri(name, uri)));
    }

    /** Adds a reference to an external service */
    withReferenceExternalService(externalService: ExternalServiceResource): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withReferenceExternalService(externalService)));
    }

    /** Adds a reference to an endpoint */
    withReferenceEndpoint(endpointReference: EndpointReference): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withReferenceEndpoint(endpointReference)));
    }

    /** Adds a network endpoint */
    withEndpoint(options?: WithEndpointOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withEndpoint(options)));
    }

    /** Adds an HTTP endpoint */
    withHttpEndpoint(options?: WithHttpEndpointOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withHttpEndpoint(options)));
    }

    /** Adds an HTTPS endpoint */
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withHttpsEndpoint(options)));
    }

    /** Makes HTTP endpoints externally accessible */
    withExternalHttpEndpoints(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withExternalHttpEndpoints()));
    }
    ...
    /** Configures resource for HTTP/2 */
    asHttp2Service(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.asHttp2Service()));
    }

    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withUrlsCallback(callback)));
    }

    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withUrlsCallbackAsync(callback)));
    }

    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withUrl(url, options)));
    }

    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withUrlExpression(url, options)));
    }

    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withUrlForEndpoint(endpointName, callback)));
    }

    /** Adds a URL for a specific endpoint via factory callback */
    withUrlForEndpointFactory(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withUrlForEndpointFactory(endpointName, callback)));
    }

    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.excludeFromManifest()));
    }

    /** Waits for another resource to be ready */
    waitFor(dependency: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.waitFor(dependency)));
    }

    /** Waits for another resource with specific behavior */
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.waitForWithBehavior(dependency, waitBehavior)));
    }

    /** Waits for another resource to start */
    waitForStart(dependency: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.waitForStart(dependency)));
    }

    /** Waits for another resource to start with specific behavior */
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.waitForStartWithBehavior(dependency, waitBehavior)));
    }

    /** Prevents resource from starting automatically */
    withExplicitStart(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withExplicitStart()));
    }

    /** Waits for resource completion */
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.waitForCompletion(dependency, options)));
    }

    /** Adds a health check by key */
    withHealthCheck(key: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withHealthCheck(key)));
    }

    /** Adds an HTTP health check */
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withHttpHealthCheck(options)));
    }

    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withCommand(name, displayName, executeCommand, options)));
    }

    /** Configures developer certificate trust */
    withDeveloperCertificateTrust(trust: boolean): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withDeveloperCertificateTrust(trust)));
    }

    /** Sets the certificate trust scope */
    withCertificateTrustScope(scope: CertificateTrustScope): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withCertificateTrustScope(scope)));
    }

    /** Configures HTTPS with a developer certificate */
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withHttpsDeveloperCertificate(options)));
    }

    /** Removes HTTPS certificate configuration */
    withoutHttpsCertificate(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withoutHttpsCertificate()));
    }

    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withParentRelationship(parent)));
    }

    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withChildRelationship(child)));
    }

    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withIconName(iconName, options)));
    }

    /** Adds an HTTP health probe to the resource */
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withHttpProbe(probeType, options)));
    }

    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.excludeFromMcp()));
    }

    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withRemoteImageName(remoteImageName)));
    }

    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withRemoteImageTag(remoteImageTag)));
    }

    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withPipelineStepFactory(stepName, callback, options)));
    }

    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withPipelineConfigurationAsync(callback)));
    }

    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withPipelineConfiguration(callback)));
    }

    /** Adds a volume */
    withVolume(target: string, options?: WithVolumeOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withVolume(target, options)));
    }
    ...
    /** Adds a Milvus database resource to a Milvus server resource. */
    addDatabase(name: string, options?: AddDatabaseOptions): MilvusDatabaseResourcePromise {
        return new MilvusDatabaseResourcePromise(this._promise.then(obj => obj.addDatabase(name, options)));
    }

    /** Adds the Attu administration tool for Milvus. */
    withAttu(options?: WithAttuOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withAttu(options)));
    }

    /** Adds a persistent data volume to the Milvus server resource. */
    withDataVolume(options?: WithDataVolumeOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withDataVolume(options)));
    }

    /** Mounts a host directory as the Milvus data directory. */
    withDataBindMount(source: string, options?: WithDataBindMountOptions): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withDataBindMount(source, options)));
    }

    /** Copies a Milvus configuration file into the container. */
    withConfigurationFile(configurationFilePath: string): MilvusServerResourcePromise {
        return new MilvusServerResourcePromise(this._promise.then(obj => obj.withConfigurationFile(configurationFilePath)));
    }
...
registerHandleWrapper('Aspire.Hosting.Milvus/Aspire.Hosting.Milvus.AttuResource', (handle, client) => new AttuResource(handle as AttuResourceHandle, client));
...
registerHandleWrapper('Aspire.Hosting.Milvus/Aspire.Hosting.ApplicationModel.MilvusDatabaseResource', (handle, client) => new MilvusDatabaseResource(handle as MilvusDatabaseResourceHandle, client));
registerHandleWrapper('Aspire.Hosting.Milvus/Aspire.Hosting.Milvus.MilvusServerResource', (handle, client) => new MilvusServerResource(handle as MilvusServerResourceHandle, client));
