...
/** Handle to PythonAppResource */
type PythonAppResourceHandle = Handle<'Aspire.Hosting.Python/Aspire.Hosting.Python.PythonAppResource'>;

/** Handle to UvicornAppResource */
type UvicornAppResourceHandle = Handle<'Aspire.Hosting.Python/Aspire.Hosting.Python.UvicornAppResource'>;
...
export interface WithPipOptions {
    install?: boolean;
    installArgs?: string[];
}
...
export interface WithUvOptions {
    install?: boolean;
    args?: string[];
}

export interface WithVirtualEnvironmentOptions {
...
export class DistributedApplicationBuilder {
    ...
    /** @internal */
    async _addPythonAppInternal(name: string, appDirectory: string, scriptPath: string): Promise<PythonAppResource> {
    ...
    addPythonApp(name: string, appDirectory: string, scriptPath: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._addPythonAppInternal(name, appDirectory, scriptPath));
    }

    /** Adds a Python module application resource */
    /** @internal */
    async _addPythonModuleInternal(name: string, appDirectory: string, moduleName: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, appDirectory, moduleName };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
    ...
    addPythonModule(name: string, appDirectory: string, moduleName: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._addPythonModuleInternal(name, appDirectory, moduleName));
    }

    /** Adds a Python executable application resource */
    /** @internal */
    async _addPythonExecutableInternal(name: string, appDirectory: string, executableName: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, appDirectory, executableName };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
    ...
    addPythonExecutable(name: string, appDirectory: string, executableName: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._addPythonExecutableInternal(name, appDirectory, executableName));
    }

    /** Adds a Uvicorn-based Python application resource */
    /** @internal */
    async _addUvicornAppInternal(name: string, appDirectory: string, app: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, appDirectory, app };
    ...
    addUvicornApp(name: string, appDirectory: string, app: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._addUvicornAppInternal(name, appDirectory, app));
    }
...
export class DistributedApplicationBuilderPromise implements PromiseLike<DistributedApplicationBuilder> {
    ...
    /** Adds a Python script application resource */
    addPythonApp(name: string, appDirectory: string, scriptPath: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.addPythonApp(name, appDirectory, scriptPath)));
    }

    /** Adds a Python module application resource */
    addPythonModule(name: string, appDirectory: string, moduleName: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.addPythonModule(name, appDirectory, moduleName)));
    }

    /** Adds a Python executable application resource */
    addPythonExecutable(name: string, appDirectory: string, executableName: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.addPythonExecutable(name, appDirectory, executableName)));
    }

    /** Adds a Uvicorn-based Python application resource */
    addUvicornApp(name: string, appDirectory: string, app: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.addUvicornApp(name, appDirectory, app)));
    }
...
export class PythonAppResource extends ResourceBuilderBase<PythonAppResourceHandle> {
    constructor(handle: PythonAppResourceHandle, client: AspireClientRpc) {
        super(handle, client);
    }

    /** @internal */
    private async _withContainerRegistryInternal(registry: ResourceBuilderBase): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withContainerRegistry',
            rpcArgs
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withContainerRegistryInternal(registry));
    }

    /** @internal */
    private async _withDockerfileBaseImageInternal(buildImage?: string, runtimeImage?: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (buildImage !== undefined) rpcArgs.buildImage = buildImage;
        if (runtimeImage !== undefined) rpcArgs.runtimeImage = runtimeImage;
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
    ...
    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): PythonAppResourcePromise {
        const buildImage = options?.buildImage;
        const runtimeImage = options?.runtimeImage;
        return new PythonAppResourcePromise(this._withDockerfileBaseImageInternal(buildImage, runtimeImage));
    }

    /** @internal */
    private async _publishAsDockerFileInternal(): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/publishAsDockerFile',
            rpcArgs
    ...
    /** Publishes the executable as a Docker container */
    publishAsDockerFile(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._publishAsDockerFileInternal());
    }

    /** @internal */
    private async _publishAsDockerFileWithConfigureInternal(configure: (obj: ContainerResource) => Promise<void>): Promise<PythonAppResource> {
        const configureId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as ContainerResourceHandle;
            const obj = new ContainerResource(objHandle, this._client);
            await configure(obj);
    ...
    /** Publishes an executable as a Docker file with optional container configuration */
    publishAsDockerFileWithConfigure(configure: (obj: ContainerResource) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._publishAsDockerFileWithConfigureInternal(configure));
    }

    /** @internal */
    private async _withExecutableCommandInternal(command: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, command };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withExecutableCommand',
            rpcArgs
    ...
    /** Sets the executable command */
    withExecutableCommand(command: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withExecutableCommandInternal(command));
    }

    /** @internal */
    private async _withWorkingDirectoryInternal(workingDirectory: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, workingDirectory };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withWorkingDirectory',
            rpcArgs
    ...
    /** Sets the executable working directory */
    withWorkingDirectory(workingDirectory: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withWorkingDirectoryInternal(workingDirectory));
    }

    /** @internal */
    private async _withMcpServerInternal(path?: string, endpointName?: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (path !== undefined) rpcArgs.path = path;
        if (endpointName !== undefined) rpcArgs.endpointName = endpointName;
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
    ...
    /** Configures an MCP server endpoint on the resource */
    withMcpServer(options?: WithMcpServerOptions): PythonAppResourcePromise {
        const path = options?.path;
        const endpointName = options?.endpointName;
        return new PythonAppResourcePromise(this._withMcpServerInternal(path, endpointName));
    }

    /** @internal */
    private async _withOtlpExporterInternal(): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withOtlpExporter',
            rpcArgs
    ...
    /** Configures OTLP telemetry export */
    withOtlpExporter(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withOtlpExporterInternal());
    }

    /** @internal */
    private async _withOtlpExporterProtocolInternal(protocol: OtlpProtocol): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, protocol };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withOtlpExporterProtocol',
            rpcArgs
    ...
    /** Configures OTLP telemetry export with specific protocol */
    withOtlpExporterProtocol(protocol: OtlpProtocol): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withOtlpExporterProtocolInternal(protocol));
    }

    /** @internal */
    private async _withRequiredCommandInternal(command: string, helpLink?: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, command };
        if (helpLink !== undefined) rpcArgs.helpLink = helpLink;
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withRequiredCommand',
    ...
    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): PythonAppResourcePromise {
        const helpLink = options?.helpLink;
        return new PythonAppResourcePromise(this._withRequiredCommandInternal(command, helpLink));
    }

    /** @internal */
    private async _withEnvironmentInternal(name: string, value: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withEnvironment',
            rpcArgs
    ...
    /** Sets an environment variable */
    withEnvironment(name: string, value: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withEnvironmentInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentExpressionInternal(name: string, value: ReferenceExpression): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withEnvironmentExpression',
            rpcArgs
    ...
    /** Adds an environment variable with a reference expression */
    withEnvironmentExpression(name: string, value: ReferenceExpression): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withEnvironmentExpressionInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentCallbackInternal(callback: (obj: EnvironmentCallbackContext) => Promise<void>): Promise<PythonAppResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as EnvironmentCallbackContextHandle;
            const obj = new EnvironmentCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Sets environment variables via callback */
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withEnvironmentCallbackInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentCallbackAsyncInternal(callback: (arg: EnvironmentCallbackContext) => Promise<void>): Promise<PythonAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as EnvironmentCallbackContextHandle;
            const arg = new EnvironmentCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Sets environment variables via async callback */
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withEnvironmentCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentEndpointInternal(name: string, endpointReference: EndpointReference): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, endpointReference };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withEnvironmentEndpoint',
            rpcArgs
    ...
    /** Sets an environment variable from an endpoint reference */
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withEnvironmentEndpointInternal(name, endpointReference));
    }

    /** @internal */
    private async _withEnvironmentParameterInternal(name: string, parameter: ParameterResource): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, parameter };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withEnvironmentParameter',
            rpcArgs
    ...
    /** Sets an environment variable from a parameter resource */
    withEnvironmentParameter(name: string, parameter: ParameterResource): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withEnvironmentParameterInternal(name, parameter));
    }

    /** @internal */
    private async _withEnvironmentConnectionStringInternal(envVarName: string, resource: ResourceBuilderBase): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, envVarName, resource };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withEnvironmentConnectionString',
            rpcArgs
    ...
    /** Sets an environment variable from a connection string resource */
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withEnvironmentConnectionStringInternal(envVarName, resource));
    }

    /** @internal */
    private async _withArgsInternal(args: string[]): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, args };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withArgs',
            rpcArgs
    ...
    /** Adds arguments */
    withArgs(args: string[]): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withArgsInternal(args));
    }

    /** @internal */
    private async _withArgsCallbackInternal(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): Promise<PythonAppResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as CommandLineArgsCallbackContextHandle;
            const obj = new CommandLineArgsCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Sets command-line arguments via callback */
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withArgsCallbackInternal(callback));
    }

    /** @internal */
    private async _withArgsCallbackAsyncInternal(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): Promise<PythonAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as CommandLineArgsCallbackContextHandle;
            const arg = new CommandLineArgsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Sets command-line arguments via async callback */
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withArgsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withReferenceInternal(source: ResourceBuilderBase, connectionName?: string, optional?: boolean): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        if (connectionName !== undefined) rpcArgs.connectionName = connectionName;
        if (optional !== undefined) rpcArgs.optional = optional;
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
    ...
    /** Adds a reference to another resource */
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): PythonAppResourcePromise {
        const connectionName = options?.connectionName;
        const optional = options?.optional;
        return new PythonAppResourcePromise(this._withReferenceInternal(source, connectionName, optional));
    }

    /** @internal */
    private async _withServiceReferenceInternal(source: ResourceBuilderBase): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withServiceReference',
            rpcArgs
    ...
    withServiceReference(source: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withServiceReferenceInternal(source));
    }

    /** @internal */
    private async _withServiceReferenceNamedInternal(source: ResourceBuilderBase, name: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source, name };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withServiceReferenceNamed',
            rpcArgs
    ...
    /** Adds a named service discovery reference */
    withServiceReferenceNamed(source: ResourceBuilderBase, name: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withServiceReferenceNamedInternal(source, name));
    }

    /** @internal */
    private async _withReferenceUriInternal(name: string, uri: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, uri };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withReferenceUri',
            rpcArgs
    ...
    /** Adds a reference to a URI */
    withReferenceUri(name: string, uri: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withReferenceUriInternal(name, uri));
    }

    /** @internal */
    private async _withReferenceExternalServiceInternal(externalService: ExternalServiceResource): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, externalService };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withReferenceExternalService',
            rpcArgs
    ...
    /** Adds a reference to an external service */
    withReferenceExternalService(externalService: ExternalServiceResource): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withReferenceExternalServiceInternal(externalService));
    }

    /** @internal */
    private async _withReferenceEndpointInternal(endpointReference: EndpointReference): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, endpointReference };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withReferenceEndpoint',
            rpcArgs
    ...
    /** Adds a reference to an endpoint */
    withReferenceEndpoint(endpointReference: EndpointReference): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withReferenceEndpointInternal(endpointReference));
    }

    /** @internal */
    private async _withEndpointInternal(port?: number, targetPort?: number, scheme?: string, name?: string, env?: string, isProxied?: boolean, isExternal?: boolean, protocol?: ProtocolType): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (scheme !== undefined) rpcArgs.scheme = scheme;
    ...
    /** Adds a network endpoint */
    withEndpoint(options?: WithEndpointOptions): PythonAppResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const scheme = options?.scheme;
        const name = options?.name;
        ...
        const isExternal = options?.isExternal;
        const protocol = options?.protocol;
        return new PythonAppResourcePromise(this._withEndpointInternal(port, targetPort, scheme, name, env, isProxied, isExternal, protocol));
    }

    /** @internal */
    private async _withHttpEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    /** Adds an HTTP endpoint */
    withHttpEndpoint(options?: WithHttpEndpointOptions): PythonAppResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new PythonAppResourcePromise(this._withHttpEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withHttpsEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    /** Adds an HTTPS endpoint */
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): PythonAppResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new PythonAppResourcePromise(this._withHttpsEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withExternalHttpEndpointsInternal(): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withExternalHttpEndpoints',
            rpcArgs
    ...
    /** Makes HTTP endpoints externally accessible */
    withExternalHttpEndpoints(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withExternalHttpEndpointsInternal());
    }
    ...
    /** @internal */
    private async _asHttp2ServiceInternal(): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/asHttp2Service',
            rpcArgs
    ...
    /** Configures resource for HTTP/2 */
    asHttp2Service(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._asHttp2ServiceInternal());
    }

    /** @internal */
    private async _withUrlsCallbackInternal(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): Promise<PythonAppResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as ResourceUrlsCallbackContextHandle;
            const obj = new ResourceUrlsCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withUrlsCallbackInternal(callback));
    }

    /** @internal */
    private async _withUrlsCallbackAsyncInternal(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): Promise<PythonAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ResourceUrlsCallbackContextHandle;
            const arg = new ResourceUrlsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withUrlsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withUrlInternal(url: string, displayText?: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withUrl',
    ...
    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): PythonAppResourcePromise {
        const displayText = options?.displayText;
        return new PythonAppResourcePromise(this._withUrlInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlExpressionInternal(url: ReferenceExpression, displayText?: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withUrlExpression',
    ...
    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): PythonAppResourcePromise {
        const displayText = options?.displayText;
        return new PythonAppResourcePromise(this._withUrlExpressionInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlForEndpointInternal(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): Promise<PythonAppResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const obj = wrapIfHandle(objData) as ResourceUrlAnnotation;
            await callback(obj);
        });
    ...
    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withUrlForEndpointInternal(endpointName, callback));
    }

    /** @internal */
    private async _withUrlForEndpointFactoryInternal(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): Promise<PythonAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as EndpointReferenceHandle;
            const arg = new EndpointReference(argHandle, this._client);
            return await callback(arg);
    ...
    withUrlForEndpointFactory(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withUrlForEndpointFactoryInternal(endpointName, callback));
    }
    ...
    /** @internal */
    private async _excludeFromManifestInternal(): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/excludeFromManifest',
            rpcArgs
    ...
    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._excludeFromManifestInternal());
    }
    ...
    /** Waits for another resource to be ready */
    waitFor(dependency: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._waitForInternal(dependency));
    }

    /** @internal */
    private async _waitForWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency, waitBehavior };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/waitForWithBehavior',
            rpcArgs
    ...
    /** Waits for another resource with specific behavior */
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._waitForWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _waitForStartInternal(dependency: ResourceBuilderBase): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/waitForStart',
            rpcArgs
    ...
    /** Waits for another resource to start */
    waitForStart(dependency: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._waitForStartInternal(dependency));
    }

    /** @internal */
    private async _waitForStartWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency, waitBehavior };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/waitForStartWithBehavior',
            rpcArgs
    ...
    /** Waits for another resource to start with specific behavior */
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._waitForStartWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _withExplicitStartInternal(): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withExplicitStart',
            rpcArgs
    ...
    /** Prevents resource from starting automatically */
    withExplicitStart(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withExplicitStartInternal());
    }

    /** @internal */
    private async _waitForCompletionInternal(dependency: ResourceBuilderBase, exitCode?: number): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        if (exitCode !== undefined) rpcArgs.exitCode = exitCode;
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/waitForCompletion',
    ...
    /** Waits for resource completion */
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): PythonAppResourcePromise {
        const exitCode = options?.exitCode;
        return new PythonAppResourcePromise(this._waitForCompletionInternal(dependency, exitCode));
    }

    /** @internal */
    private async _withHealthCheckInternal(key: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, key };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withHealthCheck',
            rpcArgs
    ...
    /** Adds a health check by key */
    withHealthCheck(key: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withHealthCheckInternal(key));
    }

    /** @internal */
    private async _withHttpHealthCheckInternal(path?: string, statusCode?: number, endpointName?: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (path !== undefined) rpcArgs.path = path;
        if (statusCode !== undefined) rpcArgs.statusCode = statusCode;
        if (endpointName !== undefined) rpcArgs.endpointName = endpointName;
    ...
    /** Adds an HTTP health check */
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): PythonAppResourcePromise {
        const path = options?.path;
        const statusCode = options?.statusCode;
        const endpointName = options?.endpointName;
        return new PythonAppResourcePromise(this._withHttpHealthCheckInternal(path, statusCode, endpointName));
    }

    /** @internal */
    private async _withCommandInternal(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, commandOptions?: CommandOptions): Promise<PythonAppResource> {
        const executeCommandId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ExecuteCommandContextHandle;
            const arg = new ExecuteCommandContext(argHandle, this._client);
            return await executeCommand(arg);
    ...
    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): PythonAppResourcePromise {
        const commandOptions = options?.commandOptions;
        return new PythonAppResourcePromise(this._withCommandInternal(name, displayName, executeCommand, commandOptions));
    }

    /** @internal */
    private async _withDeveloperCertificateTrustInternal(trust: boolean): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, trust };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withDeveloperCertificateTrust',
            rpcArgs
    ...
    /** Configures developer certificate trust */
    withDeveloperCertificateTrust(trust: boolean): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withDeveloperCertificateTrustInternal(trust));
    }

    /** @internal */
    private async _withCertificateTrustScopeInternal(scope: CertificateTrustScope): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, scope };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withCertificateTrustScope',
            rpcArgs
    ...
    /** Sets the certificate trust scope */
    withCertificateTrustScope(scope: CertificateTrustScope): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withCertificateTrustScopeInternal(scope));
    }

    /** @internal */
    private async _withHttpsDeveloperCertificateInternal(password?: ParameterResource): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (password !== undefined) rpcArgs.password = password;
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withHttpsDeveloperCertificate',
    ...
    /** Configures HTTPS with a developer certificate */
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): PythonAppResourcePromise {
        const password = options?.password;
        return new PythonAppResourcePromise(this._withHttpsDeveloperCertificateInternal(password));
    }

    /** @internal */
    private async _withoutHttpsCertificateInternal(): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withoutHttpsCertificate',
            rpcArgs
    ...
    /** Removes HTTPS certificate configuration */
    withoutHttpsCertificate(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withoutHttpsCertificateInternal());
    }

    /** @internal */
    private async _withParentRelationshipInternal(parent: ResourceBuilderBase): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, parent };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withParentRelationship',
            rpcArgs
    ...
    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withParentRelationshipInternal(parent));
    }

    /** @internal */
    private async _withChildRelationshipInternal(child: ResourceBuilderBase): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, child };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withChildRelationship',
            rpcArgs
    ...
    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withChildRelationshipInternal(child));
    }

    /** @internal */
    private async _withIconNameInternal(iconName: string, iconVariant?: IconVariant): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, iconName };
        if (iconVariant !== undefined) rpcArgs.iconVariant = iconVariant;
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withIconName',
    ...
    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): PythonAppResourcePromise {
        const iconVariant = options?.iconVariant;
        return new PythonAppResourcePromise(this._withIconNameInternal(iconName, iconVariant));
    }

    /** @internal */
    private async _withHttpProbeInternal(probeType: ProbeType, path?: string, initialDelaySeconds?: number, periodSeconds?: number, timeoutSeconds?: number, failureThreshold?: number, successThreshold?: number, endpointName?: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, probeType };
        if (path !== undefined) rpcArgs.path = path;
        if (initialDelaySeconds !== undefined) rpcArgs.initialDelaySeconds = initialDelaySeconds;
        if (periodSeconds !== undefined) rpcArgs.periodSeconds = periodSeconds;
    ...
    /** Adds an HTTP health probe to the resource */
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): PythonAppResourcePromise {
        const path = options?.path;
        const initialDelaySeconds = options?.initialDelaySeconds;
        const periodSeconds = options?.periodSeconds;
        const timeoutSeconds = options?.timeoutSeconds;
        ...
        const successThreshold = options?.successThreshold;
        const endpointName = options?.endpointName;
        return new PythonAppResourcePromise(this._withHttpProbeInternal(probeType, path, initialDelaySeconds, periodSeconds, timeoutSeconds, failureThreshold, successThreshold, endpointName));
    }

    /** @internal */
    private async _excludeFromMcpInternal(): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/excludeFromMcp',
            rpcArgs
    ...
    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._excludeFromMcpInternal());
    }

    /** @internal */
    private async _withRemoteImageNameInternal(remoteImageName: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageName };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withRemoteImageName',
            rpcArgs
    ...
    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withRemoteImageNameInternal(remoteImageName));
    }

    /** @internal */
    private async _withRemoteImageTagInternal(remoteImageTag: string): Promise<PythonAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageTag };
        const result = await this._client.invokeCapability<PythonAppResourceHandle>(
            'Aspire.Hosting/withRemoteImageTag',
            rpcArgs
    ...
    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withRemoteImageTagInternal(remoteImageTag));
    }

    /** @internal */
    private async _withPipelineStepFactoryInternal(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, dependsOn?: string[], requiredBy?: string[], tags?: string[], description?: string): Promise<PythonAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineStepContextHandle;
            const arg = new PipelineStepContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): PythonAppResourcePromise {
        const dependsOn = options?.dependsOn;
        const requiredBy = options?.requiredBy;
        const tags = options?.tags;
        const description = options?.description;
        return new PythonAppResourcePromise(this._withPipelineStepFactoryInternal(stepName, callback, dependsOn, requiredBy, tags, description));
    }

    /** @internal */
    private async _withPipelineConfigurationAsyncInternal(callback: (arg: PipelineConfigurationContext) => Promise<void>): Promise<PythonAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineConfigurationContextHandle;
            const arg = new PipelineConfigurationContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withPipelineConfigurationAsyncInternal(callback));
    }

    /** @internal */
    private async _withPipelineConfigurationInternal(callback: (obj: PipelineConfigurationContext) => Promise<void>): Promise<PythonAppResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as PipelineConfigurationContextHandle;
            const obj = new PipelineConfigurationContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._withPipelineConfigurationInternal(callback));
    }
...
/**
 * Thenable wrapper for PythonAppResource that enables fluent chaining.
 * @example
 * await builder.addSomething().withX().withY();
 */
export class PythonAppResourcePromise implements PromiseLike<PythonAppResource> {
    constructor(private _promise: Promise<PythonAppResource>) {}

    then<TResult1 = PythonAppResource, TResult2 = never>(
        onfulfilled?: ((value: PythonAppResource) => TResult1 | PromiseLike<TResult1>) | null,
        onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null
    ): PromiseLike<TResult1 | TResult2> {
        return this._promise.then(onfulfilled, onrejected);
    }
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withContainerRegistry(registry)));
    }

    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withDockerfileBaseImage(options)));
    }

    /** Publishes the executable as a Docker container */
    publishAsDockerFile(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.publishAsDockerFile()));
    }

    /** Publishes an executable as a Docker file with optional container configuration */
    publishAsDockerFileWithConfigure(configure: (obj: ContainerResource) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.publishAsDockerFileWithConfigure(configure)));
    }

    /** Sets the executable command */
    withExecutableCommand(command: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withExecutableCommand(command)));
    }

    /** Sets the executable working directory */
    withWorkingDirectory(workingDirectory: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withWorkingDirectory(workingDirectory)));
    }

    /** Configures an MCP server endpoint on the resource */
    withMcpServer(options?: WithMcpServerOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withMcpServer(options)));
    }

    /** Configures OTLP telemetry export */
    withOtlpExporter(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withOtlpExporter()));
    }

    /** Configures OTLP telemetry export with specific protocol */
    withOtlpExporterProtocol(protocol: OtlpProtocol): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withOtlpExporterProtocol(protocol)));
    }

    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withRequiredCommand(command, options)));
    }

    /** Sets an environment variable */
    withEnvironment(name: string, value: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withEnvironment(name, value)));
    }

    /** Adds an environment variable with a reference expression */
    withEnvironmentExpression(name: string, value: ReferenceExpression): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withEnvironmentExpression(name, value)));
    }

    /** Sets environment variables via callback */
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withEnvironmentCallback(callback)));
    }

    /** Sets environment variables via async callback */
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withEnvironmentCallbackAsync(callback)));
    }

    /** Sets an environment variable from an endpoint reference */
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withEnvironmentEndpoint(name, endpointReference)));
    }

    /** Sets an environment variable from a parameter resource */
    withEnvironmentParameter(name: string, parameter: ParameterResource): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withEnvironmentParameter(name, parameter)));
    }

    /** Sets an environment variable from a connection string resource */
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withEnvironmentConnectionString(envVarName, resource)));
    }

    /** Adds arguments */
    withArgs(args: string[]): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withArgs(args)));
    }

    /** Sets command-line arguments via callback */
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withArgsCallback(callback)));
    }

    /** Sets command-line arguments via async callback */
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withArgsCallbackAsync(callback)));
    }

    /** Adds a reference to another resource */
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withReference(source, options)));
    }

    /** Adds a service discovery reference to another resource */
    withServiceReference(source: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withServiceReference(source)));
    }

    /** Adds a named service discovery reference */
    withServiceReferenceNamed(source: ResourceBuilderBase, name: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withServiceReferenceNamed(source, name)));
    }

    /** Adds a reference to a URI */
    withReferenceUri(name: string, uri: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withReferenceUri(name, uri)));
    }

    /** Adds a reference to an external service */
    withReferenceExternalService(externalService: ExternalServiceResource): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withReferenceExternalService(externalService)));
    }

    /** Adds a reference to an endpoint */
    withReferenceEndpoint(endpointReference: EndpointReference): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withReferenceEndpoint(endpointReference)));
    }

    /** Adds a network endpoint */
    withEndpoint(options?: WithEndpointOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withEndpoint(options)));
    }

    /** Adds an HTTP endpoint */
    withHttpEndpoint(options?: WithHttpEndpointOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withHttpEndpoint(options)));
    }

    /** Adds an HTTPS endpoint */
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withHttpsEndpoint(options)));
    }

    /** Makes HTTP endpoints externally accessible */
    withExternalHttpEndpoints(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withExternalHttpEndpoints()));
    }
    ...
    /** Configures resource for HTTP/2 */
    asHttp2Service(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.asHttp2Service()));
    }

    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withUrlsCallback(callback)));
    }

    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withUrlsCallbackAsync(callback)));
    }

    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withUrl(url, options)));
    }

    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withUrlExpression(url, options)));
    }

    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withUrlForEndpoint(endpointName, callback)));
    }
    ...
    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.excludeFromManifest()));
    }

    /** Waits for another resource to be ready */
    waitFor(dependency: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.waitFor(dependency)));
    }

    /** Waits for another resource with specific behavior */
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.waitForWithBehavior(dependency, waitBehavior)));
    }

    /** Waits for another resource to start */
    waitForStart(dependency: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.waitForStart(dependency)));
    }

    /** Waits for another resource to start with specific behavior */
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.waitForStartWithBehavior(dependency, waitBehavior)));
    }

    /** Prevents resource from starting automatically */
    withExplicitStart(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withExplicitStart()));
    }

    /** Waits for resource completion */
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.waitForCompletion(dependency, options)));
    }

    /** Adds a health check by key */
    withHealthCheck(key: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withHealthCheck(key)));
    }

    /** Adds an HTTP health check */
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withHttpHealthCheck(options)));
    }

    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withCommand(name, displayName, executeCommand, options)));
    }

    /** Configures developer certificate trust */
    withDeveloperCertificateTrust(trust: boolean): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withDeveloperCertificateTrust(trust)));
    }

    /** Sets the certificate trust scope */
    withCertificateTrustScope(scope: CertificateTrustScope): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withCertificateTrustScope(scope)));
    }

    /** Configures HTTPS with a developer certificate */
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withHttpsDeveloperCertificate(options)));
    }

    /** Removes HTTPS certificate configuration */
    withoutHttpsCertificate(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withoutHttpsCertificate()));
    }

    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withParentRelationship(parent)));
    }

    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withChildRelationship(child)));
    }

    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withIconName(iconName, options)));
    }

    /** Adds an HTTP health probe to the resource */
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withHttpProbe(probeType, options)));
    }

    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.excludeFromMcp()));
    }

    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withRemoteImageName(remoteImageName)));
    }

    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withRemoteImageTag(remoteImageTag)));
    }

    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withPipelineStepFactory(stepName, callback, options)));
    }

    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withPipelineConfigurationAsync(callback)));
    }

    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): PythonAppResourcePromise {
        return new PythonAppResourcePromise(this._promise.then(obj => obj.withPipelineConfiguration(callback)));
    }
...
export class UvicornAppResource extends ResourceBuilderBase<UvicornAppResourceHandle> {
    constructor(handle: UvicornAppResourceHandle, client: AspireClientRpc) {
        super(handle, client);
    }

    /** @internal */
    private async _withContainerRegistryInternal(registry: ResourceBuilderBase): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withContainerRegistry',
            rpcArgs
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withContainerRegistryInternal(registry));
    }

    /** @internal */
    private async _withDockerfileBaseImageInternal(buildImage?: string, runtimeImage?: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (buildImage !== undefined) rpcArgs.buildImage = buildImage;
        if (runtimeImage !== undefined) rpcArgs.runtimeImage = runtimeImage;
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
    ...
    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): UvicornAppResourcePromise {
        const buildImage = options?.buildImage;
        const runtimeImage = options?.runtimeImage;
        return new UvicornAppResourcePromise(this._withDockerfileBaseImageInternal(buildImage, runtimeImage));
    }

    /** @internal */
    private async _publishAsDockerFileInternal(): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/publishAsDockerFile',
            rpcArgs
    ...
    /** Publishes the executable as a Docker container */
    publishAsDockerFile(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._publishAsDockerFileInternal());
    }

    /** @internal */
    private async _publishAsDockerFileWithConfigureInternal(configure: (obj: ContainerResource) => Promise<void>): Promise<UvicornAppResource> {
        const configureId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as ContainerResourceHandle;
            const obj = new ContainerResource(objHandle, this._client);
            await configure(obj);
    ...
    /** Publishes an executable as a Docker file with optional container configuration */
    publishAsDockerFileWithConfigure(configure: (obj: ContainerResource) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._publishAsDockerFileWithConfigureInternal(configure));
    }

    /** @internal */
    private async _withExecutableCommandInternal(command: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, command };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withExecutableCommand',
            rpcArgs
    ...
    /** Sets the executable command */
    withExecutableCommand(command: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withExecutableCommandInternal(command));
    }

    /** @internal */
    private async _withWorkingDirectoryInternal(workingDirectory: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, workingDirectory };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withWorkingDirectory',
            rpcArgs
    ...
    /** Sets the executable working directory */
    withWorkingDirectory(workingDirectory: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withWorkingDirectoryInternal(workingDirectory));
    }

    /** @internal */
    private async _withMcpServerInternal(path?: string, endpointName?: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (path !== undefined) rpcArgs.path = path;
        if (endpointName !== undefined) rpcArgs.endpointName = endpointName;
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
    ...
    /** Configures an MCP server endpoint on the resource */
    withMcpServer(options?: WithMcpServerOptions): UvicornAppResourcePromise {
        const path = options?.path;
        const endpointName = options?.endpointName;
        return new UvicornAppResourcePromise(this._withMcpServerInternal(path, endpointName));
    }

    /** @internal */
    private async _withOtlpExporterInternal(): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withOtlpExporter',
            rpcArgs
    ...
    /** Configures OTLP telemetry export */
    withOtlpExporter(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withOtlpExporterInternal());
    }

    /** @internal */
    private async _withOtlpExporterProtocolInternal(protocol: OtlpProtocol): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, protocol };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withOtlpExporterProtocol',
            rpcArgs
    ...
    /** Configures OTLP telemetry export with specific protocol */
    withOtlpExporterProtocol(protocol: OtlpProtocol): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withOtlpExporterProtocolInternal(protocol));
    }

    /** @internal */
    private async _withRequiredCommandInternal(command: string, helpLink?: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, command };
        if (helpLink !== undefined) rpcArgs.helpLink = helpLink;
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withRequiredCommand',
    ...
    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): UvicornAppResourcePromise {
        const helpLink = options?.helpLink;
        return new UvicornAppResourcePromise(this._withRequiredCommandInternal(command, helpLink));
    }

    /** @internal */
    private async _withEnvironmentInternal(name: string, value: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withEnvironment',
            rpcArgs
    ...
    /** Sets an environment variable */
    withEnvironment(name: string, value: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withEnvironmentInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentExpressionInternal(name: string, value: ReferenceExpression): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withEnvironmentExpression',
            rpcArgs
    ...
    /** Adds an environment variable with a reference expression */
    withEnvironmentExpression(name: string, value: ReferenceExpression): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withEnvironmentExpressionInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentCallbackInternal(callback: (obj: EnvironmentCallbackContext) => Promise<void>): Promise<UvicornAppResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as EnvironmentCallbackContextHandle;
            const obj = new EnvironmentCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Sets environment variables via callback */
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withEnvironmentCallbackInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentCallbackAsyncInternal(callback: (arg: EnvironmentCallbackContext) => Promise<void>): Promise<UvicornAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as EnvironmentCallbackContextHandle;
            const arg = new EnvironmentCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Sets environment variables via async callback */
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withEnvironmentCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentEndpointInternal(name: string, endpointReference: EndpointReference): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, endpointReference };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withEnvironmentEndpoint',
            rpcArgs
    ...
    /** Sets an environment variable from an endpoint reference */
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withEnvironmentEndpointInternal(name, endpointReference));
    }

    /** @internal */
    private async _withEnvironmentParameterInternal(name: string, parameter: ParameterResource): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, parameter };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withEnvironmentParameter',
            rpcArgs
    ...
    /** Sets an environment variable from a parameter resource */
    withEnvironmentParameter(name: string, parameter: ParameterResource): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withEnvironmentParameterInternal(name, parameter));
    }

    /** @internal */
    private async _withEnvironmentConnectionStringInternal(envVarName: string, resource: ResourceBuilderBase): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, envVarName, resource };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withEnvironmentConnectionString',
            rpcArgs
    ...
    /** Sets an environment variable from a connection string resource */
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withEnvironmentConnectionStringInternal(envVarName, resource));
    }

    /** @internal */
    private async _withArgsInternal(args: string[]): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, args };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withArgs',
            rpcArgs
    ...
    /** Adds arguments */
    withArgs(args: string[]): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withArgsInternal(args));
    }

    /** @internal */
    private async _withArgsCallbackInternal(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): Promise<UvicornAppResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as CommandLineArgsCallbackContextHandle;
            const obj = new CommandLineArgsCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Sets command-line arguments via callback */
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withArgsCallbackInternal(callback));
    }

    /** @internal */
    private async _withArgsCallbackAsyncInternal(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): Promise<UvicornAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as CommandLineArgsCallbackContextHandle;
            const arg = new CommandLineArgsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Sets command-line arguments via async callback */
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withArgsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withReferenceInternal(source: ResourceBuilderBase, connectionName?: string, optional?: boolean): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        if (connectionName !== undefined) rpcArgs.connectionName = connectionName;
        if (optional !== undefined) rpcArgs.optional = optional;
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
    ...
    /** Adds a reference to another resource */
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): UvicornAppResourcePromise {
        const connectionName = options?.connectionName;
        const optional = options?.optional;
        return new UvicornAppResourcePromise(this._withReferenceInternal(source, connectionName, optional));
    }

    /** @internal */
    private async _withServiceReferenceInternal(source: ResourceBuilderBase): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withServiceReference',
            rpcArgs
    ...
    withServiceReference(source: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withServiceReferenceInternal(source));
    }

    /** @internal */
    private async _withServiceReferenceNamedInternal(source: ResourceBuilderBase, name: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source, name };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withServiceReferenceNamed',
            rpcArgs
    ...
    /** Adds a named service discovery reference */
    withServiceReferenceNamed(source: ResourceBuilderBase, name: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withServiceReferenceNamedInternal(source, name));
    }

    /** @internal */
    private async _withReferenceUriInternal(name: string, uri: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, uri };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withReferenceUri',
            rpcArgs
    ...
    /** Adds a reference to a URI */
    withReferenceUri(name: string, uri: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withReferenceUriInternal(name, uri));
    }

    /** @internal */
    private async _withReferenceExternalServiceInternal(externalService: ExternalServiceResource): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, externalService };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withReferenceExternalService',
            rpcArgs
    ...
    /** Adds a reference to an external service */
    withReferenceExternalService(externalService: ExternalServiceResource): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withReferenceExternalServiceInternal(externalService));
    }

    /** @internal */
    private async _withReferenceEndpointInternal(endpointReference: EndpointReference): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, endpointReference };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withReferenceEndpoint',
            rpcArgs
    ...
    /** Adds a reference to an endpoint */
    withReferenceEndpoint(endpointReference: EndpointReference): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withReferenceEndpointInternal(endpointReference));
    }

    /** @internal */
    private async _withEndpointInternal(port?: number, targetPort?: number, scheme?: string, name?: string, env?: string, isProxied?: boolean, isExternal?: boolean, protocol?: ProtocolType): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (scheme !== undefined) rpcArgs.scheme = scheme;
    ...
    /** Adds a network endpoint */
    withEndpoint(options?: WithEndpointOptions): UvicornAppResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const scheme = options?.scheme;
        const name = options?.name;
        ...
        const isExternal = options?.isExternal;
        const protocol = options?.protocol;
        return new UvicornAppResourcePromise(this._withEndpointInternal(port, targetPort, scheme, name, env, isProxied, isExternal, protocol));
    }

    /** @internal */
    private async _withHttpEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    /** Adds an HTTP endpoint */
    withHttpEndpoint(options?: WithHttpEndpointOptions): UvicornAppResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new UvicornAppResourcePromise(this._withHttpEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withHttpsEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    /** Adds an HTTPS endpoint */
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): UvicornAppResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new UvicornAppResourcePromise(this._withHttpsEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withExternalHttpEndpointsInternal(): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withExternalHttpEndpoints',
            rpcArgs
    ...
    /** Makes HTTP endpoints externally accessible */
    withExternalHttpEndpoints(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withExternalHttpEndpointsInternal());
    }
    ...
    /** @internal */
    private async _asHttp2ServiceInternal(): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/asHttp2Service',
            rpcArgs
    ...
    /** Configures resource for HTTP/2 */
    asHttp2Service(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._asHttp2ServiceInternal());
    }

    /** @internal */
    private async _withUrlsCallbackInternal(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): Promise<UvicornAppResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as ResourceUrlsCallbackContextHandle;
            const obj = new ResourceUrlsCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withUrlsCallbackInternal(callback));
    }

    /** @internal */
    private async _withUrlsCallbackAsyncInternal(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): Promise<UvicornAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ResourceUrlsCallbackContextHandle;
            const arg = new ResourceUrlsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withUrlsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withUrlInternal(url: string, displayText?: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withUrl',
    ...
    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): UvicornAppResourcePromise {
        const displayText = options?.displayText;
        return new UvicornAppResourcePromise(this._withUrlInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlExpressionInternal(url: ReferenceExpression, displayText?: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withUrlExpression',
    ...
    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): UvicornAppResourcePromise {
        const displayText = options?.displayText;
        return new UvicornAppResourcePromise(this._withUrlExpressionInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlForEndpointInternal(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): Promise<UvicornAppResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const obj = wrapIfHandle(objData) as ResourceUrlAnnotation;
            await callback(obj);
        });
    ...
    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withUrlForEndpointInternal(endpointName, callback));
    }

    /** @internal */
    private async _withUrlForEndpointFactoryInternal(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): Promise<UvicornAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as EndpointReferenceHandle;
            const arg = new EndpointReference(argHandle, this._client);
            return await callback(arg);
    ...
    /** Adds a URL for a specific endpoint via factory callback */
    withUrlForEndpointFactory(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withUrlForEndpointFactoryInternal(endpointName, callback));
    }
    ...
    /** @internal */
    private async _excludeFromManifestInternal(): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/excludeFromManifest',
            rpcArgs
    ...
    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._excludeFromManifestInternal());
    }
    ...
    waitFor(dependency: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._waitForInternal(dependency));
    }

    /** @internal */
    private async _waitForWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency, waitBehavior };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/waitForWithBehavior',
            rpcArgs
    ...
    /** Waits for another resource with specific behavior */
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._waitForWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _waitForStartInternal(dependency: ResourceBuilderBase): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/waitForStart',
            rpcArgs
    ...
    /** Waits for another resource to start */
    waitForStart(dependency: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._waitForStartInternal(dependency));
    }

    /** @internal */
    private async _waitForStartWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency, waitBehavior };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/waitForStartWithBehavior',
            rpcArgs
    ...
    /** Waits for another resource to start with specific behavior */
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._waitForStartWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _withExplicitStartInternal(): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withExplicitStart',
            rpcArgs
    ...
    /** Prevents resource from starting automatically */
    withExplicitStart(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withExplicitStartInternal());
    }

    /** @internal */
    private async _waitForCompletionInternal(dependency: ResourceBuilderBase, exitCode?: number): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        if (exitCode !== undefined) rpcArgs.exitCode = exitCode;
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/waitForCompletion',
    ...
    /** Waits for resource completion */
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): UvicornAppResourcePromise {
        const exitCode = options?.exitCode;
        return new UvicornAppResourcePromise(this._waitForCompletionInternal(dependency, exitCode));
    }

    /** @internal */
    private async _withHealthCheckInternal(key: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, key };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withHealthCheck',
            rpcArgs
    ...
    /** Adds a health check by key */
    withHealthCheck(key: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withHealthCheckInternal(key));
    }

    /** @internal */
    private async _withHttpHealthCheckInternal(path?: string, statusCode?: number, endpointName?: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (path !== undefined) rpcArgs.path = path;
        if (statusCode !== undefined) rpcArgs.statusCode = statusCode;
        if (endpointName !== undefined) rpcArgs.endpointName = endpointName;
    ...
    /** Adds an HTTP health check */
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): UvicornAppResourcePromise {
        const path = options?.path;
        const statusCode = options?.statusCode;
        const endpointName = options?.endpointName;
        return new UvicornAppResourcePromise(this._withHttpHealthCheckInternal(path, statusCode, endpointName));
    }

    /** @internal */
    private async _withCommandInternal(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, commandOptions?: CommandOptions): Promise<UvicornAppResource> {
        const executeCommandId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ExecuteCommandContextHandle;
            const arg = new ExecuteCommandContext(argHandle, this._client);
            return await executeCommand(arg);
    ...
    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): UvicornAppResourcePromise {
        const commandOptions = options?.commandOptions;
        return new UvicornAppResourcePromise(this._withCommandInternal(name, displayName, executeCommand, commandOptions));
    }

    /** @internal */
    private async _withDeveloperCertificateTrustInternal(trust: boolean): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, trust };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withDeveloperCertificateTrust',
            rpcArgs
    ...
    /** Configures developer certificate trust */
    withDeveloperCertificateTrust(trust: boolean): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withDeveloperCertificateTrustInternal(trust));
    }

    /** @internal */
    private async _withCertificateTrustScopeInternal(scope: CertificateTrustScope): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, scope };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withCertificateTrustScope',
            rpcArgs
    ...
    /** Sets the certificate trust scope */
    withCertificateTrustScope(scope: CertificateTrustScope): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withCertificateTrustScopeInternal(scope));
    }

    /** @internal */
    private async _withHttpsDeveloperCertificateInternal(password?: ParameterResource): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (password !== undefined) rpcArgs.password = password;
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withHttpsDeveloperCertificate',
    ...
    /** Configures HTTPS with a developer certificate */
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): UvicornAppResourcePromise {
        const password = options?.password;
        return new UvicornAppResourcePromise(this._withHttpsDeveloperCertificateInternal(password));
    }

    /** @internal */
    private async _withoutHttpsCertificateInternal(): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withoutHttpsCertificate',
            rpcArgs
    ...
    /** Removes HTTPS certificate configuration */
    withoutHttpsCertificate(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withoutHttpsCertificateInternal());
    }

    /** @internal */
    private async _withParentRelationshipInternal(parent: ResourceBuilderBase): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, parent };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withParentRelationship',
            rpcArgs
    ...
    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withParentRelationshipInternal(parent));
    }

    /** @internal */
    private async _withChildRelationshipInternal(child: ResourceBuilderBase): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, child };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withChildRelationship',
            rpcArgs
    ...
    /** @internal */
    private async _withIconNameInternal(iconName: string, iconVariant?: IconVariant): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, iconName };
        if (iconVariant !== undefined) rpcArgs.iconVariant = iconVariant;
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withIconName',
    ...
    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): UvicornAppResourcePromise {
        const iconVariant = options?.iconVariant;
        return new UvicornAppResourcePromise(this._withIconNameInternal(iconName, iconVariant));
    }

    /** @internal */
    private async _withHttpProbeInternal(probeType: ProbeType, path?: string, initialDelaySeconds?: number, periodSeconds?: number, timeoutSeconds?: number, failureThreshold?: number, successThreshold?: number, endpointName?: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, probeType };
        if (path !== undefined) rpcArgs.path = path;
        if (initialDelaySeconds !== undefined) rpcArgs.initialDelaySeconds = initialDelaySeconds;
        if (periodSeconds !== undefined) rpcArgs.periodSeconds = periodSeconds;
    ...
    /** Adds an HTTP health probe to the resource */
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): UvicornAppResourcePromise {
        const path = options?.path;
        const initialDelaySeconds = options?.initialDelaySeconds;
        const periodSeconds = options?.periodSeconds;
        const timeoutSeconds = options?.timeoutSeconds;
        ...
        const successThreshold = options?.successThreshold;
        const endpointName = options?.endpointName;
        return new UvicornAppResourcePromise(this._withHttpProbeInternal(probeType, path, initialDelaySeconds, periodSeconds, timeoutSeconds, failureThreshold, successThreshold, endpointName));
    }

    /** @internal */
    private async _excludeFromMcpInternal(): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/excludeFromMcp',
            rpcArgs
    ...
    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._excludeFromMcpInternal());
    }

    /** @internal */
    private async _withRemoteImageNameInternal(remoteImageName: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageName };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withRemoteImageName',
            rpcArgs
    ...
    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withRemoteImageNameInternal(remoteImageName));
    }

    /** @internal */
    private async _withRemoteImageTagInternal(remoteImageTag: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageTag };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
            'Aspire.Hosting/withRemoteImageTag',
            rpcArgs
    ...
    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withRemoteImageTagInternal(remoteImageTag));
    }

    /** @internal */
    private async _withPipelineStepFactoryInternal(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, dependsOn?: string[], requiredBy?: string[], tags?: string[], description?: string): Promise<UvicornAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineStepContextHandle;
            const arg = new PipelineStepContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): UvicornAppResourcePromise {
        const dependsOn = options?.dependsOn;
        const requiredBy = options?.requiredBy;
        const tags = options?.tags;
        const description = options?.description;
        return new UvicornAppResourcePromise(this._withPipelineStepFactoryInternal(stepName, callback, dependsOn, requiredBy, tags, description));
    }

    /** @internal */
    private async _withPipelineConfigurationAsyncInternal(callback: (arg: PipelineConfigurationContext) => Promise<void>): Promise<UvicornAppResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineConfigurationContextHandle;
            const arg = new PipelineConfigurationContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withPipelineConfigurationAsyncInternal(callback));
    }

    /** @internal */
    private async _withPipelineConfigurationInternal(callback: (obj: PipelineConfigurationContext) => Promise<void>): Promise<UvicornAppResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as PipelineConfigurationContextHandle;
            const obj = new PipelineConfigurationContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withPipelineConfigurationInternal(callback));
    }
    ...
    /** @internal */
    private async _withVirtualEnvironmentInternal(virtualEnvironmentPath: string, createIfNotExists?: boolean): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, virtualEnvironmentPath };
    ...
    withVirtualEnvironment(virtualEnvironmentPath: string, options?: WithVirtualEnvironmentOptions): UvicornAppResourcePromise {
        const createIfNotExists = options?.createIfNotExists;
        return new UvicornAppResourcePromise(this._withVirtualEnvironmentInternal(virtualEnvironmentPath, createIfNotExists));
    }

    /** @internal */
    private async _withDebuggingInternal(): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
    ...
    withDebugging(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withDebuggingInternal());
    }

    /** @internal */
    private async _withEntrypointInternal(entrypointType: EntrypointType, entrypoint: string): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, entrypointType, entrypoint };
        const result = await this._client.invokeCapability<UvicornAppResourceHandle>(
    ...
    withEntrypoint(entrypointType: EntrypointType, entrypoint: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._withEntrypointInternal(entrypointType, entrypoint));
    }

    /** @internal */
    private async _withPipInternal(install?: boolean, installArgs?: string[]): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
    ...
    withPip(options?: WithPipOptions): UvicornAppResourcePromise {
        const install = options?.install;
        const installArgs = options?.installArgs;
        return new UvicornAppResourcePromise(this._withPipInternal(install, installArgs));
    }

    /** @internal */
    private async _withUvInternal(install?: boolean, args?: string[]): Promise<UvicornAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (install !== undefined) rpcArgs.install = install;
    ...
    withUv(options?: WithUvOptions): UvicornAppResourcePromise {
        ...
        return new UvicornAppResourcePromise(this._withUvInternal(install, args));
    }

}

/**
 * Thenable wrapper for UvicornAppResource that enables fluent chaining.
 * @example
 * await builder.addSomething().withX().withY();
 */
export class UvicornAppResourcePromise implements PromiseLike<UvicornAppResource> {
    constructor(private _promise: Promise<UvicornAppResource>) {}

    then<TResult1 = UvicornAppResource, TResult2 = never>(
        onfulfilled?: ((value: UvicornAppResource) => TResult1 | PromiseLike<TResult1>) | null,
        onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null
    ): PromiseLike<TResult1 | TResult2> {
        return this._promise.then(onfulfilled, onrejected);
    }
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withContainerRegistry(registry)));
    }

    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withDockerfileBaseImage(options)));
    }

    /** Publishes the executable as a Docker container */
    publishAsDockerFile(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.publishAsDockerFile()));
    }

    /** Publishes an executable as a Docker file with optional container configuration */
    publishAsDockerFileWithConfigure(configure: (obj: ContainerResource) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.publishAsDockerFileWithConfigure(configure)));
    }

    /** Sets the executable command */
    withExecutableCommand(command: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withExecutableCommand(command)));
    }

    /** Sets the executable working directory */
    withWorkingDirectory(workingDirectory: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withWorkingDirectory(workingDirectory)));
    }

    /** Configures an MCP server endpoint on the resource */
    withMcpServer(options?: WithMcpServerOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withMcpServer(options)));
    }

    /** Configures OTLP telemetry export */
    withOtlpExporter(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withOtlpExporter()));
    }

    /** Configures OTLP telemetry export with specific protocol */
    withOtlpExporterProtocol(protocol: OtlpProtocol): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withOtlpExporterProtocol(protocol)));
    }

    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withRequiredCommand(command, options)));
    }

    /** Sets an environment variable */
    withEnvironment(name: string, value: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withEnvironment(name, value)));
    }

    /** Adds an environment variable with a reference expression */
    withEnvironmentExpression(name: string, value: ReferenceExpression): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withEnvironmentExpression(name, value)));
    }

    /** Sets environment variables via callback */
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withEnvironmentCallback(callback)));
    }

    /** Sets environment variables via async callback */
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withEnvironmentCallbackAsync(callback)));
    }

    /** Sets an environment variable from an endpoint reference */
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withEnvironmentEndpoint(name, endpointReference)));
    }

    /** Sets an environment variable from a parameter resource */
    withEnvironmentParameter(name: string, parameter: ParameterResource): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withEnvironmentParameter(name, parameter)));
    }

    /** Sets an environment variable from a connection string resource */
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withEnvironmentConnectionString(envVarName, resource)));
    }

    /** Adds arguments */
    withArgs(args: string[]): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withArgs(args)));
    }

    /** Sets command-line arguments via callback */
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withArgsCallback(callback)));
    }

    /** Sets command-line arguments via async callback */
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withArgsCallbackAsync(callback)));
    }

    /** Adds a reference to another resource */
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withReference(source, options)));
    }

    /** Adds a service discovery reference to another resource */
    withServiceReference(source: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withServiceReference(source)));
    }

    /** Adds a named service discovery reference */
    withServiceReferenceNamed(source: ResourceBuilderBase, name: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withServiceReferenceNamed(source, name)));
    }

    /** Adds a reference to a URI */
    withReferenceUri(name: string, uri: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withReferenceUri(name, uri)));
    }

    /** Adds a reference to an external service */
    withReferenceExternalService(externalService: ExternalServiceResource): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withReferenceExternalService(externalService)));
    }

    /** Adds a reference to an endpoint */
    withReferenceEndpoint(endpointReference: EndpointReference): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withReferenceEndpoint(endpointReference)));
    }

    /** Adds a network endpoint */
    withEndpoint(options?: WithEndpointOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withEndpoint(options)));
    }

    /** Adds an HTTP endpoint */
    withHttpEndpoint(options?: WithHttpEndpointOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withHttpEndpoint(options)));
    }

    /** Adds an HTTPS endpoint */
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withHttpsEndpoint(options)));
    }

    /** Makes HTTP endpoints externally accessible */
    withExternalHttpEndpoints(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withExternalHttpEndpoints()));
    }
    ...
    /** Configures resource for HTTP/2 */
    asHttp2Service(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.asHttp2Service()));
    }

    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withUrlsCallback(callback)));
    }

    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withUrlsCallbackAsync(callback)));
    }

    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withUrl(url, options)));
    }

    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withUrlExpression(url, options)));
    }

    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withUrlForEndpoint(endpointName, callback)));
    }
    ...
    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.excludeFromManifest()));
    }
    ...
    /** Waits for another resource with specific behavior */
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.waitForWithBehavior(dependency, waitBehavior)));
    }

    /** Waits for another resource to start */
    waitForStart(dependency: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.waitForStart(dependency)));
    }

    /** Waits for another resource to start with specific behavior */
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.waitForStartWithBehavior(dependency, waitBehavior)));
    }

    /** Prevents resource from starting automatically */
    withExplicitStart(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withExplicitStart()));
    }

    /** Waits for resource completion */
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.waitForCompletion(dependency, options)));
    }

    /** Adds a health check by key */
    withHealthCheck(key: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withHealthCheck(key)));
    }

    /** Adds an HTTP health check */
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withHttpHealthCheck(options)));
    }

    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withCommand(name, displayName, executeCommand, options)));
    }

    /** Configures developer certificate trust */
    withDeveloperCertificateTrust(trust: boolean): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withDeveloperCertificateTrust(trust)));
    }
    ...
    /** Configures HTTPS with a developer certificate */
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withHttpsDeveloperCertificate(options)));
    }

    /** Removes HTTPS certificate configuration */
    withoutHttpsCertificate(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withoutHttpsCertificate()));
    }

    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withParentRelationship(parent)));
    }

    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withChildRelationship(child)));
    }

    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withIconName(iconName, options)));
    }

    /** Adds an HTTP health probe to the resource */
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withHttpProbe(probeType, options)));
    }

    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.excludeFromMcp()));
    }

    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withRemoteImageName(remoteImageName)));
    }

    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withRemoteImageTag(remoteImageTag)));
    }

    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withPipelineStepFactory(stepName, callback, options)));
    }

    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withPipelineConfigurationAsync(callback)));
    }

    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withPipelineConfiguration(callback)));
    }
    ...
    /** Configures the virtual environment for a Python application */
    withVirtualEnvironment(virtualEnvironmentPath: string, options?: WithVirtualEnvironmentOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withVirtualEnvironment(virtualEnvironmentPath, options)));
    }

    /** Enables debugging support for a Python application */
    withDebugging(): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withDebugging()));
    }

    /** Configures the entrypoint for a Python application */
    withEntrypoint(entrypointType: EntrypointType, entrypoint: string): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withEntrypoint(entrypointType, entrypoint)));
    }

    /** Configures pip package installation for a Python application */
    withPip(options?: WithPipOptions): UvicornAppResourcePromise {
        return new UvicornAppResourcePromise(this._promise.then(obj => obj.withPip(options)));
    }
