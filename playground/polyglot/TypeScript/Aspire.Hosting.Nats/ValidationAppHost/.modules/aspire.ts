...
/** Handle to NatsServerResource */
type NatsServerResourceHandle = Handle<'Aspire.Hosting.Nats/Aspire.Hosting.ApplicationModel.NatsServerResource'>;
...
export interface AddNatsOptions {
    port?: number;
...
export class DistributedApplicationBuilder {
    ...
    /** @internal */
    async _addNatsInternal(name: string, port?: number, userName?: ParameterResource, password?: ParameterResource): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name };
        ...
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting.Nats/addNats',
            rpcArgs
        );
        return new NatsServerResource(result, this._client);
    }

    addNats(name: string, options?: AddNatsOptions): NatsServerResourcePromise {
        const port = options?.port;
        const userName = options?.userName;
        const password = options?.password;
        return new NatsServerResourcePromise(this._addNatsInternal(name, port, userName, password));
    }
...
export class DistributedApplicationBuilderPromise implements PromiseLike<DistributedApplicationBuilder> {
    ...
    /** Adds a NATS server resource to the application model. */
    addNats(name: string, options?: AddNatsOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.addNats(name, options)));
    }
...
export class NatsServerResource extends ResourceBuilderBase<NatsServerResourceHandle> {
    constructor(handle: NatsServerResourceHandle, client: AspireClientRpc) {
        super(handle, client);
    }
    ...
    /** @internal */
    private async _withContainerRegistryInternal(registry: ResourceBuilderBase): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withContainerRegistry',
            rpcArgs
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withContainerRegistryInternal(registry));
    }
    ...
    /** Adds a bind mount */
    withBindMount(source: string, target: string, options?: WithBindMountOptions): NatsServerResourcePromise {
        const isReadOnly = options?.isReadOnly;
        return new NatsServerResourcePromise(this._withBindMountInternal(source, target, isReadOnly));
    }

    /** @internal */
    private async _withEntrypointInternal(entrypoint: string): Promise<NatsServerResource> {
    ...
    withEntrypoint(entrypoint: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withEntrypointInternal(entrypoint));
    }

    /** @internal */
    private async _withImageTagInternal(tag: string): Promise<NatsServerResource> {
    ...
    withImageTag(tag: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withImageTagInternal(tag));
    }

    /** @internal */
    private async _withImageRegistryInternal(registry: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
    ...
    withImageRegistry(registry: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withImageRegistryInternal(registry));
    }

    /** @internal */
    private async _withImageInternal(image: string, tag?: string): Promise<NatsServerResource> {
    ...
    withImage(image: string, options?: WithImageOptions): NatsServerResourcePromise {
        const tag = options?.tag;
        return new NatsServerResourcePromise(this._withImageInternal(image, tag));
    }

    /** @internal */
    private async _withImageSHA256Internal(sha256: string): Promise<NatsServerResource> {
    ...
    withImageSHA256(sha256: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withImageSHA256Internal(sha256));
    }

    /** @internal */
    private async _withContainerRuntimeArgsInternal(args: string[]): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, args };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
    ...
    withContainerRuntimeArgs(args: string[]): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withContainerRuntimeArgsInternal(args));
    }

    /** @internal */
    private async _withLifetimeInternal(lifetime: ContainerLifetime): Promise<NatsServerResource> {
    ...
    withLifetime(lifetime: ContainerLifetime): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withLifetimeInternal(lifetime));
    }

    /** @internal */
    private async _withImagePullPolicyInternal(pullPolicy: ImagePullPolicy): Promise<NatsServerResource> {
    ...
    withImagePullPolicy(pullPolicy: ImagePullPolicy): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withImagePullPolicyInternal(pullPolicy));
    }

    /** @internal */
    private async _publishAsContainerInternal(): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
    ...
    publishAsContainer(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._publishAsContainerInternal());
    }

    /** @internal */
    private async _withDockerfileInternal(contextPath: string, dockerfilePath?: string, stage?: string): Promise<NatsServerResource> {
    ...
    withDockerfile(contextPath: string, options?: WithDockerfileOptions): NatsServerResourcePromise {
        const dockerfilePath = options?.dockerfilePath;
        const stage = options?.stage;
        return new NatsServerResourcePromise(this._withDockerfileInternal(contextPath, dockerfilePath, stage));
    }

    /** @internal */
    private async _withContainerNameInternal(name: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
    ...
    withContainerName(name: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withContainerNameInternal(name));
    }

    /** @internal */
    private async _withBuildArgInternal(name: string, value: ParameterResource): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
    ...
    withBuildArg(name: string, value: ParameterResource): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withBuildArgInternal(name, value));
    }

    /** @internal */
    private async _withBuildSecretInternal(name: string, value: ParameterResource): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
    ...
    withBuildSecret(name: string, value: ParameterResource): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withBuildSecretInternal(name, value));
    }

    /** @internal */
    private async _withEndpointProxySupportInternal(proxyEnabled: boolean): Promise<NatsServerResource> {
    ...
    withEndpointProxySupport(proxyEnabled: boolean): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withEndpointProxySupportInternal(proxyEnabled));
    }

    /** @internal */
    private async _withDockerfileBaseImageInternal(buildImage?: string, runtimeImage?: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (buildImage !== undefined) rpcArgs.buildImage = buildImage;
        if (runtimeImage !== undefined) rpcArgs.runtimeImage = runtimeImage;
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
    ...
    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): NatsServerResourcePromise {
        const buildImage = options?.buildImage;
        const runtimeImage = options?.runtimeImage;
        return new NatsServerResourcePromise(this._withDockerfileBaseImageInternal(buildImage, runtimeImage));
    }

    /** @internal */
    private async _withContainerNetworkAliasInternal(alias: string): Promise<NatsServerResource> {
    ...
    withContainerNetworkAlias(alias: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withContainerNetworkAliasInternal(alias));
    }

    /** @internal */
    private async _withMcpServerInternal(path?: string, endpointName?: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (path !== undefined) rpcArgs.path = path;
        if (endpointName !== undefined) rpcArgs.endpointName = endpointName;
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
    ...
    /** Configures an MCP server endpoint on the resource */
    withMcpServer(options?: WithMcpServerOptions): NatsServerResourcePromise {
        const path = options?.path;
        const endpointName = options?.endpointName;
        return new NatsServerResourcePromise(this._withMcpServerInternal(path, endpointName));
    }

    /** @internal */
    private async _withOtlpExporterInternal(): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withOtlpExporter',
            rpcArgs
    ...
    /** Configures OTLP telemetry export */
    withOtlpExporter(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withOtlpExporterInternal());
    }

    /** @internal */
    private async _withOtlpExporterProtocolInternal(protocol: OtlpProtocol): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, protocol };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withOtlpExporterProtocol',
            rpcArgs
    ...
    /** Configures OTLP telemetry export with specific protocol */
    withOtlpExporterProtocol(protocol: OtlpProtocol): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withOtlpExporterProtocolInternal(protocol));
    }

    /** @internal */
    private async _publishAsConnectionStringInternal(): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
    ...
    publishAsConnectionString(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._publishAsConnectionStringInternal());
    }

    /** @internal */
    private async _withRequiredCommandInternal(command: string, helpLink?: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, command };
        if (helpLink !== undefined) rpcArgs.helpLink = helpLink;
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withRequiredCommand',
    ...
    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): NatsServerResourcePromise {
        const helpLink = options?.helpLink;
        return new NatsServerResourcePromise(this._withRequiredCommandInternal(command, helpLink));
    }

    /** @internal */
    private async _withEnvironmentInternal(name: string, value: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withEnvironment',
            rpcArgs
    ...
    /** Sets an environment variable */
    withEnvironment(name: string, value: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withEnvironmentInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentExpressionInternal(name: string, value: ReferenceExpression): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withEnvironmentExpression',
            rpcArgs
    ...
    /** Adds an environment variable with a reference expression */
    withEnvironmentExpression(name: string, value: ReferenceExpression): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withEnvironmentExpressionInternal(name, value));
    }

    /** @internal */
    private async _withEnvironmentCallbackInternal(callback: (obj: EnvironmentCallbackContext) => Promise<void>): Promise<NatsServerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as EnvironmentCallbackContextHandle;
            const obj = new EnvironmentCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Sets environment variables via callback */
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withEnvironmentCallbackInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentCallbackAsyncInternal(callback: (arg: EnvironmentCallbackContext) => Promise<void>): Promise<NatsServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as EnvironmentCallbackContextHandle;
            const arg = new EnvironmentCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Sets environment variables via async callback */
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withEnvironmentCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withEnvironmentEndpointInternal(name: string, endpointReference: EndpointReference): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, endpointReference };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withEnvironmentEndpoint',
            rpcArgs
    ...
    /** Sets an environment variable from an endpoint reference */
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withEnvironmentEndpointInternal(name, endpointReference));
    }

    /** @internal */
    private async _withEnvironmentParameterInternal(name: string, parameter: ParameterResource): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, parameter };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withEnvironmentParameter',
            rpcArgs
    ...
    /** Sets an environment variable from a parameter resource */
    withEnvironmentParameter(name: string, parameter: ParameterResource): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withEnvironmentParameterInternal(name, parameter));
    }

    /** @internal */
    private async _withEnvironmentConnectionStringInternal(envVarName: string, resource: ResourceBuilderBase): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, envVarName, resource };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withEnvironmentConnectionString',
            rpcArgs
    ...
    /** Sets an environment variable from a connection string resource */
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withEnvironmentConnectionStringInternal(envVarName, resource));
    }

    /** @internal */
    private async _withConnectionPropertyInternal(name: string, value: ReferenceExpression): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withConnectionProperty',
            rpcArgs
    ...
    /** Adds a connection property with a reference expression */
    withConnectionProperty(name: string, value: ReferenceExpression): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withConnectionPropertyInternal(name, value));
    }

    /** @internal */
    private async _withConnectionPropertyValueInternal(name: string, value: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withConnectionPropertyValue',
            rpcArgs
    ...
    /** Adds a connection property with a string value */
    withConnectionPropertyValue(name: string, value: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withConnectionPropertyValueInternal(name, value));
    }

    /** @internal */
    private async _withArgsInternal(args: string[]): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, args };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withArgs',
            rpcArgs
    ...
    /** Adds arguments */
    withArgs(args: string[]): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withArgsInternal(args));
    }

    /** @internal */
    private async _withArgsCallbackInternal(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): Promise<NatsServerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as CommandLineArgsCallbackContextHandle;
            const obj = new CommandLineArgsCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Sets command-line arguments via callback */
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withArgsCallbackInternal(callback));
    }

    /** @internal */
    private async _withArgsCallbackAsyncInternal(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): Promise<NatsServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as CommandLineArgsCallbackContextHandle;
            const arg = new CommandLineArgsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Sets command-line arguments via async callback */
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withArgsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withReferenceInternal(source: ResourceBuilderBase, connectionName?: string, optional?: boolean): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        if (connectionName !== undefined) rpcArgs.connectionName = connectionName;
        if (optional !== undefined) rpcArgs.optional = optional;
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
    ...
    /** Adds a reference to another resource */
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): NatsServerResourcePromise {
        const connectionName = options?.connectionName;
        const optional = options?.optional;
        return new NatsServerResourcePromise(this._withReferenceInternal(source, connectionName, optional));
    }

    /** @internal */
    private async _withServiceReferenceInternal(source: ResourceBuilderBase): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withServiceReference',
            rpcArgs
    ...
    /** Adds a service discovery reference to another resource */
    withServiceReference(source: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withServiceReferenceInternal(source));
    }

    /** @internal */
    private async _withServiceReferenceNamedInternal(source: ResourceBuilderBase, name: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source, name };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withServiceReferenceNamed',
            rpcArgs
    ...
    /** Adds a named service discovery reference */
    withServiceReferenceNamed(source: ResourceBuilderBase, name: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withServiceReferenceNamedInternal(source, name));
    }

    /** @internal */
    private async _withReferenceUriInternal(name: string, uri: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, uri };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withReferenceUri',
            rpcArgs
    ...
    /** Adds a reference to a URI */
    withReferenceUri(name: string, uri: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withReferenceUriInternal(name, uri));
    }

    /** @internal */
    private async _withReferenceExternalServiceInternal(externalService: ExternalServiceResource): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, externalService };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withReferenceExternalService',
            rpcArgs
    ...
    /** Adds a reference to an external service */
    withReferenceExternalService(externalService: ExternalServiceResource): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withReferenceExternalServiceInternal(externalService));
    }

    /** @internal */
    private async _withReferenceEndpointInternal(endpointReference: EndpointReference): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, endpointReference };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withReferenceEndpoint',
            rpcArgs
    ...
    /** Adds a reference to an endpoint */
    withReferenceEndpoint(endpointReference: EndpointReference): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withReferenceEndpointInternal(endpointReference));
    }

    /** @internal */
    private async _withEndpointInternal(port?: number, targetPort?: number, scheme?: string, name?: string, env?: string, isProxied?: boolean, isExternal?: boolean, protocol?: ProtocolType): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (scheme !== undefined) rpcArgs.scheme = scheme;
    ...
    /** Adds a network endpoint */
    withEndpoint(options?: WithEndpointOptions): NatsServerResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const scheme = options?.scheme;
        const name = options?.name;
        ...
        const isExternal = options?.isExternal;
        const protocol = options?.protocol;
        return new NatsServerResourcePromise(this._withEndpointInternal(port, targetPort, scheme, name, env, isProxied, isExternal, protocol));
    }

    /** @internal */
    private async _withHttpEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    /** Adds an HTTP endpoint */
    withHttpEndpoint(options?: WithHttpEndpointOptions): NatsServerResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new NatsServerResourcePromise(this._withHttpEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withHttpsEndpointInternal(port?: number, targetPort?: number, name?: string, env?: string, isProxied?: boolean): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (port !== undefined) rpcArgs.port = port;
        if (targetPort !== undefined) rpcArgs.targetPort = targetPort;
        if (name !== undefined) rpcArgs.name = name;
    ...
    /** Adds an HTTPS endpoint */
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): NatsServerResourcePromise {
        const port = options?.port;
        const targetPort = options?.targetPort;
        const name = options?.name;
        const env = options?.env;
        const isProxied = options?.isProxied;
        return new NatsServerResourcePromise(this._withHttpsEndpointInternal(port, targetPort, name, env, isProxied));
    }

    /** @internal */
    private async _withExternalHttpEndpointsInternal(): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withExternalHttpEndpoints',
            rpcArgs
    ...
    /** Makes HTTP endpoints externally accessible */
    withExternalHttpEndpoints(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withExternalHttpEndpointsInternal());
    }
    ...
    /** @internal */
    private async _asHttp2ServiceInternal(): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/asHttp2Service',
            rpcArgs
    ...
    /** Configures resource for HTTP/2 */
    asHttp2Service(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._asHttp2ServiceInternal());
    }

    /** @internal */
    private async _withUrlsCallbackInternal(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): Promise<NatsServerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as ResourceUrlsCallbackContextHandle;
            const obj = new ResourceUrlsCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withUrlsCallbackInternal(callback));
    }

    /** @internal */
    private async _withUrlsCallbackAsyncInternal(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): Promise<NatsServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ResourceUrlsCallbackContextHandle;
            const arg = new ResourceUrlsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withUrlsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withUrlInternal(url: string, displayText?: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withUrl',
    ...
    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): NatsServerResourcePromise {
        const displayText = options?.displayText;
        return new NatsServerResourcePromise(this._withUrlInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlExpressionInternal(url: ReferenceExpression, displayText?: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withUrlExpression',
    ...
    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): NatsServerResourcePromise {
        const displayText = options?.displayText;
        return new NatsServerResourcePromise(this._withUrlExpressionInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlForEndpointInternal(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): Promise<NatsServerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const obj = wrapIfHandle(objData) as ResourceUrlAnnotation;
            await callback(obj);
        });
    ...
    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withUrlForEndpointInternal(endpointName, callback));
    }

    /** @internal */
    private async _withUrlForEndpointFactoryInternal(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): Promise<NatsServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as EndpointReferenceHandle;
            const arg = new EndpointReference(argHandle, this._client);
            return await callback(arg);
    ...
    /** Adds a URL for a specific endpoint via factory callback */
    withUrlForEndpointFactory(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withUrlForEndpointFactoryInternal(endpointName, callback));
    }

    /** @internal */
    private async _excludeFromManifestInternal(): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/excludeFromManifest',
            rpcArgs
    ...
    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._excludeFromManifestInternal());
    }

    /** @internal */
    private async _waitForInternal(dependency: ResourceBuilderBase): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/waitFor',
            rpcArgs
    ...
    /** Waits for another resource to be ready */
    waitFor(dependency: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._waitForInternal(dependency));
    }

    /** @internal */
    private async _waitForWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency, waitBehavior };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/waitForWithBehavior',
            rpcArgs
    ...
    /** Waits for another resource with specific behavior */
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._waitForWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _waitForStartInternal(dependency: ResourceBuilderBase): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/waitForStart',
            rpcArgs
    ...
    /** Waits for another resource to start */
    waitForStart(dependency: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._waitForStartInternal(dependency));
    }

    /** @internal */
    private async _waitForStartWithBehaviorInternal(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency, waitBehavior };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/waitForStartWithBehavior',
            rpcArgs
    ...
    /** Waits for another resource to start with specific behavior */
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._waitForStartWithBehaviorInternal(dependency, waitBehavior));
    }

    /** @internal */
    private async _withExplicitStartInternal(): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withExplicitStart',
            rpcArgs
    ...
    /** Prevents resource from starting automatically */
    withExplicitStart(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withExplicitStartInternal());
    }

    /** @internal */
    private async _waitForCompletionInternal(dependency: ResourceBuilderBase, exitCode?: number): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, dependency };
        if (exitCode !== undefined) rpcArgs.exitCode = exitCode;
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/waitForCompletion',
    ...
    /** Waits for resource completion */
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): NatsServerResourcePromise {
        const exitCode = options?.exitCode;
        return new NatsServerResourcePromise(this._waitForCompletionInternal(dependency, exitCode));
    }

    /** @internal */
    private async _withHealthCheckInternal(key: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, key };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withHealthCheck',
            rpcArgs
    ...
    /** Adds a health check by key */
    withHealthCheck(key: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withHealthCheckInternal(key));
    }

    /** @internal */
    private async _withHttpHealthCheckInternal(path?: string, statusCode?: number, endpointName?: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (path !== undefined) rpcArgs.path = path;
        if (statusCode !== undefined) rpcArgs.statusCode = statusCode;
        if (endpointName !== undefined) rpcArgs.endpointName = endpointName;
    ...
    /** Adds an HTTP health check */
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): NatsServerResourcePromise {
        const path = options?.path;
        const statusCode = options?.statusCode;
        const endpointName = options?.endpointName;
        return new NatsServerResourcePromise(this._withHttpHealthCheckInternal(path, statusCode, endpointName));
    }

    /** @internal */
    private async _withCommandInternal(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, commandOptions?: CommandOptions): Promise<NatsServerResource> {
        const executeCommandId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ExecuteCommandContextHandle;
            const arg = new ExecuteCommandContext(argHandle, this._client);
            return await executeCommand(arg);
    ...
    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): NatsServerResourcePromise {
        const commandOptions = options?.commandOptions;
        return new NatsServerResourcePromise(this._withCommandInternal(name, displayName, executeCommand, commandOptions));
    }

    /** @internal */
    private async _withDeveloperCertificateTrustInternal(trust: boolean): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, trust };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withDeveloperCertificateTrust',
            rpcArgs
    ...
    /** Configures developer certificate trust */
    withDeveloperCertificateTrust(trust: boolean): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withDeveloperCertificateTrustInternal(trust));
    }

    /** @internal */
    private async _withCertificateTrustScopeInternal(scope: CertificateTrustScope): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, scope };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withCertificateTrustScope',
            rpcArgs
    ...
    /** Sets the certificate trust scope */
    withCertificateTrustScope(scope: CertificateTrustScope): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withCertificateTrustScopeInternal(scope));
    }

    /** @internal */
    private async _withHttpsDeveloperCertificateInternal(password?: ParameterResource): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (password !== undefined) rpcArgs.password = password;
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withHttpsDeveloperCertificate',
    ...
    /** Configures HTTPS with a developer certificate */
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): NatsServerResourcePromise {
        const password = options?.password;
        return new NatsServerResourcePromise(this._withHttpsDeveloperCertificateInternal(password));
    }

    /** @internal */
    private async _withoutHttpsCertificateInternal(): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withoutHttpsCertificate',
            rpcArgs
    ...
    /** Removes HTTPS certificate configuration */
    withoutHttpsCertificate(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withoutHttpsCertificateInternal());
    }

    /** @internal */
    private async _withParentRelationshipInternal(parent: ResourceBuilderBase): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, parent };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withParentRelationship',
            rpcArgs
    ...
    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withParentRelationshipInternal(parent));
    }

    /** @internal */
    private async _withChildRelationshipInternal(child: ResourceBuilderBase): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, child };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withChildRelationship',
            rpcArgs
    ...
    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withChildRelationshipInternal(child));
    }

    /** @internal */
    private async _withIconNameInternal(iconName: string, iconVariant?: IconVariant): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, iconName };
        if (iconVariant !== undefined) rpcArgs.iconVariant = iconVariant;
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withIconName',
    ...
    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): NatsServerResourcePromise {
        const iconVariant = options?.iconVariant;
        return new NatsServerResourcePromise(this._withIconNameInternal(iconName, iconVariant));
    }

    /** @internal */
    private async _withHttpProbeInternal(probeType: ProbeType, path?: string, initialDelaySeconds?: number, periodSeconds?: number, timeoutSeconds?: number, failureThreshold?: number, successThreshold?: number, endpointName?: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, probeType };
        if (path !== undefined) rpcArgs.path = path;
        if (initialDelaySeconds !== undefined) rpcArgs.initialDelaySeconds = initialDelaySeconds;
        if (periodSeconds !== undefined) rpcArgs.periodSeconds = periodSeconds;
    ...
    /** Adds an HTTP health probe to the resource */
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): NatsServerResourcePromise {
        const path = options?.path;
        const initialDelaySeconds = options?.initialDelaySeconds;
        const periodSeconds = options?.periodSeconds;
        const timeoutSeconds = options?.timeoutSeconds;
        ...
        const successThreshold = options?.successThreshold;
        const endpointName = options?.endpointName;
        return new NatsServerResourcePromise(this._withHttpProbeInternal(probeType, path, initialDelaySeconds, periodSeconds, timeoutSeconds, failureThreshold, successThreshold, endpointName));
    }

    /** @internal */
    private async _excludeFromMcpInternal(): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/excludeFromMcp',
            rpcArgs
    ...
    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._excludeFromMcpInternal());
    }

    /** @internal */
    private async _withRemoteImageNameInternal(remoteImageName: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageName };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withRemoteImageName',
            rpcArgs
    ...
    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withRemoteImageNameInternal(remoteImageName));
    }

    /** @internal */
    private async _withRemoteImageTagInternal(remoteImageTag: string): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageTag };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting/withRemoteImageTag',
            rpcArgs
    ...
    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withRemoteImageTagInternal(remoteImageTag));
    }

    /** @internal */
    private async _withPipelineStepFactoryInternal(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, dependsOn?: string[], requiredBy?: string[], tags?: string[], description?: string): Promise<NatsServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineStepContextHandle;
            const arg = new PipelineStepContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): NatsServerResourcePromise {
        const dependsOn = options?.dependsOn;
        const requiredBy = options?.requiredBy;
        const tags = options?.tags;
        const description = options?.description;
        return new NatsServerResourcePromise(this._withPipelineStepFactoryInternal(stepName, callback, dependsOn, requiredBy, tags, description));
    }

    /** @internal */
    private async _withPipelineConfigurationAsyncInternal(callback: (arg: PipelineConfigurationContext) => Promise<void>): Promise<NatsServerResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineConfigurationContextHandle;
            const arg = new PipelineConfigurationContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withPipelineConfigurationAsyncInternal(callback));
    }

    /** @internal */
    private async _withPipelineConfigurationInternal(callback: (obj: PipelineConfigurationContext) => Promise<void>): Promise<NatsServerResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as PipelineConfigurationContextHandle;
            const obj = new PipelineConfigurationContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._withPipelineConfigurationInternal(callback));
    }

    /** @internal */
    private async _withVolumeInternal(target: string, name?: string, isReadOnly?: boolean): Promise<NatsServerResource> {
    ...
    /** Adds a volume */
    withVolume(target: string, options?: WithVolumeOptions): NatsServerResourcePromise {
        const name = options?.name;
        const isReadOnly = options?.isReadOnly;
        return new NatsServerResourcePromise(this._withVolumeInternal(target, name, isReadOnly));
    }
    ...
    /** @internal */
    private async _withJetStreamInternal(): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting.Nats/withJetStream',
            rpcArgs
        );
        return new NatsServerResource(result, this._client);
    }
    ...
    /** @internal */
    private async _withDataVolumeInternal(name?: string, isReadOnly?: boolean): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (name !== undefined) rpcArgs.name = name;
        if (isReadOnly !== undefined) rpcArgs.isReadOnly = isReadOnly;
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting.Nats/withDataVolume',
            rpcArgs
        );
        return new NatsServerResource(result, this._client);
    }
    ...
    withDataVolume(options?: WithDataVolumeOptions): NatsServerResourcePromise {
        const name = options?.name;
        const isReadOnly = options?.isReadOnly;
        return new NatsServerResourcePromise(this._withDataVolumeInternal(name, isReadOnly));
    }

    /** @internal */
    private async _withDataBindMountInternal(source: string, isReadOnly?: boolean): Promise<NatsServerResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, source };
        if (isReadOnly !== undefined) rpcArgs.isReadOnly = isReadOnly;
        const result = await this._client.invokeCapability<NatsServerResourceHandle>(
            'Aspire.Hosting.Nats/withDataBindMount',
            rpcArgs
        );
        return new NatsServerResource(result, this._client);
    }
...
/**
 * Thenable wrapper for NatsServerResource that enables fluent chaining.
 * @example
 * await builder.addSomething().withX().withY();
 */
export class NatsServerResourcePromise implements PromiseLike<NatsServerResource> {
    constructor(private _promise: Promise<NatsServerResource>) {}

    then<TResult1 = NatsServerResource, TResult2 = never>(
        onfulfilled?: ((value: NatsServerResource) => TResult1 | PromiseLike<TResult1>) | null,
        onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null
    ): PromiseLike<TResult1 | TResult2> {
        return this._promise.then(onfulfilled, onrejected);
    }
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withContainerRegistry(registry)));
    }

    /** Adds a bind mount */
    withBindMount(source: string, target: string, options?: WithBindMountOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withBindMount(source, target, options)));
    }

    /** Sets the container entrypoint */
    withEntrypoint(entrypoint: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withEntrypoint(entrypoint)));
    }

    /** Sets the container image tag */
    withImageTag(tag: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withImageTag(tag)));
    }

    /** Sets the container image registry */
    withImageRegistry(registry: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withImageRegistry(registry)));
    }

    /** Sets the container image */
    withImage(image: string, options?: WithImageOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withImage(image, options)));
    }

    /** Sets the image SHA256 digest */
    withImageSHA256(sha256: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withImageSHA256(sha256)));
    }

    /** Adds runtime arguments for the container */
    withContainerRuntimeArgs(args: string[]): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withContainerRuntimeArgs(args)));
    }

    /** Sets the lifetime behavior of the container resource */
    withLifetime(lifetime: ContainerLifetime): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withLifetime(lifetime)));
    }

    /** Sets the container image pull policy */
    withImagePullPolicy(pullPolicy: ImagePullPolicy): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withImagePullPolicy(pullPolicy)));
    }

    /** Configures the resource to be published as a container */
    publishAsContainer(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.publishAsContainer()));
    }

    /** Configures the resource to use a Dockerfile */
    withDockerfile(contextPath: string, options?: WithDockerfileOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withDockerfile(contextPath, options)));
    }

    /** Sets the container name */
    withContainerName(name: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withContainerName(name)));
    }

    /** Adds a build argument from a parameter resource */
    withBuildArg(name: string, value: ParameterResource): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withBuildArg(name, value)));
    }

    /** Adds a build secret from a parameter resource */
    withBuildSecret(name: string, value: ParameterResource): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withBuildSecret(name, value)));
    }

    /** Configures endpoint proxy support */
    withEndpointProxySupport(proxyEnabled: boolean): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withEndpointProxySupport(proxyEnabled)));
    }

    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withDockerfileBaseImage(options)));
    }

    /** Adds a network alias for the container */
    withContainerNetworkAlias(alias: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withContainerNetworkAlias(alias)));
    }

    /** Configures an MCP server endpoint on the resource */
    withMcpServer(options?: WithMcpServerOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withMcpServer(options)));
    }

    /** Configures OTLP telemetry export */
    withOtlpExporter(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withOtlpExporter()));
    }

    /** Configures OTLP telemetry export with specific protocol */
    withOtlpExporterProtocol(protocol: OtlpProtocol): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withOtlpExporterProtocol(protocol)));
    }

    /** Publishes the resource as a connection string */
    publishAsConnectionString(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.publishAsConnectionString()));
    }

    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withRequiredCommand(command, options)));
    }

    /** Sets an environment variable */
    withEnvironment(name: string, value: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withEnvironment(name, value)));
    }

    /** Adds an environment variable with a reference expression */
    withEnvironmentExpression(name: string, value: ReferenceExpression): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withEnvironmentExpression(name, value)));
    }

    /** Sets environment variables via callback */
    withEnvironmentCallback(callback: (obj: EnvironmentCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withEnvironmentCallback(callback)));
    }

    /** Sets environment variables via async callback */
    withEnvironmentCallbackAsync(callback: (arg: EnvironmentCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withEnvironmentCallbackAsync(callback)));
    }

    /** Sets an environment variable from an endpoint reference */
    withEnvironmentEndpoint(name: string, endpointReference: EndpointReference): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withEnvironmentEndpoint(name, endpointReference)));
    }

    /** Sets an environment variable from a parameter resource */
    withEnvironmentParameter(name: string, parameter: ParameterResource): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withEnvironmentParameter(name, parameter)));
    }

    /** Sets an environment variable from a connection string resource */
    withEnvironmentConnectionString(envVarName: string, resource: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withEnvironmentConnectionString(envVarName, resource)));
    }

    /** Adds a connection property with a reference expression */
    withConnectionProperty(name: string, value: ReferenceExpression): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withConnectionProperty(name, value)));
    }

    /** Adds a connection property with a string value */
    withConnectionPropertyValue(name: string, value: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withConnectionPropertyValue(name, value)));
    }

    /** Adds arguments */
    withArgs(args: string[]): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withArgs(args)));
    }

    /** Sets command-line arguments via callback */
    withArgsCallback(callback: (obj: CommandLineArgsCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withArgsCallback(callback)));
    }

    /** Sets command-line arguments via async callback */
    withArgsCallbackAsync(callback: (arg: CommandLineArgsCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withArgsCallbackAsync(callback)));
    }

    /** Adds a reference to another resource */
    withReference(source: ResourceBuilderBase, options?: WithReferenceOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withReference(source, options)));
    }

    /** Adds a service discovery reference to another resource */
    withServiceReference(source: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withServiceReference(source)));
    }

    /** Adds a named service discovery reference */
    withServiceReferenceNamed(source: ResourceBuilderBase, name: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withServiceReferenceNamed(source, name)));
    }

    /** Adds a reference to a URI */
    withReferenceUri(name: string, uri: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withReferenceUri(name, uri)));
    }

    /** Adds a reference to an external service */
    withReferenceExternalService(externalService: ExternalServiceResource): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withReferenceExternalService(externalService)));
    }

    /** Adds a reference to an endpoint */
    withReferenceEndpoint(endpointReference: EndpointReference): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withReferenceEndpoint(endpointReference)));
    }

    /** Adds a network endpoint */
    withEndpoint(options?: WithEndpointOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withEndpoint(options)));
    }

    /** Adds an HTTP endpoint */
    withHttpEndpoint(options?: WithHttpEndpointOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withHttpEndpoint(options)));
    }

    /** Adds an HTTPS endpoint */
    withHttpsEndpoint(options?: WithHttpsEndpointOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withHttpsEndpoint(options)));
    }

    /** Makes HTTP endpoints externally accessible */
    withExternalHttpEndpoints(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withExternalHttpEndpoints()));
    }
    ...
    /** Configures resource for HTTP/2 */
    asHttp2Service(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.asHttp2Service()));
    }

    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withUrlsCallback(callback)));
    }

    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withUrlsCallbackAsync(callback)));
    }

    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withUrl(url, options)));
    }

    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withUrlExpression(url, options)));
    }

    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withUrlForEndpoint(endpointName, callback)));
    }

    /** Adds a URL for a specific endpoint via factory callback */
    withUrlForEndpointFactory(endpointName: string, callback: (arg: EndpointReference) => Promise<ResourceUrlAnnotation>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withUrlForEndpointFactory(endpointName, callback)));
    }

    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.excludeFromManifest()));
    }

    /** Waits for another resource to be ready */
    waitFor(dependency: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.waitFor(dependency)));
    }

    /** Waits for another resource with specific behavior */
    waitForWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.waitForWithBehavior(dependency, waitBehavior)));
    }

    /** Waits for another resource to start */
    waitForStart(dependency: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.waitForStart(dependency)));
    }

    /** Waits for another resource to start with specific behavior */
    waitForStartWithBehavior(dependency: ResourceBuilderBase, waitBehavior: WaitBehavior): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.waitForStartWithBehavior(dependency, waitBehavior)));
    }

    /** Prevents resource from starting automatically */
    withExplicitStart(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withExplicitStart()));
    }

    /** Waits for resource completion */
    waitForCompletion(dependency: ResourceBuilderBase, options?: WaitForCompletionOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.waitForCompletion(dependency, options)));
    }

    /** Adds a health check by key */
    withHealthCheck(key: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withHealthCheck(key)));
    }

    /** Adds an HTTP health check */
    withHttpHealthCheck(options?: WithHttpHealthCheckOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withHttpHealthCheck(options)));
    }

    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withCommand(name, displayName, executeCommand, options)));
    }

    /** Configures developer certificate trust */
    withDeveloperCertificateTrust(trust: boolean): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withDeveloperCertificateTrust(trust)));
    }

    /** Sets the certificate trust scope */
    withCertificateTrustScope(scope: CertificateTrustScope): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withCertificateTrustScope(scope)));
    }

    /** Configures HTTPS with a developer certificate */
    withHttpsDeveloperCertificate(options?: WithHttpsDeveloperCertificateOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withHttpsDeveloperCertificate(options)));
    }

    /** Removes HTTPS certificate configuration */
    withoutHttpsCertificate(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withoutHttpsCertificate()));
    }

    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withParentRelationship(parent)));
    }

    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withChildRelationship(child)));
    }

    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withIconName(iconName, options)));
    }

    /** Adds an HTTP health probe to the resource */
    withHttpProbe(probeType: ProbeType, options?: WithHttpProbeOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withHttpProbe(probeType, options)));
    }

    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.excludeFromMcp()));
    }

    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withRemoteImageName(remoteImageName)));
    }

    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withRemoteImageTag(remoteImageTag)));
    }

    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withPipelineStepFactory(stepName, callback, options)));
    }

    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withPipelineConfigurationAsync(callback)));
    }

    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withPipelineConfiguration(callback)));
    }

    /** Adds a volume */
    withVolume(target: string, options?: WithVolumeOptions): NatsServerResourcePromise {
        return new NatsServerResourcePromise(this._promise.then(obj => obj.withVolume(target, options)));
    }
...
registerHandleWrapper('Aspire.Hosting.Nats/Aspire.Hosting.ApplicationModel.NatsServerResource', (handle, client) => new NatsServerResource(handle as NatsServerResourceHandle, client));
