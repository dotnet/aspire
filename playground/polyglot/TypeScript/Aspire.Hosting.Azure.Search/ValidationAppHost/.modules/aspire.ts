...
/** Handle to AzureSearchResource */
type AzureSearchResourceHandle = Handle<'Aspire.Hosting.Azure.Search/Aspire.Hosting.Azure.AzureSearchResource'>;
...
export enum AzureSearchRole {
    SearchIndexDataContributor = "SearchIndexDataContributor",
...
export class DistributedApplicationBuilder {
    ...
    /** @internal */
    async _addAzureSearchInternal(name: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure.Search/addAzureSearch',
            rpcArgs
        );
        return new AzureSearchResource(result, this._client);
    }

    addAzureSearch(name: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._addAzureSearchInternal(name));
    }
...
export class DistributedApplicationBuilderPromise implements PromiseLike<DistributedApplicationBuilder> {
    ...
    /** Adds an Azure AI Search service resource */
    addAzureSearch(name: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.addAzureSearch(name)));
    }
...
export class AzureSearchResource extends ResourceBuilderBase<AzureSearchResourceHandle> {
    constructor(handle: AzureSearchResourceHandle, client: AspireClientRpc) {
        super(handle, client);
    }

    /** @internal */
    private async _withContainerRegistryInternal(registry: ResourceBuilderBase): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, registry };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/withContainerRegistry',
            rpcArgs
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withContainerRegistryInternal(registry));
    }

    /** @internal */
    private async _withDockerfileBaseImageInternal(buildImage?: string, runtimeImage?: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        if (buildImage !== undefined) rpcArgs.buildImage = buildImage;
        if (runtimeImage !== undefined) rpcArgs.runtimeImage = runtimeImage;
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
    ...
    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): AzureSearchResourcePromise {
        const buildImage = options?.buildImage;
        const runtimeImage = options?.runtimeImage;
        return new AzureSearchResourcePromise(this._withDockerfileBaseImageInternal(buildImage, runtimeImage));
    }

    /** @internal */
    private async _withRequiredCommandInternal(command: string, helpLink?: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, command };
        if (helpLink !== undefined) rpcArgs.helpLink = helpLink;
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/withRequiredCommand',
    ...
    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): AzureSearchResourcePromise {
        const helpLink = options?.helpLink;
        return new AzureSearchResourcePromise(this._withRequiredCommandInternal(command, helpLink));
    }

    /** @internal */
    private async _withConnectionPropertyInternal(name: string, value: ReferenceExpression): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
    ...
    withConnectionProperty(name: string, value: ReferenceExpression): AzureSearchResourcePromise {
    ...
    /** @internal */
    private async _withUrlsCallbackInternal(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): Promise<AzureSearchResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as ResourceUrlsCallbackContextHandle;
            const obj = new ResourceUrlsCallbackContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withUrlsCallbackInternal(callback));
    }

    /** @internal */
    private async _withUrlsCallbackAsyncInternal(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): Promise<AzureSearchResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ResourceUrlsCallbackContextHandle;
            const arg = new ResourceUrlsCallbackContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withUrlsCallbackAsyncInternal(callback));
    }

    /** @internal */
    private async _withUrlInternal(url: string, displayText?: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/withUrl',
    ...
    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): AzureSearchResourcePromise {
        const displayText = options?.displayText;
        return new AzureSearchResourcePromise(this._withUrlInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlExpressionInternal(url: ReferenceExpression, displayText?: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, url };
        if (displayText !== undefined) rpcArgs.displayText = displayText;
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/withUrlExpression',
    ...
    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): AzureSearchResourcePromise {
        const displayText = options?.displayText;
        return new AzureSearchResourcePromise(this._withUrlExpressionInternal(url, displayText));
    }

    /** @internal */
    private async _withUrlForEndpointInternal(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): Promise<AzureSearchResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const obj = wrapIfHandle(objData) as ResourceUrlAnnotation;
            await callback(obj);
        });
    ...
    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withUrlForEndpointInternal(endpointName, callback));
    }

    /** @internal */
    private async _excludeFromManifestInternal(): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/excludeFromManifest',
            rpcArgs
    ...
    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._excludeFromManifestInternal());
    }

    /** @internal */
    private async _withExplicitStartInternal(): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/withExplicitStart',
            rpcArgs
    ...
    /** Prevents resource from starting automatically */
    withExplicitStart(): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withExplicitStartInternal());
    }

    /** @internal */
    private async _withHealthCheckInternal(key: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, key };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/withHealthCheck',
            rpcArgs
    ...
    /** Adds a health check by key */
    withHealthCheck(key: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withHealthCheckInternal(key));
    }

    /** @internal */
    private async _withCommandInternal(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, commandOptions?: CommandOptions): Promise<AzureSearchResource> {
        const executeCommandId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as ExecuteCommandContextHandle;
            const arg = new ExecuteCommandContext(argHandle, this._client);
            return await executeCommand(arg);
    ...
    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): AzureSearchResourcePromise {
        const commandOptions = options?.commandOptions;
        return new AzureSearchResourcePromise(this._withCommandInternal(name, displayName, executeCommand, commandOptions));
    }

    /** @internal */
    private async _withParentRelationshipInternal(parent: ResourceBuilderBase): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, parent };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/withParentRelationship',
            rpcArgs
    ...
    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withParentRelationshipInternal(parent));
    }

    /** @internal */
    private async _withChildRelationshipInternal(child: ResourceBuilderBase): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, child };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/withChildRelationship',
            rpcArgs
    ...
    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withChildRelationshipInternal(child));
    }

    /** @internal */
    private async _withIconNameInternal(iconName: string, iconVariant?: IconVariant): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, iconName };
        if (iconVariant !== undefined) rpcArgs.iconVariant = iconVariant;
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/withIconName',
    ...
    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): AzureSearchResourcePromise {
        const iconVariant = options?.iconVariant;
        return new AzureSearchResourcePromise(this._withIconNameInternal(iconName, iconVariant));
    }

    /** @internal */
    private async _excludeFromMcpInternal(): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/excludeFromMcp',
            rpcArgs
    ...
    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._excludeFromMcpInternal());
    }

    /** @internal */
    private async _withRemoteImageNameInternal(remoteImageName: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageName };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/withRemoteImageName',
            rpcArgs
    ...
    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withRemoteImageNameInternal(remoteImageName));
    }

    /** @internal */
    private async _withRemoteImageTagInternal(remoteImageTag: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, remoteImageTag };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting/withRemoteImageTag',
            rpcArgs
    ...
    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withRemoteImageTagInternal(remoteImageTag));
    }

    /** @internal */
    private async _withPipelineStepFactoryInternal(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, dependsOn?: string[], requiredBy?: string[], tags?: string[], description?: string): Promise<AzureSearchResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineStepContextHandle;
            const arg = new PipelineStepContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): AzureSearchResourcePromise {
        const dependsOn = options?.dependsOn;
        const requiredBy = options?.requiredBy;
        const tags = options?.tags;
        const description = options?.description;
        return new AzureSearchResourcePromise(this._withPipelineStepFactoryInternal(stepName, callback, dependsOn, requiredBy, tags, description));
    }

    /** @internal */
    private async _withPipelineConfigurationAsyncInternal(callback: (arg: PipelineConfigurationContext) => Promise<void>): Promise<AzureSearchResource> {
        const callbackId = registerCallback(async (argData: unknown) => {
            const argHandle = wrapIfHandle(argData) as PipelineConfigurationContextHandle;
            const arg = new PipelineConfigurationContext(argHandle, this._client);
            await callback(arg);
    ...
    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withPipelineConfigurationAsyncInternal(callback));
    }

    /** @internal */
    private async _withPipelineConfigurationInternal(callback: (obj: PipelineConfigurationContext) => Promise<void>): Promise<AzureSearchResource> {
        const callbackId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as PipelineConfigurationContextHandle;
            const obj = new PipelineConfigurationContext(objHandle, this._client);
            await callback(obj);
    ...
    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withPipelineConfigurationInternal(callback));
    }
    ...
    /** @internal */
    private async _withParameterInternal(name: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/withParameter',
            rpcArgs
    ...
    /** Adds a Bicep parameter without a value */
    withParameter(name: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withParameterInternal(name));
    }

    /** @internal */
    private async _withParameterStringValueInternal(name: string, value: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/withParameterStringValue',
            rpcArgs
    ...
    /** Adds a Bicep parameter with a string value */
    withParameterStringValue(name: string, value: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withParameterStringValueInternal(name, value));
    }

    /** @internal */
    private async _withParameterStringValuesInternal(name: string, value: string[]): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/withParameterStringValues',
            rpcArgs
    ...
    /** Adds a Bicep parameter with a string list value */
    withParameterStringValues(name: string, value: string[]): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withParameterStringValuesInternal(name, value));
    }

    /** @internal */
    private async _withParameterFromParameterInternal(name: string, value: ParameterResource): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/withParameterFromParameter',
            rpcArgs
    ...
    /** Adds a Bicep parameter from a parameter resource builder */
    withParameterFromParameter(name: string, value: ParameterResource): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withParameterFromParameterInternal(name, value));
    }

    /** @internal */
    private async _withParameterFromConnectionStringInternal(name: string, value: ResourceBuilderBase): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/withParameterFromConnectionString',
            rpcArgs
    ...
    /** Adds a Bicep parameter from a connection string resource builder */
    withParameterFromConnectionString(name: string, value: ResourceBuilderBase): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withParameterFromConnectionStringInternal(name, value));
    }

    /** @internal */
    private async _withParameterFromOutputInternal(name: string, value: BicepOutputReference): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/withParameterFromOutput',
            rpcArgs
    ...
    /** Adds a Bicep parameter from another Bicep output reference */
    withParameterFromOutput(name: string, value: BicepOutputReference): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withParameterFromOutputInternal(name, value));
    }

    /** @internal */
    private async _withParameterFromReferenceExpressionInternal(name: string, value: ReferenceExpression): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/withParameterFromReferenceExpression',
            rpcArgs
    ...
    /** Adds a Bicep parameter from a reference expression */
    withParameterFromReferenceExpression(name: string, value: ReferenceExpression): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withParameterFromReferenceExpressionInternal(name, value));
    }

    /** @internal */
    private async _withParameterFromEndpointInternal(name: string, value: EndpointReference): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, value };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/withParameterFromEndpoint',
            rpcArgs
    ...
    /** Adds a Bicep parameter from an endpoint reference */
    withParameterFromEndpoint(name: string, value: EndpointReference): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._withParameterFromEndpointInternal(name, value));
    }

    /** @internal */
    private async _configureInfrastructureInternal(configure: (obj: AzureResourceInfrastructure) => Promise<void>): Promise<AzureSearchResource> {
        const configureId = registerCallback(async (objData: unknown) => {
            const objHandle = wrapIfHandle(objData) as AzureResourceInfrastructureHandle;
            const obj = new AzureResourceInfrastructure(objHandle, this._client);
            await configure(obj);
    ...
    configureInfrastructure(configure: (obj: AzureResourceInfrastructure) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._configureInfrastructureInternal(configure));
    }

    /** @internal */
    private async _publishAsConnectionStringInternal(): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/publishAsConnectionString',
            rpcArgs
    ...
    /** Publishes an Azure resource to the manifest as a connection string */
    publishAsConnectionString(): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._publishAsConnectionStringInternal());
    }
    ...
    /** @internal */
    private async _clearDefaultRoleAssignmentsInternal(): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/clearDefaultRoleAssignments',
            rpcArgs
    ...
    /** Clears the default Azure role assignments from a resource */
    clearDefaultRoleAssignments(): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._clearDefaultRoleAssignmentsInternal());
    }
    ...
    /** @internal */
    private async _runAsExistingFromParametersInternal(nameParameter: ParameterResource, resourceGroupParameter: ParameterResource): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, nameParameter, resourceGroupParameter };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/runAsExistingFromParameters',
            rpcArgs
    ...
    /** Marks an Azure resource as existing in run mode by using parameter resources */
    runAsExistingFromParameters(nameParameter: ParameterResource, resourceGroupParameter: ParameterResource): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._runAsExistingFromParametersInternal(nameParameter, resourceGroupParameter));
    }

    /** @internal */
    private async _runAsExistingInternal(name: string, resourceGroup: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, resourceGroup };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/runAsExisting',
            rpcArgs
    ...
    /** Marks an Azure resource as existing in run mode */
    runAsExisting(name: string, resourceGroup: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._runAsExistingInternal(name, resourceGroup));
    }

    /** @internal */
    private async _publishAsExistingFromParametersInternal(nameParameter: ParameterResource, resourceGroupParameter: ParameterResource): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, nameParameter, resourceGroupParameter };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/publishAsExistingFromParameters',
            rpcArgs
    ...
    /** Marks an Azure resource as existing in publish mode by using parameter resources */
    publishAsExistingFromParameters(nameParameter: ParameterResource, resourceGroupParameter: ParameterResource): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._publishAsExistingFromParametersInternal(nameParameter, resourceGroupParameter));
    }

    /** @internal */
    private async _publishAsExistingInternal(name: string, resourceGroup: string): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, name, resourceGroup };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/publishAsExisting',
            rpcArgs
    ...
    /** Marks an Azure resource as existing in publish mode */
    publishAsExisting(name: string, resourceGroup: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._publishAsExistingInternal(name, resourceGroup));
    }

    /** @internal */
    private async _asExistingInternal(nameParameter: ParameterResource, resourceGroupParameter: ParameterResource): Promise<AzureSearchResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, nameParameter, resourceGroupParameter };
        const result = await this._client.invokeCapability<AzureSearchResourceHandle>(
            'Aspire.Hosting.Azure/asExisting',
            rpcArgs
    ...
    /** Marks an Azure resource as existing in both run and publish modes */
    asExisting(nameParameter: ParameterResource, resourceGroupParameter: ParameterResource): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._asExistingInternal(nameParameter, resourceGroupParameter));
    }
 ...
 * await builder.addSomething().withX().withY();
 */
export class AzureSearchResourcePromise implements PromiseLike<AzureSearchResource> {
    constructor(private _promise: Promise<AzureSearchResource>) {}

    then<TResult1 = AzureSearchResource, TResult2 = never>(
        onfulfilled?: ((value: AzureSearchResource) => TResult1 | PromiseLike<TResult1>) | null,
        onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null
    ): PromiseLike<TResult1 | TResult2> {
        return this._promise.then(onfulfilled, onrejected);
    }
    ...
    /** Configures a resource to use a container registry */
    withContainerRegistry(registry: ResourceBuilderBase): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withContainerRegistry(registry)));
    }

    /** Sets the base image for a Dockerfile build */
    withDockerfileBaseImage(options?: WithDockerfileBaseImageOptions): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withDockerfileBaseImage(options)));
    }

    /** Adds a required command dependency */
    withRequiredCommand(command: string, options?: WithRequiredCommandOptions): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withRequiredCommand(command, options)));
    }
    ...
    /** Customizes displayed URLs via callback */
    withUrlsCallback(callback: (obj: ResourceUrlsCallbackContext) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withUrlsCallback(callback)));
    }

    /** Customizes displayed URLs via async callback */
    withUrlsCallbackAsync(callback: (arg: ResourceUrlsCallbackContext) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withUrlsCallbackAsync(callback)));
    }

    /** Adds or modifies displayed URLs */
    withUrl(url: string, options?: WithUrlOptions): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withUrl(url, options)));
    }

    /** Adds a URL using a reference expression */
    withUrlExpression(url: ReferenceExpression, options?: WithUrlExpressionOptions): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withUrlExpression(url, options)));
    }

    /** Customizes the URL for a specific endpoint via callback */
    withUrlForEndpoint(endpointName: string, callback: (obj: ResourceUrlAnnotation) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withUrlForEndpoint(endpointName, callback)));
    }

    /** Excludes the resource from the deployment manifest */
    excludeFromManifest(): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.excludeFromManifest()));
    }

    /** Prevents resource from starting automatically */
    withExplicitStart(): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withExplicitStart()));
    }

    /** Adds a health check by key */
    withHealthCheck(key: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withHealthCheck(key)));
    }

    /** Adds a resource command */
    withCommand(name: string, displayName: string, executeCommand: (arg: ExecuteCommandContext) => Promise<ExecuteCommandResult>, options?: WithCommandOptions): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withCommand(name, displayName, executeCommand, options)));
    }

    /** Sets the parent relationship */
    withParentRelationship(parent: ResourceBuilderBase): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withParentRelationship(parent)));
    }

    /** Sets a child relationship */
    withChildRelationship(child: ResourceBuilderBase): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withChildRelationship(child)));
    }

    /** Sets the icon for the resource */
    withIconName(iconName: string, options?: WithIconNameOptions): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withIconName(iconName, options)));
    }

    /** Excludes the resource from MCP server exposure */
    excludeFromMcp(): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.excludeFromMcp()));
    }

    /** Sets the remote image name for publishing */
    withRemoteImageName(remoteImageName: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withRemoteImageName(remoteImageName)));
    }

    /** Sets the remote image tag for publishing */
    withRemoteImageTag(remoteImageTag: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withRemoteImageTag(remoteImageTag)));
    }

    /** Adds a pipeline step to the resource */
    withPipelineStepFactory(stepName: string, callback: (arg: PipelineStepContext) => Promise<void>, options?: WithPipelineStepFactoryOptions): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withPipelineStepFactory(stepName, callback, options)));
    }

    /** Configures pipeline step dependencies via an async callback */
    withPipelineConfigurationAsync(callback: (arg: PipelineConfigurationContext) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withPipelineConfigurationAsync(callback)));
    }

    /** Configures pipeline step dependencies via a callback */
    withPipelineConfiguration(callback: (obj: PipelineConfigurationContext) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withPipelineConfiguration(callback)));
    }
    ...
    /** Adds a Bicep parameter without a value */
    withParameter(name: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withParameter(name)));
    }

    /** Adds a Bicep parameter with a string value */
    withParameterStringValue(name: string, value: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withParameterStringValue(name, value)));
    }

    /** Adds a Bicep parameter with a string list value */
    withParameterStringValues(name: string, value: string[]): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withParameterStringValues(name, value)));
    }

    /** Adds a Bicep parameter from a parameter resource builder */
    withParameterFromParameter(name: string, value: ParameterResource): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withParameterFromParameter(name, value)));
    }

    /** Adds a Bicep parameter from a connection string resource builder */
    withParameterFromConnectionString(name: string, value: ResourceBuilderBase): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withParameterFromConnectionString(name, value)));
    }

    /** Adds a Bicep parameter from another Bicep output reference */
    withParameterFromOutput(name: string, value: BicepOutputReference): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withParameterFromOutput(name, value)));
    }

    /** Adds a Bicep parameter from a reference expression */
    withParameterFromReferenceExpression(name: string, value: ReferenceExpression): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withParameterFromReferenceExpression(name, value)));
    }

    /** Adds a Bicep parameter from an endpoint reference */
    withParameterFromEndpoint(name: string, value: EndpointReference): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.withParameterFromEndpoint(name, value)));
    }

    /** Configures the Azure provisioning infrastructure callback */
    configureInfrastructure(configure: (obj: AzureResourceInfrastructure) => Promise<void>): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.configureInfrastructure(configure)));
    }

    /** Publishes an Azure resource to the manifest as a connection string */
    publishAsConnectionString(): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.publishAsConnectionString()));
    }
    ...
    /** Clears the default Azure role assignments from a resource */
    clearDefaultRoleAssignments(): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.clearDefaultRoleAssignments()));
    }
    ...
    /** Marks an Azure resource as existing in run mode by using parameter resources */
    runAsExistingFromParameters(nameParameter: ParameterResource, resourceGroupParameter: ParameterResource): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.runAsExistingFromParameters(nameParameter, resourceGroupParameter)));
    }

    /** Marks an Azure resource as existing in run mode */
    runAsExisting(name: string, resourceGroup: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.runAsExisting(name, resourceGroup)));
    }

    /** Marks an Azure resource as existing in publish mode by using parameter resources */
    publishAsExistingFromParameters(nameParameter: ParameterResource, resourceGroupParameter: ParameterResource): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.publishAsExistingFromParameters(nameParameter, resourceGroupParameter)));
    }

    /** Marks an Azure resource as existing in publish mode */
    publishAsExisting(name: string, resourceGroup: string): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.publishAsExisting(name, resourceGroup)));
    }

    /** Marks an Azure resource as existing in both run and publish modes */
    asExisting(nameParameter: ParameterResource, resourceGroupParameter: ParameterResource): AzureSearchResourcePromise {
        return new AzureSearchResourcePromise(this._promise.then(obj => obj.asExisting(nameParameter, resourceGroupParameter)));
    }
...
export class ConnectionStringResource extends ResourceBuilderBase<ConnectionStringResourceHandle> {
    ...
    /** @internal */
    private async _withSearchRoleAssignmentsInternal(target: AzureSearchResource, roles: AzureSearchRole[]): Promise<ConnectionStringResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, target, roles };
        const result = await this._client.invokeCapability<ConnectionStringResourceHandle>(
            'Aspire.Hosting.Azure.Search/withSearchRoleAssignments',
            rpcArgs
    ...
    /** Assigns Azure AI Search roles to a resource */
    withSearchRoleAssignments(target: AzureSearchResource, roles: AzureSearchRole[]): ConnectionStringResourcePromise {
        return new ConnectionStringResourcePromise(this._withSearchRoleAssignmentsInternal(target, roles));
    }
...
export class ConnectionStringResourcePromise implements PromiseLike<ConnectionStringResource> {
    ...
    /** Assigns Azure AI Search roles to a resource */
    withSearchRoleAssignments(target: AzureSearchResource, roles: AzureSearchRole[]): ConnectionStringResourcePromise {
        return new ConnectionStringResourcePromise(this._promise.then(obj => obj.withSearchRoleAssignments(target, roles)));
    }
...
export class ContainerRegistryResource extends ResourceBuilderBase<ContainerRegistryResourceHandle> {
    ...
    /** @internal */
    private async _withSearchRoleAssignmentsInternal(target: AzureSearchResource, roles: AzureSearchRole[]): Promise<ContainerRegistryResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, target, roles };
        const result = await this._client.invokeCapability<ContainerRegistryResourceHandle>(
            'Aspire.Hosting.Azure.Search/withSearchRoleAssignments',
            rpcArgs
    ...
    /** Assigns Azure AI Search roles to a resource */
    withSearchRoleAssignments(target: AzureSearchResource, roles: AzureSearchRole[]): ContainerRegistryResourcePromise {
        return new ContainerRegistryResourcePromise(this._withSearchRoleAssignmentsInternal(target, roles));
    }
...
export class ContainerRegistryResourcePromise implements PromiseLike<ContainerRegistryResource> {
    ...
    /** Assigns Azure AI Search roles to a resource */
    withSearchRoleAssignments(target: AzureSearchResource, roles: AzureSearchRole[]): ContainerRegistryResourcePromise {
        return new ContainerRegistryResourcePromise(this._promise.then(obj => obj.withSearchRoleAssignments(target, roles)));
    }
...
export class CSharpAppResource extends ResourceBuilderBase<CSharpAppResourceHandle> {
    ...
    /** @internal */
    private async _withSearchRoleAssignmentsInternal(target: AzureSearchResource, roles: AzureSearchRole[]): Promise<CSharpAppResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, target, roles };
        const result = await this._client.invokeCapability<CSharpAppResourceHandle>(
            'Aspire.Hosting.Azure.Search/withSearchRoleAssignments',
            rpcArgs
    ...
    /** Assigns Azure AI Search roles to a resource */
    withSearchRoleAssignments(target: AzureSearchResource, roles: AzureSearchRole[]): CSharpAppResourcePromise {
        return new CSharpAppResourcePromise(this._withSearchRoleAssignmentsInternal(target, roles));
    }
...
export class CSharpAppResourcePromise implements PromiseLike<CSharpAppResource> {
    ...
    /** Assigns Azure AI Search roles to a resource */
    withSearchRoleAssignments(target: AzureSearchResource, roles: AzureSearchRole[]): CSharpAppResourcePromise {
        return new CSharpAppResourcePromise(this._promise.then(obj => obj.withSearchRoleAssignments(target, roles)));
    }
...
export class DotnetToolResource extends ResourceBuilderBase<DotnetToolResourceHandle> {
    ...
    /** @internal */
    private async _withSearchRoleAssignmentsInternal(target: AzureSearchResource, roles: AzureSearchRole[]): Promise<DotnetToolResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, target, roles };
        const result = await this._client.invokeCapability<DotnetToolResourceHandle>(
            'Aspire.Hosting.Azure.Search/withSearchRoleAssignments',
            rpcArgs
    ...
    /** Assigns Azure AI Search roles to a resource */
    withSearchRoleAssignments(target: AzureSearchResource, roles: AzureSearchRole[]): DotnetToolResourcePromise {
        return new DotnetToolResourcePromise(this._withSearchRoleAssignmentsInternal(target, roles));
    }
...
export class DotnetToolResourcePromise implements PromiseLike<DotnetToolResource> {
    ...
    /** Assigns Azure AI Search roles to a resource */
    withSearchRoleAssignments(target: AzureSearchResource, roles: AzureSearchRole[]): DotnetToolResourcePromise {
        return new DotnetToolResourcePromise(this._promise.then(obj => obj.withSearchRoleAssignments(target, roles)));
    }
...
export class ExternalServiceResource extends ResourceBuilderBase<ExternalServiceResourceHandle> {
    ...
    /** @internal */
    private async _withSearchRoleAssignmentsInternal(target: AzureSearchResource, roles: AzureSearchRole[]): Promise<ExternalServiceResource> {
        const rpcArgs: Record<string, unknown> = { builder: this._handle, target, roles };
        const result = await this._client.invokeCapability<ExternalServiceResourceHandle>(
            'Aspire.Hosting.Azure.Search/withSearchRoleAssignments',
            rpcArgs
    ...
    /** Assigns Azure AI Search roles to a resource */
    withSearchRoleAssignments(target: AzureSearchResource, roles: AzureSearchRole[]): ExternalServiceResourcePromise {
        return new ExternalServiceResourcePromise(this._withSearchRoleAssignmentsInternal(target, roles));
    }
...
export class ExternalServiceResourcePromise implements PromiseLike<ExternalServiceResource> {
    ...
    /** Assigns Azure AI Search roles to a resource */
    withSearchRoleAssignments(target: AzureSearchResource, roles: AzureSearchRole[]): ExternalServiceResourcePromise {
        return new ExternalServiceResourcePromise(this._promise.then(obj => obj.withSearchRoleAssignments(target, roles)));
    }
...
registerHandleWrapper('Aspire.Hosting.Azure.Search/Aspire.Hosting.Azure.AzureSearchResource', (handle, client) => new AzureSearchResource(handle as AzureSearchResourceHandle, client));
