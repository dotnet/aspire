...
class CSharpAppResource extends ResourceBuilderBase {
    ...
    public IResourceWithEnvironment withOrleansClientReference(OrleansServiceClient orleansServiceClient) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("builder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("orleansServiceClient", AspireClient.serializeValue(orleansServiceClient));
        return (IResourceWithEnvironment) getClient().invokeCapability("Aspire.Hosting.Orleans/withOrleansClientReference", reqArgs);
    }

    /** Adds an Orleans silo reference to a resource */
    public IResourceWithEnvironment withOrleansReference(OrleansService orleansService) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("builder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("orleansService", AspireClient.serializeValue(orleansService));
        return (IResourceWithEnvironment) getClient().invokeCapability("Aspire.Hosting.Orleans/withOrleansReference", reqArgs);
    }
...
class IDistributedApplicationBuilder extends HandleWrapperBase {
    ...
    public OrleansService addOrleans(String name) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("builder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("name", AspireClient.serializeValue(name));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/addOrleans", reqArgs);
    }
...
/** Wrapper for Aspire.Hosting.Orleans/Aspire.Hosting.Orleans.OrleansService. */
class OrleansService extends HandleWrapperBase {
    OrleansService(Handle handle, AspireClient client) {
        super(handle, client);
    }

    /** Sets the Orleans cluster ID */
    public OrleansService withClusterId(String clusterId) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("clusterId", AspireClient.serializeValue(clusterId));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withClusterId", reqArgs);
    }

    /** Sets the Orleans service ID */
    public OrleansService withServiceId(String serviceId) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("serviceId", AspireClient.serializeValue(serviceId));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withServiceId", reqArgs);
    }

    /** Configures Orleans clustering using a resource connection */
    public OrleansService withClustering(IResourceWithConnectionString provider) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("provider", AspireClient.serializeValue(provider));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withClustering", reqArgs);
    }

    public OrleansService withClustering(ResourceBuilderBase provider) {
        return withClustering(new IResourceWithConnectionString(provider.getHandle(), provider.getClient()));
    }

    /** Configures Orleans development clustering */
    public OrleansService withDevelopmentClustering() {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withDevelopmentClustering", reqArgs);
    }

    /** Adds an Orleans grain storage provider */
    public OrleansService withGrainStorage(String name, IResourceWithConnectionString provider) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("name", AspireClient.serializeValue(name));
        reqArgs.put("provider", AspireClient.serializeValue(provider));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withGrainStorage", reqArgs);
    }

    public OrleansService withGrainStorage(String name, ResourceBuilderBase provider) {
        return withGrainStorage(name, new IResourceWithConnectionString(provider.getHandle(), provider.getClient()));
    }

    /** Adds in-memory Orleans grain storage */
    public OrleansService withMemoryGrainStorage(String name) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("name", AspireClient.serializeValue(name));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withMemoryGrainStorage", reqArgs);
    }

    /** Adds an Orleans stream provider */
    public OrleansService withStreaming(String name, IResourceWithConnectionString provider) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("name", AspireClient.serializeValue(name));
        reqArgs.put("provider", AspireClient.serializeValue(provider));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withStreaming", reqArgs);
    }

    public OrleansService withStreaming(String name, ResourceBuilderBase provider) {
        return withStreaming(name, new IResourceWithConnectionString(provider.getHandle(), provider.getClient()));
    }

    /** Adds in-memory Orleans streaming */
    public OrleansService withMemoryStreaming(String name) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("name", AspireClient.serializeValue(name));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withMemoryStreaming", reqArgs);
    }

    /** Adds an Orleans broadcast channel provider */
    public OrleansService withBroadcastChannel(String name) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("name", AspireClient.serializeValue(name));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withBroadcastChannel", reqArgs);
    }

    /** Configures Orleans reminder storage */
    public OrleansService withReminders(IResourceWithConnectionString provider) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("provider", AspireClient.serializeValue(provider));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withReminders", reqArgs);
    }

    public OrleansService withReminders(ResourceBuilderBase provider) {
        return withReminders(new IResourceWithConnectionString(provider.getHandle(), provider.getClient()));
    }

    /** Configures in-memory Orleans reminders */
    public OrleansService withMemoryReminders() {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withMemoryReminders", reqArgs);
    }

    /** Adds an Orleans grain directory provider */
    public OrleansService withGrainDirectory(String name, IResourceWithConnectionString provider) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansServiceBuilder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("name", AspireClient.serializeValue(name));
        reqArgs.put("provider", AspireClient.serializeValue(provider));
        return (OrleansService) getClient().invokeCapability("Aspire.Hosting.Orleans/withGrainDirectory", reqArgs);
    }

    public OrleansService withGrainDirectory(String name, ResourceBuilderBase provider) {
        return withGrainDirectory(name, new IResourceWithConnectionString(provider.getHandle(), provider.getClient()));
    }

    /** Creates an Orleans client view for the service */
    public OrleansServiceClient asClient() {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("orleansService", AspireClient.serializeValue(getHandle()));
        return (OrleansServiceClient) getClient().invokeCapability("Aspire.Hosting.Orleans/asClient", reqArgs);
    }

}

/** Wrapper for Aspire.Hosting.Orleans/Aspire.Hosting.Orleans.OrleansServiceClient. */
class OrleansServiceClient extends HandleWrapperBase {
    OrleansServiceClient(Handle handle, AspireClient client) {
        super(handle, client);
    }
...
class AspireRegistrations {
    static {
        ...
        AspireClient.registerHandleWrapper("Aspire.Hosting.Orleans/Aspire.Hosting.Orleans.OrleansService", (h, c) -> new OrleansService(h, c));
        AspireClient.registerHandleWrapper("Aspire.Hosting.Orleans/Aspire.Hosting.Orleans.OrleansServiceClient", (h, c) -> new OrleansServiceClient(h, c));
