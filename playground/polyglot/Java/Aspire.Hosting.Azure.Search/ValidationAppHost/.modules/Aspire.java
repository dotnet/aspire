...
/** AzureSearchRole enum. */
enum AzureSearchRole implements WireValueEnum {
    SEARCH_INDEX_DATA_CONTRIBUTOR("SearchIndexDataContributor"),
    SEARCH_INDEX_DATA_READER("SearchIndexDataReader"),
    SEARCH_SERVICE_CONTRIBUTOR("SearchServiceContributor");

    private final String value;

    AzureSearchRole(String value) {
        this.value = value;
    }

    public String getValue() { return value; }

    public static AzureSearchRole fromValue(String value) {
        for (AzureSearchRole e : values()) {
            if (e.value.equals(value)) return e;
        }
        throw new IllegalArgumentException("Unknown value: " + value);
    }
...
class AzureBicepResource extends ResourceBuilderBase {
    ...
    public IResource withSearchRoleAssignments(AzureSearchResource target, AzureSearchRole[] roles) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("builder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("target", AspireClient.serializeValue(target));
        reqArgs.put("roles", AspireClient.serializeValue(roles));
        return (IResource) getClient().invokeCapability("Aspire.Hosting.Azure.Search/withSearchRoleAssignments", reqArgs);
    }
...
/** Wrapper for Aspire.Hosting.Azure.Search/Aspire.Hosting.Azure.AzureSearchResource. */
class AzureSearchResource extends ResourceBuilderBase {
    AzureSearchResource(Handle handle, AspireClient client) {
        super(handle, client);
    }
...
class IDistributedApplicationBuilder extends HandleWrapperBase {
    ...
    public AzureSearchResource addAzureSearch(String name) {
        Map<String, Object> reqArgs = new HashMap<>();
        reqArgs.put("builder", AspireClient.serializeValue(getHandle()));
        reqArgs.put("name", AspireClient.serializeValue(name));
        return (AzureSearchResource) getClient().invokeCapability("Aspire.Hosting.Azure.Search/addAzureSearch", reqArgs);
    }
...
class AspireRegistrations {
    static {
        ...
        AspireClient.registerHandleWrapper("Aspire.Hosting.Azure.Search/Aspire.Hosting.Azure.AzureSearchResource", (h, c) -> new AzureSearchResource(h, c));
