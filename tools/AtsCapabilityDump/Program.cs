...
public static class Program
{
    ...
    {
        // Estimate initial capacity based on typical output size (roughly 100 chars per capability)
        var estimatedSize = (result.Capabilities.Count + result.HandleTypes.Count + result.DtoTypes.Count + result.EnumTypes.Count) * 100;
        var sb = new StringBuilder(Math.Max(4096, estimatedSize));
    ...
    {
        // Estimate initial capacity for JSON output (roughly 200 chars per item)
    ...
    {
        // Use JsonEncodedText for proper JSON escaping
